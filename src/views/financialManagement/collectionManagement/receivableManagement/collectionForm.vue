<template>
  <el-dialog append-to-body title="收款" :close-on-click-modal="false" :close-on-press-escape="false"
    ...
    <el-form ref="dataForm" v-loading="formLoading" :model="dataForm" :rules="dataRule" label-position="top"
      ...
      <el-row :gutter="30">
        ...
        <el-col :span="12">
          <el-form-item :label="Number(dataForm.totalReconciliationAmount) > 0 ? '应收金额' : '应退金额'"
            prop="totalReconciliationAmount">
            <el-input v-model="dataForm.totalReconciliationAmount"
              :placeholder="Number(dataForm.totalReconciliationAmount) > 0 ? '应收金额' : '应退金额'" maxlength="20" disabled />
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item :label="Number(dataForm.totalReconciliationAmount) > 0 ? '待收金额' : '待退金额'" prop="dueAmount">
            <el-input v-model="dataForm.dueAmount"
              :placeholder="Number(dataForm.totalReconciliationAmount) > 0 ? '待收金额' : '待退金额'" maxlength="20" disabled />
          </el-form-item>
        </el-col>

        <el-col :span="12">
          <el-form-item :label="Number(dataForm.totalReconciliationAmount) > 0 ? '收款方式' : '退款方式'" prop="paymentMethod">
            <el-select v-model="dataForm.paymentMethod"
              :placeholder="Number(dataForm.totalReconciliationAmount) > 0 ? '请选择收款方式' : '请选择退款方式'" clearable
              style="width: 100%;">
        ...
        <el-col :span="12">
          <el-form-item :label="Number(dataForm.totalReconciliationAmount) > 0 ? '收款金额' : '退款金额'" prop="paymentAmount">
            <el-input v-model="dataForm.paymentAmount"
              :placeholder="Number(dataForm.totalReconciliationAmount) > 0 ? '请输入收款金额' : '请输入退款金额'" maxlength="20" />
          </el-form-item>
        </el-col>

        <el-col :span="12">

          <el-form-item :label="Number(dataForm.totalReconciliationAmount) > 0 ? '收款日期' : '退款日期'" prop="paymentDate">
            <el-date-picker v-model="dataForm.paymentDate" type="date" value-format="yyyy-MM-dd" style="width: 100%;"
              :placeholder="Number(dataForm.totalReconciliationAmount) > 0 ? '请选择收款日期' : '请选择退款日期'">
            </el-date-picker>
          </el-form-item>
        </el-col>
    ...
    return {
      ...
      dataRule: {
        ...
        paymentAmount: [
          ...
          { validator: this.formValidate({ type: 'decimal', params: [10, 2, "", (errMsg) => { this.$message.error('金额' + errMsg) }] }), trigger: 'blur' },
          { validator: this.formValidate({ type: 'decimal2', params: [10, 2, "", (errMsg) => { this.$message.error('金额' + errMsg) }] }), trigger: 'blur' },
          // { validator: this.numCalcMethod3(), trigger: 'blur' },
          { validator: this.numCalcMethod2(), trigger: 'blur' },
          { validator: this.numCalcMethod(), trigger: 'blur' },
          // { validator: this.formValidate('noZero', `${this.noZero}`, ), trigger: 'blur' }
        ],
      },
    }
  },
  ...
  methods: {
    numCalcMethod() {
      return (rule, value, callback) => {
        let msg = Number(this.dataForm.totalReconciliationAmount) > 0 ? "收款金额不能超过待收金额" : '退款金额不能超过待退款金额'
        if (!value) { callback() }
        else {
          if (value > 0) {
    ...
    //     if (Number(this.dataForm.totalReconciliationAmount) > 0) {
    //       if (Number(value) > 0) {
    //         let reg = new RegExp(`^(0|[1-9](\\d{0,7}))((\\.\\d{1,2})?)$`);
    //         let msg = `请输入正确的数字，整数不能超过8位，小数不能超过2位`
      ...
      this.$nextTick(() => {
        ...
        getfinAccountDetail(this.dataForm.accountsReceivableReconciliationId).then(res => {
          ...
          if (Number(this.dataForm.totalReconciliationAmount) < 0) {
            ...
            console.log(this.dataRule.paymentAmount);
            this.dataRule.paymentAmount.splice(1,1)
          } else {
            ...
            this.dataRule.paymentAmount.splice(2,1)
          }
        })
