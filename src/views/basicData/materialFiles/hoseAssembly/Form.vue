  ...
  <transition name="el-zoom-in-center">
    <div class="JNPF-preview-main org-form">
      <div :class="['JNPF-common-page-header', btnType ? 'noButtons' : '']">
        <el-page-header @back="goBack" :content="btnType ? '查看软管总成档案' : title" />
...
export default {
          ...
          tc.itemRules.push({
            validator: (rule, value, callback) => {
              if (!value) { callback() }
                ...
                checkCodeExist({ classAttribute: "finish_product", code: this.dataForm.code }).then((res) => {
  ...
  computed: {
    openMode() {
      return this.title === '新建软管总成档案' ? '新建' : this.title === '编辑软管总成档案' ? '编辑' : '只读'
    }
  },
  methods: {
    // 渲染自定义属性表单
    ...
    init(id, btnType = false) {
      ...
      if (!!id) {
        this.dataForm.id = id
        this.title = btnType ? '查看软管总成档案' : '编辑软管总成档案'
        // 获取详情
