    ...
    <div class="JNPF-preview-main org-form">
      <div class="JNPF-common-page-header">
        ...
        <el-page-header @back="goBack"
          :content="btnType ? '查看客户档案' : !parentId ? $t(`customer.addCustomer`) : $t(`customer.editCustomer`)" />
              ...
              <el-row :gutter="30" class="custom-row">
                ...
                <el-col :sm="8" :xs="24">
                  <el-form-item label="客户名称" prop="name">
                    <el-input v-model="dataForm.name" placeholder="请输入客户名称" :disabled="btnType ? true : false"
                      maxlength="100" />
                  </el-form-item>
                </el-col>
                <el-col :sm="8" :xs="24">
                ...
                <el-col :sm="8" :xs="24">
                  <el-form-item label="等级" prop="grade">
                    <el-select v-model="dataForm.grade" placeholder="请选择等级" :disabled="btnType ? true : false"
                      style="width: 100%;">
                ...
                <el-col :sm="8" :xs="24">
                  <el-form-item label="地区" prop="regionCode">
                    <el-select v-model="dataForm.regionCode" placeholder="请选择地区" style="width: 100%;"
                      @change="handleChange" :disabled="btnType ? true : false">
          ...
          <el-tab-pane label="收货信息" name="second">
            <el-table :data="deliveryAddressList" style="width: 100%">
                ...
                <template slot-scope="scope">
                  <el-select v-model="scope.row.province" placeholder="请选择省份"
                    ...
                    <el-option v-for="item in provinces" :key="item.id" :label="item.fullName" :value="item.id"
                      @click.native="changeProvince(item, scope.row)"></el-option>
                  </el-select>
                </template>
              </el-table-column>
              <el-table-column prop="city" label="市" width="180">
                ...
                <template slot-scope="scope">
                  <el-select v-model="scope.row.city" placeholder="请选择城市"
                    :disabled="scope.row.country !== 'CN' ? true : btnType ? true : false">
                    <el-option v-for="item in cities" :key="item.id" :label="item.fullName" :value="item.id"
                      @click.native="changeCity(item, scope.row)"></el-option>
                  </el-select>
                </template>
              </el-table-column>
              <el-table-column prop="area" label="区" width="180">
                ...
                <template slot-scope="scope">
                  <el-select v-model="scope.row.area" placeholder="请选择区"
                    :disabled="scope.row.country !== 'CN' ? true : btnType ? true : false">
                    <el-option v-for="item in area" :key="item.id" :label="item.fullName" :value="item.id"></el-option>
                  </el-select>
                </template>
              </el-table-column>
              <el-table-column prop="address" label="地址" width="220">
          ...
          this.$nextTick(() => {
            ...
            if (this.dataForm.departmentId) {
              getOrganizeInfo(this.dataForm.departmentId).then(sss => {
                ...
                this.organizeIdTree.push(this.dataForm.departmentId)
                console.log(8888, this.organizeIdTree);
              })
            }
