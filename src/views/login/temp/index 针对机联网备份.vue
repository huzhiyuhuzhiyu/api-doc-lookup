<template>
  <div class="login-container">
    ...
    <el-image v-show="isbgload" class="login-bg" :src="define.comUrl+sysConfig.loginBg" @load="bgload" />
    <img v-show="!isbgload" class="login-bg" src="@/assets/images/login_bg.png" alt=""></img>

    <div class="body">
      <div style="opacity: .8;">
        <div v-for="(value,key) in berning" :style="value" class="ber" :class="222" />
      </div>
      <div class="body_box">
        <div class="body_right">
          ...
          <div class="login-content">
            <div class="login-form">
              ...
              >
                <el-form-item>
                  <el-select v-show="showTenancy" v-model="loginForm.sys" class="sel-item" size="medium" @change="getConfig">
                    <el-option v-for="(value,key) in tanants" :key="key" :value="value.enCode" :label="value.fullName" />
                  </el-select>
                </el-form-item>
    ...
    <p class="componey">
      ...
      {{ sysConfig.companyTelePhone ? sysConfig.companyTelePhone : "0574-89079512" }}
            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
      <a :href="(sysConfig.companyUrl ? sysConfig.companyUrl : 'http://www.nbjuxuan.com')" target="_blank">{{ sysConfig.copyright ? sysConfig.copyright : "Copyright @ 2012 宁波聚轩信息科技有限公司版权所有" }}</a>
      &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
...
export default {
  ...
  created() {
    ...
    for (var i = 0; i < 5; i++) {
      ...
      const s = Math.floor(Math.random() * 300 + 50)
      const dir = 'ber' + Math.round(Math.random())
      const bers = {
        width: s + 'px',
        height: s + 'px',
        left: x + 'vw',
    ...
    getConfig('isTenancy').then(res => {
      ...
      if (this.isTenancy) {
        this.showTenancy = (this.argSys == 'all' || location.host.indexOf('localhost') > -1)
        if (this.showTenancy) {
      ...
      this.getConfig(this.loginForm.sys + '@iot')
    })
