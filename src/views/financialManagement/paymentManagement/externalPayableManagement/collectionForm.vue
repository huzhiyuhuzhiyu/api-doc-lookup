      ...
      <el-row :gutter="30">
        <el-col :span="12">
          <el-form-item label="对账单号" prop="orderNo">
            <el-input v-model="dataForm.orderNo" placeholder="请输入对账单号" maxlength="20"
              disabled />
          </el-form-item>
        </el-col>

        <el-col :span="12">
          <el-form-item :label="Number(dataForm.totalReconciliationAmount) >= 0 ? '应付金额' : '应退金额'" prop="totalReconciliationAmount">
            <el-input v-model="dataForm.totalReconciliationAmount" :placeholder="Number(dataForm.totalReconciliationAmount) >= 0 ? '应付金额' : '应退金额'" maxlength="20" disabled />
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item :label="Number(dataForm.totalReconciliationAmount) >= 0 ? '待付金额' : '待退金额'" prop="dueAmount">
            <el-input v-model="dataForm.dueAmount" :placeholder="Number(dataForm.totalReconciliationAmount) >= 0 ? '待付金额' : '待退金额'" maxlength="20" disabled />
          </el-form-item>
        </el-col>

        <el-col :span="12">
          <el-form-item :label="Number(dataForm.totalReconciliationAmount) >= 0 ? '付款方式' : '退款方式'" prop="paymentMethod">
            <el-select v-model="dataForm.paymentMethod" :placeholder="Number(dataForm.totalReconciliationAmount) >= 0 ? '请选择付款方式' : '请选择退款方式'" clearable style="width: 100%;" >
        ...
        <el-col :span="12">
          <el-form-item :label="Number(dataForm.totalReconciliationAmount) >= 0 ? '付款金额' : '退款金额'" prop="paymentAmount">
            <el-input v-model="dataForm.paymentAmount" :placeholder="Number(dataForm.totalReconciliationAmount) >= 0 ? '请输入付款金额' : '请输入退款金额'" maxlength="20"  />
          </el-form-item>
        </el-col>
        ...
        <el-col :span="12">
          ...
          <el-form-item :label="Number(dataForm.totalReconciliationAmount) >= 0 ? '付款日期' : '退款日期'" prop="paymentDate">
            <el-date-picker v-model="dataForm.paymentDate" type="date" value-format="yyyy-MM-dd"
                style="width: 100%;"  :placeholder="Number(dataForm.totalReconciliationAmount) >= 0 ? '请选择付款日期' : '请选择退款日期'">
            </el-date-picker>
          </el-form-item>
        </el-col>
