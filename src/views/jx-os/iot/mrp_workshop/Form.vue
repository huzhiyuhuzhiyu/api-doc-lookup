        ...
        <template v-if="!loading">
          ...
          <el-col :span="24">
            <el-form-item label="上级名称" prop="parentId">
              <JNPFTreeSelect v-model="dataForm.parentId" placeholder="请选择" clearable :options="parentIdOptions"
                :props="parentIdProps">
          ...
          <el-col :span="24">
            <el-form-item label="负责人" prop="employeeId">
              <user-select v-model="dataForm.employeeId" placeholder="请选择" clearable>
  ...
  export default {
    ...
    methods: {
      ...
      init(id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.loading = true
            request({
              url: '/api/mrp_workshop/Mrp_workshop/' + this.dataForm.id,
              method: 'get'
            }).then(res => {
      ...
      request() {
        ...
        if (!this.dataForm.id) {
          request({
            url: '/api/mrp_workshop/Mrp_workshop',
            method: 'post',
            data: _data
          }).then((res) => {
            this.$message({
