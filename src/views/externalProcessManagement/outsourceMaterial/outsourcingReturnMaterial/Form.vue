                ...
                <el-form ref="elForm" :model="dataForm" :rules="rules" size="small" label-width="100px"
                  ...
                  <el-col :sm="8" :xs="24" v-if="type === 'look'">
                    <el-form-item label="退料状态" prop="deliveryStatus" ref="deliveryStatus">
                      <el-input type="text" :value="dataForm.deliveryStatus === 'not_returned' ? '未完成' : '已完成'"
                  ...
                  <el-col :sm="8" :xs="24" v-if="type === 'look'">
                    <el-form-item label="检验状态" prop="inspectionStatus" ref="inspectionStatus">
                      <el-input type="text"
                        :value="dataForm.inspectionStatus === 'unInspect' ? '待检验' : dataForm.inspectionStatus === 'inspecting' ? '检验中' : '已检验'"
                ...
                linesData.forEach(item => {
                  _data.forEach(item2 => {
                    if (item.id === item2.ordersLineId) {
                      ...
                      item2.deliveryQuantity = ''
                      item2.productsId = item.productId
                      item2.id = ''
                    }
                  })
                })
              ...
              if (!valid_2) {
                ...
                if (this.type === 'add' || this.type === 'copy') {
                  dataFormTwoData = this.dataFormTwo.data.map(item => {
                    return {
                      ...item,
                      ordersLineId: item.id ? item.id : item.ordersLineId,
                      notifyType: "external",
                      ordersId: this.dataForm.ordersId,
                      id: null,
                      outboundQuantity: 0,
                      productsId: item.productId ? item.productId : item.productsId
                    }
                  })
                  _data = {
      ...
      outPoolexternal(this.dataForm.ordersId).then(res => {
        ...
        this.dataFormTwo.data = this.dataFormTwo.data.map(item => {
          return {
            ...
            deliveryQuantity: Number(item.outboundQuantity - item.returnQuantity) > 0 ? Number(item.outboundQuantity - item.returnQuantity) : 0,
