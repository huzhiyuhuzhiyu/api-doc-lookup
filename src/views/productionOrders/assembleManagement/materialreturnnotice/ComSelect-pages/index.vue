        ...
        <div class="JNPF-common-layout-center JNPF-flex-main">
          <el-row class="JNPF-common-search-box" :gutter="16">
            <el-form @submit.native.prevent>
              <template v-for="(item, index) in searchList">
                <el-col :span="5" :key="item.prop" v-if="index <= 2"
                  ...
                  <el-form-item>
                    ...
                    <el-date-picker v-else-if="item.type === 'date'" v-model="deliveryDate" type="daterange" value-format="yyyy-MM-dd" clearable :start-placeholder='"开始"+item.label' :end-placeholder='"结束"+item.label'>
                    </el-date-picker>
                  </el-form-item>
                </el-col>
              </template>
              ...
              <el-button v-if="searchList.length > 3" style="float: right;margin-right: 20px;" size="mini" type="primary"
          ...
          <div class="JNPF-common-layout-main JNPF-flex-main">
            <JNPF-table v-loading="treeLoading || listLoading" :data="tableData" hasNO fixedNO :hasC="multiple"
              ...
              :row-class-name="getRowClassName" :checkSelectable="checkSelectable" ref="dataTable" @sort-change="sortChange"
              @row-click="handleRowClick" :tree-props="{ children: 'childrenList', hasChildren: '' }" row-key="index"
              :default-expand-all="expandsTable">

              <!-- 普通结构 -->
              <template v-if="!listDataTreeFlag && picktype">
                <el-table-column prop="orderNo" label="订单号" width="200"/>
                <el-table-column prop="productCode" label="产品编码" width="180"/>
                <el-table-column prop="productName" label="产品名称" width="180" sortable="custom"/>
                ...
                <el-table-column prop="productionQuantity" label="生产数量" width="140"/>
                <el-table-column prop="blankProductsName" label="下料产品" width="180" sortable="custom"/>
                <el-table-column prop="routingCode" label="工艺路线编码" width="180"/>
                <el-table-column prop="routingName" label="工艺路线名称" width="180" sortable="custom"/>
                <!-- <el-table-column prop="pickingStatus" label="领料状态">
                  <template slot-scope="scope">
                    <div v-if="scope.row.pickingStatus == 'picked'">已领料</div>
                    <div v-else-if="scope.row.pickingStatus == 'not_picked'">未领料</div>
                    <div v-else-if="scope.row.pickingStatus == 'picking'">领料中</div>
                  </template>
                </el-table-column> -->
              </template>
              <template v-if="!listDataTreeFlag && !picktype">
        ...
        <el-form ref="diaForm" :model="listQuery" label-width="120px" label-position="top">
          ...
          <el-col :span="12" v-for="item in searchList" :key="item.prop">
            <el-form-item :label="item.label">
              ...
              <el-date-picker v-else-if="item.type === 'date'" style="width: 100%;" v-model="deliveryDate" type="daterange" value-format="yyyy-MM-dd" clearable :start-placeholder='"开始"+item.label' :end-placeholder='"结束"+item.label'>
              </el-date-picker>
            </el-form-item>
          </el-col>
        </el-form>
...
export default {
  ...
  methods: {
    sortChange({prop, order}){
      let newProp;
      if (prop === 'productName' || prop ==='blankProductsName' || prop === 'routingName') {
        newProp = prop
      } else {
    ...
    initData() {
      ...
      if (this.deliveryDate && this.deliveryDate.length > 0) {
        this.listQuery[this.datetype.start] = this.deliveryDate[0]
        this.listQuery[this.datetype.end] = this.deliveryDate[1]
      } else {
        this.listQuery[this.datetype.start] = ''
        this.listQuery[this.datetype.end] = ''
      }
