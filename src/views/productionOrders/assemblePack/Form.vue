    ...
    <transition name="el-zoom-in-center">
      <div class="JNPF-preview-main org-form">
        <div :class="['JNPF-common-page-header', type === 'look' ? 'noButtons' : '']">
          <el-page-header @back="goBack" :content="type == 'add' ? '新建组装生产装箱单' : type == 'edit' ? '编辑组装生产装箱单' : '查看组装生产装箱单'" />
...
<script>
import { addSalePackingList, updateSalePackingList, detailSalePackingList, prodOrderPackingList } from '@/api/warehouseManagement/packingList'
      ...
      if (data.length) {
        ...
        list.forEach(item => {
          if (this.dataForm.packingType === 'mixed') {
            ...
            _data.push({
              waitPackingQuantity: this.jnpf.math('subtract', [item.completedQuantity, item.packingQuantity]),
              totalNum: this.jnpf.math('subtract', [item.completedQuantity, item.packingQuantity]),
              cooperativePartnerId: item.cooperativePartnerId,
              calculationDirection: item.calculationDirection,
              deputyUnit: item.deputyUnit,
              ...
              oddBoxName: item.packagingMaterialsName,
              oddBox: item.packagingMaterials,
              num: Number(item.productPacking) ? Math.ceil(Number(item.completedQuantity -
                item.packingQuantity) / Number(item.productPacking)) : 0,
              packingQuantity: 0,         // 已装箱
              packedQuantity: '',         // 待装箱
            ...
            if (valid_2) {
              if (this.dataForm.packingType === 'single') {
                ...
                if (this.mixedTableData.length === 0) {
                  ...
                  //         product.returnDeliveryNoticeLineId == obj.returnDeliveryNoticeLineId
                  //       );
                  //       if (existingObj) {
