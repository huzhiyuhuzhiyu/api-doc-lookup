        ...
        <div class="main" v-loading="listLoading">
          ...
          <div v-if="type !== 'look'">
            ...
            <el-button type="text" style="margin-right:8px;margin-left:8px; font-size:14px!important" icon="el-icon-plus"
              @click="addAdjustmentBtn()">增加调价行</el-button>
          </div>

          <el-form :model="dataFormTwo" v-bind="dataFormTwo" ref="productForm">
            <el-table style="border: 1px solid #e3e7ee;" hasNO fixedNO v-bind="dataFormTwo.data" :data="dataFormTwo.data"
              id="table">
              ...
              <el-table-column prop="receiptReturnType" label="收/退货类型" min-width="200" show-overflow-tooltip>
                <template slot-scope="scope">
                  <el-form-item :prop="'data.' + scope.$index + '.' + 'receiptReturnType'">
                    <div class="viewData">
                      <span>{{ scope.row.receiptReturnType === 'receipt' ? '收货' : scope.row.receiptReturnType === 'back' ?
                        '退货' : '' }}</span>
                    </div>
                  </el-form-item>
                </template>
                ...
                <template slot-scope="scope">
                  <el-form-item :prop="'data.' + scope.$index + '.' + 'excludingTaxAmount'"
                    ...
                    <div :class="['viewData', scope.row.receiptReturnType === 'receipt' ? 'green' : 'red']">
                      ...
                      <span v-if="scope.row.receiptReturnType === 'receipt'">{{ scope.row.excludingTaxAmount }}</span>
                      <span v-else-if="scope.row.receiptReturnType === 'back'">{{ scope.row.excludingTaxAmount }}</span>
              ...
              <el-table-column prop="taxAmount" label="税额" min-width="120" fixed="right">
                <template slot-scope="scope">
                  <el-form-item :prop="'data.' + scope.$index + '.' + 'taxAmount'" :rules="productFormRules.taxAmount">
                    <div :class="['viewData', scope.row.receiptReturnType === 'receipt' ? 'green' : 'red']">
                ...
                <template slot-scope="scope">
                  <el-form-item :prop="'data.' + scope.$index + '.' + 'includingTaxAmount'"
                    ...
                    <div :class="['viewData', scope.row.receiptReturnType === 'receipt' ? 'green' : 'red']">
                      ...
                      <span v-if="scope.row.receiptReturnType === 'receipt'">{{ scope.row.includingTaxAmount }}</span>
                      <span v-else-if="scope.row.receiptReturnType === 'back'">{{ scope.row.includingTaxAmount }}</span>
          ...
          <div class="text" style="height: 40px; line-height: 40px; background: #f5f7fa;">
            ...
            <span style="font-weight:500;margin-right:10px"> 不含税金额：<span
                :class="computedValue > 0 ? 'green' : 'red'">{{ computedValue > 0 ? '+' + computedValue : computedValue
                }}</span></span>
  ...
  methods: {
    ...
    init(row,data,type) {
      ...
      this.dataFormTwo.data.forEach((item, index) => {
        if (item.receiptReturnType === 'receipt') {
          console.log(666,item.excludingTaxAmount,item.taxAmount);
          this.includingTaxAmount += this.jnpf.numberFormat(item.excludingTaxAmount + item.taxAmount)
        } else {
          console.log(555,item.excludingTaxAmount,item.taxAmount);
