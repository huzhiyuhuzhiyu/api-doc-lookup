<template>
  <div>
    <transition name="el-zoom-in-center">
      <div class="JNPF-preview-main org-form">
        <div :class="['JNPF-common-page-header', type === 'look' ? 'noButtons' : '']">
          <el-page-header @back="goBack" :content="type === 'look' ? '查看外协订单' : '查看外协订单明细'" />
          <div class="options">
          ...
          <el-tabs v-model="activeName">
            <el-tab-pane label="基础信息" name="jcInfo">
                  ...
                  <el-col :sm="8" :span="12">
                    <el-form-item label="交货日期" prop="deliveryDate">
                      <el-date-picker disabled v-model="dataForm.deliveryDate" type="date" value-format="yyyy-MM-dd"
                        style="width: 100%;" :picker-options="dataPickerOptions2" placeholder="请选择交货日期">
                      </el-date-picker>
                    </el-form-item>
                  </el-col>
              ...
              <el-form :model="dataFormTwo" v-bind="dataFormTwo" ref="productForm">
                <el-table style="border: 1px solid #e3e7ee;" hasNO fixedNO v-bind="dataFormTwo.data"
                  ...
                  <!-- <el-table-column prop="planDemandQuantity" label="计划需求数量" min-width="200" show-overflow-tooltip>
                    <template slot-scope="scope">
                  ...
                  <el-table-column prop="purchaseQuantity" label="已下单数量" min-width="200">
                    <template slot-scope="scope">
                  ...
                  <el-table-column prop="purchaseQuantity2" label="数量(副)" min-width="140">
                    <template slot-scope="scope">
                      <el-form-item :prop="'data.' + scope.$index + '.' + 'purchaseQuantity2'">
                        <div class="viewData">
                          <span>{{ scope.row.purchaseQuantity2 }}</span>
                        </div>
                      </el-form-item>
                    </template>
                  </el-table-column>
                  ...
                  <!-- <el-table-column prop="excludingTaxPrice" label="不含税单价" min-width="200">
                    <template slot="header">
                  ...
                  <!-- 
                  <el-table-column prop="excludingTaxAmount" label="总金额(不含税)" min-width="200">
                    <template slot="header">
                      <span class="required">*</span>总金额(不含税)
                    </template>
                    <template slot-scope="scope">
                    ...
                    <template slot-scope="scope">
                      <el-form-item :prop="'data.' + scope.$index + '.' + 'deliveryDate'"
                        ...
                        <el-date-picker v-model="scope.row.deliveryDate" type="date" value-format="yyyy-MM-dd"
                          style="width: 100%;" disabled :picker-options="dataPickerOptions" placeholder="请选择交货日期">
                        </el-date-picker>
                      </el-form-item>
                    </template>
                  </el-table-column>
                  ...
                  <el-table-column prop="remark" label="备注" min-width="220">
                    <template slot-scope="scope">
                      <el-input v-model="scope.row.remark" disabled maxlength="20" :placeholder="type == 'look' ? '' : '请输入备注'">{{
                        scope.row.remark }}
                      </el-input>
                    </template>
                  </el-table-column>
              ...
              <div style="height: 40px; line-height: 40px; background: #f5f7fa;" class="text">
                <span style="font-weight:500;margin-right:10px">总金额：{{ dataForm.excludingTaxTotalAmount
                }}</span>
            ...
            <el-tab-pane label="进度跟踪" name="schedule" v-if="type === 'look'">
              ...
              <JNPF-table :partentOrChild="'child'" v-loading="formLoading" :data="scheduleData" custom-column ref="scheduleRef">
                ...
                <el-table-column prop="schedule" label="收货进度" min-width="170">
                  <template slot-scope="scope">
                    <el-progress
    ...
    <source-area v-if="sourceVisibled" ref="sourceRef"></source-area>
...
export default {
  ...
  data() {
    return {
      ...
      productRules: {
        ...
        purchaseQuantity: [{ required: true, trigger: ['blur'] }, { validator: this.formValidate({ type: 'decimal', params: [20, 4, "", (errMsg) => { this.$message.error('下单数量：' + errMsg) }] }), trigger: 'blur' }],
        deliveryDate: [{ required: true, message: '请选择交货日期', trigger: ['change'] }],
      },
      productArr: [],
      defaultProps: {
  ...
  methods: {
    ...
    handlerOpenSource(index, type) {
      ...
      this.index = index
      this.sourceData = this.dataFormTwo.data[this.index].outShipmentVOList
      this.$nextTick(() => {
        ...
        if (!this.dataForm.id) {
          ...
          purPurchaseOrderdetail(this.dataForm.id).then(res => {
            ...
            this.dataForm = {
              ...
              excludingTaxTotalAmount: res.data.excludingTaxTotalAmount,
      ...
      this.$refs['elForm'].validate((valid) => {
        if (valid) {
          if (!valid_2) {
            ...
            this.btnLoading = true
            insertPurchaseOrder(this.dataForm).then(res => {
