    ...
    <div class="JNPF-common-layout-center JNPF-flex-main">
      ...
      <div class="JNPF-common-layout-main JNPF-flex-main">
        ...
        <JNPF-table ref="dataTable" v-loading="listLoading" :data="tableData" :fixedNO="true" @sort-change="sortChange"
          ...
          <el-table-column prop="shipmentFreezeFlag" label="发货状态" width="150" fixed="left" sortable="custom">
            <template slot-scope="scope">
              <div v-if="scope.row.shipmentFreezeFlag">禁止</div>
              <div v-else>未禁止</div>
            </template>
          </el-table-column>
          ...
          <el-table-column label="操作" width="240" fixed="right">
            <template slot-scope="scope">
              <el-button size="mini" type="text" :disabled="!scope.row.shipmentFreezeFlag"
                @click="LiftingProhibition(scope.row.id)">解除禁止</el-button>
              <el-button size="mini" type="text" class="JNPF-table-delBtn" :disabled="scope.row.shipmentFreezeFlag"
                @click="prohibitShipment(scope.row.id)">禁止发货</el-button>
...
import { getcategoryTree, getCooperativeData, deleteCooperative, editShipmentfreeze } from '@/api/basicData/index'
...
export default {
  ...
  methods: {
    // 解除禁止
    LiftingProhibition(row) {
      console.log("row",);
    },
    // 禁止发货
    prohibitShipment(row) {
      console.log("row", row);
      let obj = {
        shipmentFreezeFlag: true,
        id: row
      }
      editShipmentfreeze(obj).then(res => {
        console.log("res", res);
        this.initData()
      })
    },
