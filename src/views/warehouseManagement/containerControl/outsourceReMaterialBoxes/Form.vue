      ...
      <div class="JNPF-preview-main org-form">
        <div :class="['JNPF-common-page-header', type === 'look' ? 'noButtons' : '']">
          <el-page-header @back="goBack"
            :content="type == 'add' ? '新建外协退料装箱单' : type == 'edit' ? '编辑外协退料装箱单' : '查看外协退料装箱单'" />
            ...
            <el-tab-pane label="基础信息" name="jcInfo">
              ...
              <el-row :gutter="30" class="">
                <el-form ref="elForm" :model="dataForm" :rules="rules" size="small" label-width="100px"
                  ...
                  <el-col :sm="6" :xs="24">
                    <el-form-item label="外协退料单号" prop="sourceOrderNo" ref="sourceOrderNo">
                      <ComSelect-page :beforeSubmit="beforeSubmit" :renderTree="false" :isdisabled="type === 'look'"
                        ...
                        :tableItems="purchaseTableItems" :placeholder="'请选择外协退料单号'" title="选择外协退料单号"
                        :listMethod="getQuotationdatasendlist" :listRequestObj="purPurchaseListRequestObj"
                  ...
                  <!-- <el-col :sm="6" :xs="24">
                    <el-form-item label="供应商编码" prop="partnerCode" ref="partnerCode">
                      <el-input type="text" v-model="dataForm.partnerCode" placeholder="供应商编码" maxlength="20" disabled>
                      </el-input>
                    </el-form-item>
                  </el-col> -->
                  ...
                  <el-col :sm="6" :xs="24" v-if="dataForm.packingType === 'mixed'">
                    <el-form-item label="标准箱" prop="standardBoxName">
                      <ComSelect-page :beforeSubmit="beforeSubmit1" :value="dataForm.standardBoxName" :renderTree="false"
              ...
              <el-form :model="dataFormTwo" v-bind="dataFormTwo" ref="productForm" class="tableContainer"
                ...
                <el-table style="border: 1px solid #e3e7ee;" hasNO fixedNO v-bind="dataFormTwo.data"
                  ...
                  <el-table-column prop="productsCode" label="产品编码" min-width="200" key="productsCode" show-overflow-tooltip>
                  ...
                  <el-table-column prop="productsName" label="产品名称" min-width="200" key="productsName" show-overflow-tooltip>
                    <template slot-scope="scope">
                  ...
                  <el-table-column prop="drawingNo" label="产品图号" min-width="200" key="drawingNo" show-overflow-tooltip>
                    <template slot-scope="scope">
                  ...
                  <el-table-column prop="routingName" label="工艺路线名称" min-width="180" key="routingName"
                  ...
                  <el-table-column prop="processName" label="工序名称" min-width="180" key="processName"
                    show-overflow-tooltip>
                    <template slot-scope="scope">
                  ...
                  <el-table-column prop="standardBoxName" label="标准箱" min-width="180" key="standardBoxName"
                    ...
                    <template slot-scope="scope">
                      <el-form-item   :prop="'data.' + scope.$index + '.' + 'standardBoxName'"
                        ...
                        <ComSelect-page :beforeSubmit="beforeSubmitStand" :value="scope.row.standardBoxName" :renderTree="false"
                  ...
                  <el-table-column v-if="dataForm.packingType === 'single'" prop="packing" label="标准箱数量" min-width="180" key="packing">
                    <template slot="header">
                      <span class="required">*</span>标准箱数量
                    </template>
                    <template slot-scope="scope">
                      <!-- <el-form-item :prop="'data.' + scope.$index + '.' + 'packing'" :rules='productRules.packing' v-if="!scope.row.defaultFlag">
                        <div class="viewData">
                  ...
                  <el-table-column prop="oddBoxName" label="零头箱" v-if="dataForm.packingType === 'single'" min-width="180" key="oddBoxName">
                    <template slot="header">
                    ...
                    <template slot-scope="scope">
                      ...
                      <el-form-item  :prop="'data.' + scope.$index + '.' + 'oddBoxName'"
                        ...
                        <ComSelect-page :beforeSubmit="beforeSubmitStand2" :value="scope.row.oddBoxName" :renderTree="false"
                          :isdisabled="type == 'look'" placeholder="请选择产品名称" ref="ComSelect-page" @change="addth"
                          :tableItems="ProductTableItems" title="选择产品" treeTitle="产品分类" :methodArr="ProductMethodArr"
                          :listMethod="getProductList" :listRequestObj="ProductListRequestObj"
                          :searchList="ProductTableSearchList" :elementShow="true"
                  ...
                  <el-table-column prop="num" label="箱数" v-if="dataForm.packingType === 'single'" min-width="140" key="num">
                    <template slot-scope="scope">
...
import { getQuotationdatasendlist, getQuotationsendlist } from '@/api/salesManagement'
...
export default {
  ...
  data() {
    return {
      ...
      // 外协退料弹窗渲染列表
      purchaseTableItems: [
        { prop: 'orderNo', label: '外协退料单号', fixed: 'left' },
        { prop: 'ordersNo', label: '外协工序订单号', fixed: 'left' },
      ...
      purPurchaseTableSearchList: [
        { prop: "orderNo", label: "外协退料单号", type: 'input' },
      ...
      rules: {
        sourceOrderNo: [{ required: true, message: '请选择收货单号', trigger: ['change'] }],
  ...
  methods: {
    ...
    // 有装箱信息后，改变装箱数量
    // changePacking(index, val) {
    //   if (this.oldsData.length) {
    //     if (Number(this.oldsData[index].packing) !== Number(val)) {
    //       this.$confirm('已有装箱信息，修改装箱数量将清空装箱信息，您是否继续!', '提示', {
    //         confirmButtonText: '确定',
    //         cancelButtonText: '取消',
    //         type: 'warning'
    //       }).then(() => {
    ...
    //           message: '已取消'
    //         });
    //         this.dataFormTwo.data[index].packing = Number(this.oldsData[index].packing)
    //       })
    //     }
    //   }
    // },
    ...
    async beforeSubmit(data, paramsObj) {
      ...
      if (paramsObj.oldData.length) {
        flag = await this.$confirm('已选择过外协退料单号，是否切换，切换后会清空供应商信息和产品信息是否继续？', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
    ...
    purchaseData(id, data) {
      ...
      if (data.length === 0) {
        ...
        getQuotationsendlist(this.dataForm.documentId).then(res => {
          console.log(res, '明细');
          let _data = []
          res.data.noticeLineList.forEach(item => {
            _data.push({
              waitPackingQuantity: item.deliveryQuantity,
              totalNum: item.deliveryQuantity,
              ...
              packedQuantity: '',         // 待装箱
              // totalNum: this.jnpf.math('subtract', [item.deliveryQuantity, item.scrapQuantity]), // 装箱总数 = 退发货数量 - 报废数量
              // totalNum: item.deliveryQuantity, // 外协退料入库，没有报废数量
