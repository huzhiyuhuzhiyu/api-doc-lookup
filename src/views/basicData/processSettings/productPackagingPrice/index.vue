    ...
    <div class="JNPF-common-layout-center JNPF-flex-main">
      <el-row class="JNPF-common-search-box" :gutter="16">
        <el-form @submit.native.prevent>
          ...
          <el-col :span="4">
            <el-form-item>
              <el-select v-model="listQuery.packagingMethod" placeholder="包装方式" clearable style="width: 100%;">
                <el-option v-for="(item, index) in bimProductAttributesObj.pa015" :key="index" :label="item.name"
...
<script>
import { getBimProductPackagingPrice, batchAddBimProductPackagingPrice, uploadBimProductPackagingPrice } from '@/api/bimProcess/index'
...
export default {
  ...
  methods: {
    ...
    downLoadTemplate() {
      ...
      a.setAttribute('href', location.origin + '/static/产品包装单价导入模板.xlsx')
    ...
    UploadProduct(data) {
      ...
      //调用上传文件接口
      uploadBimProductPackagingPrice(formData)
        .then((res) => {
          if (!res.data) {
    ...
    handleMessage(data) {
      ...
      this.$message({
        ...
        message: h(
          ...
          [
            h('p', { style: 'font-size:14px;' }, '导入成功，存在产品包装价格导入相关错误！'),
            h(
              'el-button',
              {
                props: {
    ...
    sortChange({ prop, order }) {
      let newProp
      if (['productsName', 'productsCode'].includes(prop)) {
        newProp = prop
      } else {
    ...
    initData() {
      ...
      // }
      getBimProductPackagingPrice(this.listQuery)
        .then((res) => {
    ...
    async dataFormSubmit() {
      ...
      if (submitFlag) {
        ...
        batchAddBimProductPackagingPrice(this.selectedData)
          .then((res) => {
