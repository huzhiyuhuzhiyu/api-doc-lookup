<template>
  <el-dialog :title="!dataForm.id ? '复制字典' : '复制字典'" :close-on-click-modal="false" :close-on-press-escape="false"
    :visible.sync="visible" lock-scroll width="600px" class="JNPF-dialog JNPF-dialog_center">
        ...
        getDictionaryDataTypeSelector(this.dataForm.dictionaryTypeId, isTree, id || 0).then(res => {
          ...
          if (this.dataForm.id) {
            getDictionaryDataInfo(this.dataForm.id).then(res => {
              ...
              this.dataForm.id = ''
              this.dataForm.sortCode = list.length
