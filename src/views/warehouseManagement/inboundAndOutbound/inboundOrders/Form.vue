    ...
    refeshDataFormItems() {
      ...
      this.dataFormItems = [
        ...
        },
        {
          ...
          ], itemDisabled: this.sourceType === "inbound_transfer" || this.openMode === '只读', dialogTitle: '选择仓库', sm,
      ...
      if (id) {
        ...
        if (inboundId) {
          ...
          const tempFunction = (rowData) => {
            ...
            if (sourceType === 'inbound_order_production' && false) {
              // 入库数量 = 合格数量 - 已入库数量
              deliveryQuantity = this.jnpf.math('subtract', [rowData.qualifiedQuantity, rowData.receivedQuantity])
            } else {
              // 入库数量 = 生产完工数量 - 已入库数量
          ...
          this.$nextTick(() => {
            ...
            let tempLinesList = data.map(rowData => {
              let deliveryQuantity
              // deliveryQuantity = this.jnpf.math('subtract', [rowData.qualifiedQuantity, rowData.receivedQuantity])
              if (sourceType === 'inbound_order_production_batch' && false) {
                // 入库数量 = 合格数量 - 已入库数量
                deliveryQuantity = this.jnpf.math('subtract', [rowData.qualifiedQuantity, rowData.receivedQuantity])
              } else {
                // 入库数量 = 生产完工数量 - 已入库数量
