                    ...
                    <el-tab-pane  label="基本信息" name="basicInfo">
                        <el-collapse v-model="activeNames">
                            ...
                            <el-collapse-item title="产品信息" name="basicInfo" class="orderInfo">
                                <JNPF-col v-model="dataForm" :tabContent="productDataFormItem" ref="dataForm" :openMode="openMode" />
                            </el-collapse-item>
                            <el-collapse-item title="其他信息" name="otherInfo">
                                <JNPF-col v-model="dataForm" :tabContent="otherItems" ref="dataForm2" :openMode="openMode" />
                            </el-collapse-item>
                        </el-collapse>
                    </el-tab-pane>
...
export default {
    ...
    methods: {
        async setDataFormItems(){
            console.log(this.$store.getters.configData.product.enable_productName,'this.$store.getters.configData.product.enable_productName')
            let projectList = []
            if (this.$store.getters.configData.system.project){
                const res = await getProjectList({pageNum: 1, pageSize: -1})
                if (res.data.records){
                    projectList = res.data.records.map((item) => {
            ...
            this.attrDictionaryData = productAttribute.filter(item=>!['pa008','pa015','pa016','pa017','pa018','pa019','pa020','pa021','pa022','pa023','pa024','pa025','pa026','pa027'].includes(item.enCode)).map(item=>{
                return {
                    ...item,
                    render: this.$store.getters.configData.product.enable_productAttribute,
                    prop:item.description,
                    label:this.$store.getters[item.description] || item.fullName,
                    type:'select',
                    options:this.bimProductAttributesList[item.enCode].map(item=>{
                        return {
                            ...item,
                            label:item.name,
                            value:item.name
                    ...
                    itemRules:[{ required: this.flag,trigger: 'change' } ],
                    itemDisabled:this.btnType,
                    value:'',
                    change: (val) => {
                        if (this.flag) {
                            if (this.dataForm.steelBallManufacturer) {
            ...
            const basicItem = [
                { prop: "projectId", label: "所属项目", value: '' , type: 'select', itemRules: [{ required: true, trigger: "blur" }],options: projectList, filterable: true, render: this.$store.getters.configData.system.project,
                    itemDisabled: this.dataForm.id && !this.btnType ? true : this.btnType
                },
                { prop: "code", label: "产品编码", value: "",maxlength: 100, type: 'input',
                    itemRules: [
                ...
                { prop: "name", label: "产品名称", value: "", type: 'input', maxlength: 100,render: this.$store.getters.configData.product.enable_productName, itemDisabled: this.btnType,},
                { prop: "productCategoryName", label: "产品分类", value: "", type: 'custom', customComponent: "ComSelect-list", itemRules: [{ required: true, message: "请选择产品分类", trigger: "no" }],
                    itemDisabled: this.btnType,
                    method:getcategoryTree, requestObj:{ classAttribute: 'finish_product'} ,
                    change: (val, data) => {
                        // dom更新后重新校验此元素
                        this.$nextTick(() => {
                            this.$refs['dataForm'][0].$children[0].validateField('productCategoryName')
                ...
                { prop: "mainUnit", label: "主单位", value: "", type: 'select', itemRules: [{ required: true, trigger: "blur" }], options: this.productUnit, filterable: true,itemDisabled: this.btnType, },
                { prop: "deputyUnit", label: "副单位", value: "", type: 'select', itemRules: [{ required: true, trigger: "blur" }], options: this.productUnit, filterable: true ,itemDisabled: this.btnType,},
                { prop: "ratio", label: "转换系数", value: "", type: 'input', itemRules: [{ required: true, trigger: "blur" }, { validator: formValidate({ type: 'decimal', params: [12, 6] }), trigger: 'blur' }, { validator: formValidate('positiveNumber'), trigger: "blur" }] ,itemDisabled: this.btnType,},
                { prop: "calculationDirection", label: "计算方向", type: "select", options: [{ label: "乘", value: "multiplication" }, { label: "除", value: "division" }], itemRules: [{ required: true, trigger: "change" }] ,itemDisabled: this.btnType,},
                { prop: "productSource", label: "产品来源", type: "select", options: [{ label: "组装", value: "assemble" }, { label: "生产", value: "produce" }, { label: "采购", value: "purchase" }, { label: "外协", value: "out" }], itemRules: [{ required: true, trigger: "change" }], itemDisabled: this.dataForm.id && !this.btnType ? true : this.btnType,
                    change : (val) => { this.dataForm.bomFlag = val === 'assemble' }
                },
                { prop: "productStatus", label: "产品状态", value: "enable", type: "select", options: [{ label: "启用", value: "enable" }, { label: "禁用", value: "disabled" }], itemRules: [{ required: true, trigger: "change" }] ,itemDisabled: this.btnType,},
                { prop: "steelBallManufacturerName", label: "钢球厂家", value: "", type: "custom", customComponent: 'ComSelect-page', itemRules: [{ required: false, trigger: "change" }], itemDisabled: this.btnType,
                    dialogTitle: '选择钢球厂家',treeTitle: '钢球厂家分类',
                    methodArr : this.PartnerMethodArr,
                    listMethod : getCooperativeData,
                    listRequestObj : this.PartnerListRequestObj,
                    tableItems : this.PartnerTableItems,
                    searchList : this.PartnerTableSearchList,
                    clearable : true,
                    change : this.steelBallChange,
                    paramsObj : { prop: 'steelBallManufacturerName' },
                ...
                //     itemDisabled: this.btnType,
                //     dialogTitle : '选择型号' ,
                //     method : getbimProductsModelList,
                //     requestObj: this.modelQuery,
                //     render:this.$store.getters.configData.product.enable_model,
                //     dataFormatting: (res) => {
                //         let data = res.data.records.map((item) => {
                //             return {
                //                 ...item,
            ...
            const remark = [{ prop: "remark", label: "备注", value: "", type: "textarea", sm: 12 ,itemDisabled: this.btnType },]
            this.productDataFormItem = [...basicItem,...this.attrDictionaryData,...remark]
            this.$nextTick(() => {
        ...
        async init(id, btnType = false, flag,row,isProjectSwitch) {
            ...
            if (!!id) {
                ...
                await this.fetchData('CPBM', true)
                await this.setDataFormItems()
                this.formLoading = false
            }
        },
        async handleConfirm() {
