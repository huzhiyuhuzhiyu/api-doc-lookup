...
export default {
  components: { TableFormProduct, workFlow, okOrNo, transferForm },
  data() {
    return {
      okOrNoVisible: false,
  ...
  methods: {
    async init(row, btnType, isShow, nodeData, productId) {
      this.visible = true
      ...
      if (this.dataForm.id && this.btnType != 'add') {
        ...
        if (productId) {
          ...
          detailBomData(bomId).then(res => {
            ...
            getBomTree(this.dataForm.id).then(res => {
          ...
          detailBomData(this.dataForm.id).then(res => {
            this.autoCode = res.data.bom.code
            this.dataForm = JSON.parse(JSON.stringify(res.data.bom))
            this.documentStatus = res.data.bom.documentStatus
            this.linesList = res.data.lines.map(line => {
      ...
      } else if (this.dataForm.id && this.btnType == 'add') {
        this.firstId = this.firstId ? this.firstId : productId
        this.title = '新建BOM'
    ...
    handleNodeClick(nodeData, node) {
      ...
      if (nodeData.productId === this.selectedNodeKey) {
        ...
        this.init(this.row, this.btnType, this.isShow, nodeData, nodeData.productId)
      } else {
