  ...
  <transition name="el-zoom-in-center">
    <template v-if="!isFinish">
      <div class="JNPF-preview-main org-form" ref="main" v-loading="pageLoading">
        <template v-if="!approvalFlag">
          <div :class="['JNPF-common-page-header', isView ? 'noButtons' : '']">
            ...
            <div class="options" v-if="!isView">
              <el-button type="success" :loading="btnLoading"
                @click="handleConfirm(DocumentStatus.DRAFT)">保存草稿</el-button>
              <el-button type="primary" :loading="btnLoading"
                @click="handleConfirm(DocumentStatus.SUBMIT)">保存并提交</el-button>
              <el-button @click="goBack('cancel')">{{ $t('common.cancelButton') }}</el-button>
            </div>
          </div>
          <el-tabs v-model="activeName">
            <el-tab-pane label="基础信息" name="info">
              <component :is="basicInfoComName" :is-add="isAdd" :is-edit="isEdit" :is-view="isView" :type="type"
                ref="basicInfo" />
            </el-tab-pane>
        ...
        <component v-if="approvalFlag" :is="basicInfoComName" :is-add="isAdd" :is-edit="isEdit" :is-view="isView"
          :type="type" ref="dataForm" />
      </div>
    </template>
    <FinishSubmit v-else :left-btn-text="'再建一个'" :right-btn-text="'返回列表'" @left-btn-click="recreate"
      @right-btn-click="goBack"></FinishSubmit>
  </transition>
</template>
