...
<template>
    <div class="hub-upload-slice-wrap">
        <el-upload ref="hub-upload-slice" :disabled="(upLoading && !multiple) || disabled" action="" :accept="accept"
            :limit="limit" :size="size" :multiple="multiple" :list-type="listType" :auto-upload="false"
            :file-list="inFileList" :on-change="handleChange" :before-upload="handleBeforeUpload" :on-error="handleError"
            :on-exceed="handleExceed" :on-remove="handleRemove" v-bind="$attrs" v-on="$listeners">
            <!-- 插槽: 触发文件框 -->
            ...
            <template v-else>
                ...
                <template v-else>
                    ...
                    <el-button v-if="!hideBtn || preLoading || (hideBtn && !upLoading) || multiple" slot="trigger"
                        type="primary" icon="el-icon-upload2" :loading="preLoading" plain style="margin-bottom: 10px;"
                        @click="resetUpload">
        ...
        <template v-if="listType === 'picture-card'">
            <div v-for="(item, index) in inFileList" :key="index" class="picture-card-file-info-wrap">
                <div class="progress-wrap">
                    <el-progress v-if="item.totalProgress && item.totalProgress < 100" type="circle" :stroke-width="2"
                        :width="50" :percentage="item.totalProgress" />
                    <el-progress v-if="item.totalProgress === 100" type="circle" :percentage="100" :stroke-width="2"
                        :width="50" status="success" />
                    <div v-if="item.totalProgress === 100" class="delete" @click="deleteFile(item)">
...
import { calculateHashIdle } from './component/md5'
export default {
    name: 'HubUploadSlice',
    props: {
        action: {
