      ...
      <el-form ref="dataForm" v-loading="formLoading" :model="dataForm" :type="dataForm.type" :rules="dataRule"
        ...
        <el-form-item label="上级分类" prop="parentName">
          <ComSelect-list :isdisabled="dataForm.id ? true : false" v-model="dataForm.parentName" placeholder="请选择上级分类"
            auth @change="onOrganizeChange" :title="'选择上级分类'" :method="crmProductCategorytree" :requestObj="requestObjTwo"
            :paramsObj="{}" />
        </el-form-item>
...
<script>
import {
  detailcrmProductCategory,
  updatecrmProductCategory,
  addcrmProductCategory,
  checkCategoryCode,
  crmProductCategorytree
} from '@/api/CRMmanagement/index'
...
export default {
  ...
  methods: {
    ...
    init(id, parentId, btntype) {
      ...
      this.$nextTick(() => {
        if (btntype == 'edit') {
          detailcrmProductCategory(this.dataForm.id).then((res) => {
    ...
    async dataFormSubmit() {
      ...
      if (valid) {
        ...
        let formMethod = this.dataForm.id ? updatecrmProductCategory : addcrmProductCategory
