...
export default {
  ...
  data() {
    return {
      ...
      fullScreenWidthOptions: ['800px', '1000px', '1200px', '1400px', '50%', '60%', '70%', '90%', '90%', '100%'],
      layoutTreeProps: {
        label(data, node) {
  ...
  methods: {
    ...
    async updateFieldOptions(data) {
      ...
      let queryType = 0, type = 4//this.getFormInfo().type
      if (type == 3 || type == 4 || type == 5) queryType = 1
      let res = 10//await DataModelFieldList(this.getFormInfo().dbLinkId, tableName, queryType)
