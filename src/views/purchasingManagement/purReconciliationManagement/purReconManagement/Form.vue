                      ...
                      <el-table-column prop="receiptReturnType" label="收/退货类型" min-width="200" show-overflow-tooltip>
                        <template slot-scope="scope">
                          <el-form-item :prop="'data.' + scope.$index + '.' + 'receiptReturnType'">
                            <div class="viewData">
                              <span>{{ scope.row.receiptReturnType === 'outbound_purchase' ? '退货' :
                                scope.row.receiptReturnType ===
                                  'inbound_purchase' ? '收货' : '' }}</span>
                            </div>
                          </el-form-item>
                        </template>
                      </el-table-column>
                      ...
                      <el-table-column prop="excludingTaxAmount" label="不含税总金额" min-width="140" fixed="right">
                        <template slot-scope="scope">
                          <el-form-item :prop="'data.' + scope.$index + '.' + 'excludingTaxAmount'"
                            ...
                            <div
                              :class="['viewData', scope.row.receiptReturnType === 'outbound_purchase' ? 'green' : 'red']">
                      ...
                      <el-table-column prop="taxAmount" label="税额" min-width="140" fixed="right">
                        ...
                        <template slot-scope="scope">
                          <el-form-item :prop="'data.' + scope.$index + '.' + 'taxAmount'"
                            ...
                            <div
                              :class="['viewData', scope.row.receiptReturnType === 'outbound_purchase' ? 'green' : 'red']">
                              <span v-if="scope.row.receiptReturnType === 'outbound_purchase'">+{{ scope.row.taxAmount
                                }}</span>
                              <span v-else-if="scope.row.receiptReturnType === 'inbound_purchase'">{{
                                scope.row.taxAmount
                              }}</span>
...
export default {
  ...
  methods: {
    ...
    init(data) {
      ...
      _data.forEach(item => {
        let excludingTaxAmount = item.businessType === 'outbound_purchase' ? (this.jnpf.numberFormat(item.num * (this.jnpf.numberFormat(item.costPrice / (1 + (item.taxRate * 1 / 100)), 2)), 2)) : -this.jnpf.numberFormat(item.num * this.jnpf.numberFormat(item.costPrice / (1 + (item.taxRate * 1 / 100))))
        let includingTaxAmount = item.businessType === 'inbound_purchase' ? (this.jnpf.numberFormat(Math.abs(excludingTaxAmount) * (1 + ((item.taxRate * 1) / 100)))) : this.jnpf.numberFormat(excludingTaxAmount * (1 + ((item.taxRate * 1) / 100)))
        this.dataFormTwo.data.push({
          accountsReceivableId: '',
          ...
          taxAmount: item.businessType === 'outbound_purchase' ? this.jnpf.numberFormat(includingTaxAmount - excludingTaxAmount) : this.jnpf.numberFormat(includingTaxAmount - excludingTaxAmount),
