          ...
          <el-tabs v-model="activeName">
            ...
            <el-tab-pane :label="showLabel + '款抵扣'" name="pay" v-if="type === 'look' && showTab">
              <div style="display: flex;flex-direction: column;height: 100%;">
                <JNPF-table fixedNO :partentOrChild="'child'" v-loading="formLoading" :data="payData" custom-column ref="payRef" show-summary
...
export default {
  ...
  computed: {
    ...
    showTitle() {
      let title = this.reconciliationType === 'receivable' ? '销售预收款' : this.reconciliationType === 'payable' ? '采购预付款' : '外协预付款'
      return title
    },
  },
  data() {
    return {
      ...
      dataRules: {
        ...
        paymentAmount: [
          ...
          { validator: this.formValidate('noZero', `${this.reconciliationType != 'receivable' ? '付' : '收'}款金额不能为0`,), trigger: 'blur' }
        ],
      },
      payData: [],
      payForm: {
