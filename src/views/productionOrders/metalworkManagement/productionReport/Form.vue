  ...
  <transition name="el-zoom-in-center">
    <div class="JNPF-preview-main org-form">
      <div :class="['JNPF-common-page-header', btnType !== 'fp' ? 'noButtons' : '']">
        <el-page-header @back="goBack" :content="'分配订单'" />
        <div class="options">
      ...
      <div class="main">
        <el-tabs v-model="activeName" @tab-click="handleClick">
          <el-tab-pane label="订单分配" name="orderAllocation" v-loading="formLoading">
            ...
            <el-table ref="product" :data="productData" :fixedNO="true" :span-method="objectSpanMethod"
              ...
              <el-table-column prop="customerProductDrawingNo" label="客户产品图号" width="150" show-overflow-tooltip />
              ...
              <el-table-column prop="drawingNo" label="产品图号" width="120" show-overflow-tooltip />
              <el-table-column prop="num" label="数量(主)" width="200" show-overflow-tooltip />
              <el-table-column prop="mainUnit" label="单位(主)" width="80" show-overflow-tooltip />
              <el-table-column prop="assistantNum" label="数量(副)" width="200" show-overflow-tooltip />
              <el-table-column prop="deputyUnit" label="单位(副)" width="80" show-overflow-tooltip />
              <el-table-column prop="price" label="单价(含税)" width="200" show-overflow-tooltip />
              <el-table-column prop="totalAmount" label="金额" width="200" show-overflow-tooltip />
              <el-table-column prop="availableQuantity" label="可用库存数量" width="200" show-overflow-tooltip />
              <el-table-column prop="utilizationQuantity" label="利用库存数量" width="200">
                ...
                <template slot-scope="scope">
                  <el-input v-model="scope.row.utilizationQuantity" placeholder="请输入利用库存数量"
                    @input="watchKC(scope.row, scope.$index)" @blur="handleKC(scope)">{{
                      scope.row.recipient }}
                  </el-input>
                </template>
              </el-table-column>
              <el-table-column prop="productionQuantity" label="需生产数量" width="200">
                ...
                <template slot-scope="scope">
                  <el-input v-model="scope.row.productionQuantity" placeholder="请输入需生产数量"
                    ...
                    :disabled="scope.row.productSource == 'produce'">{{
                      scope.row.recipient }}
                  </el-input>
                </template>
              </el-table-column>
              <el-table-column prop="purchaseQuantity" label="需采购数量" width="200">
                ...
                <template slot-scope="scope">
                  <el-input v-model="scope.row.purchaseQuantity" placeholder="请输入需采购数量"
                    ...
                    :disabled="scope.row.productSource == 'purchase'">{{
                      scope.row.recipient }}
                  </el-input>
                </template>
              </el-table-column>
              <el-table-column prop="outsourcingQuantity" label="需外协数量" width="200">
                ...
                <template slot-scope="scope">
                  <el-input v-model="scope.row.outsourcingQuantity" placeholder="请输入需外协数量"
                    ...
                    :disabled="scope.row.productSource == 'out'">{{
                      scope.row.recipient }}
                  </el-input>
                </template>
              </el-table-column>
              <el-table-column prop="deliveryDate" label="交货日期" width="200" show-overflow-tooltip />
              ...
              <el-row :gutter="30" class="custom-row">
                <el-col :sm="6" :xs="24">
                  <el-form-item label="订单类型" prop="orderType">
                    <el-select v-model="dataForm.orderType" placeholder="请选择订单类型" clearable style="width: 100%;"
                      :disabled="btnType == 'fp' ? true : false">
                ...
                <el-col :sm="6" :xs="24">
                  <el-form-item label="工作令号" prop="workOrderNo">
                    <el-input v-model="dataForm.workOrderNo" placeholder="请输入工作令号"
                      :disabled="btnType == 'fp' ? true : false" maxlength="20" />
                  </el-form-item>
                </el-col>
                <el-col :sm="6" :xs="24">
                  <el-form-item label="来源单号" prop="sourceOrderNo">
                    <el-input v-model="dataForm.sourceOrderNo" :disabled="btnType == 'fp' ? true : false"
                      maxlength="50" />
                  </el-form-item>
                </el-col>
                <el-col :sm="6" :xs="24">
                ...
                <el-col :sm="6" :xs="24">
                  <el-form-item label="交货日期" prop="deliveryDate">
                    <el-date-picker v-model="dataForm.deliveryDate" type="date" value-format="yyyy-MM-dd"
                      style="width: 100%;" placeholder="请选择交货日期" :disabled="btnType == 'fp' ? true : false">
                    </el-date-picker>
                  </el-form-item>
                </el-col>
                <el-col :sm="6" :xs="24">
                ...
                <el-col :sm="6" :xs="24">
                  <el-form-item label="付款周期" prop="paymentCycle">
                    <el-select v-model="dataForm.paymentCycle" placeholder="请选择付款周期" clearable style="width: 100%;"
                      :disabled="btnType == 'fp' ? true : false">
                      <el-option v-for="(item, index) in paymentCycleList" :key="index" :label="item.fullName"
                        :value="item.enCode"></el-option>
                    </el-select>
                  </el-form-item>
                </el-col>
                <el-col :sm="6" :xs="24">
                  <el-form-item label="对应车型" prop="vehicleTypeCorrespondingName">
                    ...
                    <el-select v-model="dataForm.vehicleTypeCorrespondingName" placeholder="" clearable filterable
                      style="width: 100%;" :disabled="btnType == 'fp' ? true : false">
                      <el-option v-for="(item, index) in bimVehicle" :key="index" :label="item.nname"
                        :value="item.id"></el-option>
                    </el-select>
                  </el-form-item>
                </el-col>
                <el-col :sm="6" :xs="24">
                  <el-form-item label="收件人" prop="recipient">
                    <el-input v-model="dataForm.recipient" readonly :disabled="btnType == 'fp' ? true : false"
                      maxlength="20" />
                  </el-form-item>
                </el-col>
                <el-col :sm="6" :xs="24">
                  <el-form-item label="收件人电话" prop="phone">
                    <el-input v-model="dataForm.phone" readonly :disabled="btnType == 'fp' ? true : false"
                      maxlength="20" />
                  </el-form-item>
                </el-col>
                <el-col :sm="12" :xs="24">
                  <el-form-item label="地址" prop="address">
                    <el-input v-model="defaultAddress" readonly :disabled="btnType == 'fp' ? true : false"
                      maxlength="300" />
                  </el-form-item>
                </el-col>
                <el-col :sm="12" :xs="24">
                  <el-form-item label="备注" prop="remark">
                    <el-input v-model="dataForm.remark" :disabled="btnType == 'fp' ? true : false" type="textarea"
  ...
  methods: {
    ...
    getSummaries(param) {
      ...
      if (this.btnType == 'edit' || this.btnType == 'add' || this.btnType == 'copy') {
        ...
        this.totalAmount = sums[11]
      } else if (this.btnType == 'fp') {
        this.totalNum = sums[5]
        this.totalAssistantNum = sums[7]
        this.totalAmount = sums[10]
      }
      return sums;
    },
    ...
    init(id, btnType) {
      ...
      if (btnType == 'fp') { this.isdisabled = true }
      if (this.dataForm.id) {
        getOrderDetail(this.dataForm.id).then(res => {
          this.$nextTick(() => {
            ...
            if (res.data.orderLines.length > 0) {
              res.data.orderLines.forEach(item => {
                ...
                if (Number(item.num) > Number(item.availableQuantity)) {
                  item.utilizationQuantity = item.availableQuantity
            ...
            this.productArr = Array.from(res.data.orderLines);
