              ...
              >
                <template v-if="column.slot" v-slot="scope">
                  <template v-if="column.prop === 'orderNo'">
                    <el-link
                      type="primary"
                      @click.native="handleAction(scope.row.id, 'look')"
                    >
                      {{ scope.row.orderNo }}
                    </el-link>
                  </template>
            ...
            <el-table-column label="操作" width="180" fixed="right">
              <template v-slot="scope">
                <el-button
                  ...
                  @click="handleAction(scope.row.id, 'edit')"
                >
                  编辑
                </el-button>
                <el-button
                ...
                <el-dropdown hide-on-click>
                  ...
                  <el-dropdown-menu>
                    <el-dropdown-item
                      ...
                      @click.native="handleAction(scope.row.id, 'add')"
                    >
                      重新提交
                    </el-dropdown-item>
                    <el-dropdown-item
                      v-if="scope.row.approvalStatus === 'ing' && showAppCodeFlag"
                      @click.native="withdrawnHandle(scope.row.id)"
                    >
                      审批撤回
                    </el-dropdown-item>
                    <el-dropdown-item @click.native="handleAction(scope.row.id, 'look')">
                      查看详情
                    </el-dropdown-item>
                  </el-dropdown-menu>
                </el-dropdown>
...
import { deletePurchaseChange, getPurchaseChangeList } from '@/api/salesManagement/orderChanges'
...
export default {
  ...
  data() {
    return {
      ...
      // 搜索表单配置
      searchConfig: [
        { prop: 'orderNo', label: '变更单号', span: 6 },
        { prop: 'partnerName', label: '供应商名称', span: 6 }
      ],

      // 表格列配置
      columnsConfig: [
        { prop: 'orderNo', label: '变更单号', minWidth: 200, sortable: 'custom', slot: true },
        { prop: 'purchaseOrderNo', label: '采购订单号', minWidth: 200, sortable: 'custom' },
        { prop: 'cooperativePartnerName', label: '供应商名称', minWidth: 150 },
        { prop: 'newCooperativePartnerName', label: '新供应商名称', minWidth: 150 },
  ...
  async created() {
    ...
    this.showAppCodeFlag = res?.enabledMark || false
  ...
  methods: {
    ...
    async initData() {
      this.loading = true
      try {
        const res = await getPurchaseChangeList(this.queryParams)
