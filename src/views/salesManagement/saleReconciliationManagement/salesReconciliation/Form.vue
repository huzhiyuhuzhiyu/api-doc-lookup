            ...
            <el-tab-pane label="基础信息" name="jcInfo">
              ...
              <el-form :model="dataFormTwo" v-bind="dataFormTwo" ref="productForm">
                <el-table style="border: 1px solid #e3e7ee;" hasNO fixedNO v-bind="dataFormTwo.data"
                  ...
                  <el-table-column prop="receiptReturnType" label="发/退货类型" min-width="200" show-overflow-tooltip>
                    <template slot-scope="scope">
                      <el-form-item :prop="'data.' + scope.$index + '.' + 'receiptReturnType'">
                        <div class="viewData">
                          <span>{{ scope.row.receiptReturnType === 'delivery' ? '发货' : scope.row.receiptReturnType ===
            'back' ? '退货' : '' }}</span>
                        </div>
                      </el-form-item>
                    </template>
                  ...
                  <el-table-column prop="excludingTaxAmount" label="不含税总金额" min-width="140" fixed="right">
                    <template slot-scope="scope">
                      <el-form-item :prop="'data.' + scope.$index + '.' + 'excludingTaxAmount'"
                        ...
                        <div :class="['viewData', scope.row.receiptReturnType === 'delivery' ? 'green' : 'red']">


                          <span v-if="scope.row.receiptReturnType === 'delivery'">+{{ scope.row.excludingTaxAmount
                            }}</span>
                  ...
                  <el-table-column prop="taxAmount" label="税额" min-width="140" fixed="right">
                    ...
                    <template slot-scope="scope">
                      <el-form-item :prop="'data.' + scope.$index + '.' + 'taxAmount'"
                        ...
                        <div :class="['viewData', scope.row.receiptReturnType === 'delivery' ? 'green' : 'red']">
                          <span v-if="scope.row.receiptReturnType === 'delivery'">+{{ scope.row.taxAmount }}</span>
                  ...
                  <el-table-column prop="includingTaxAmount" label="含税总金额" min-width="140" fixed="right">
                    <template slot-scope="scope">
                      <el-form-item :prop="'data.' + scope.$index + '.' + 'includingTaxAmount'"
                        ...
                        <div :class="['viewData', scope.row.receiptReturnType === 'delivery' ? 'green' : 'red']">
                          <span v-if="scope.row.receiptReturnType === 'delivery'">+{{ scope.row.includingTaxAmount
                            }}</span>
...
export default {
  ...
  methods: {
    ...
    init(data) {
      ...
      _data.forEach(item => {
        let excludingTaxAmount = item.returnDeliveryType === 'delivery' ? (this.jnpf.numberFormat(item.actualQuantity * (this.jnpf.numberFormat(item.price / (1 + (item.taxRate * 1 / 100)), 2)), 2)) : -this.jnpf.numberFormat(item.actualQuantity * this.jnpf.numberFormat(item.price / (1 + (item.taxRate * 1 / 100))))
        let includingTaxAmount = item.returnDeliveryType === 'delivery' ? (this.jnpf.numberFormat(Math.abs(excludingTaxAmount) * (1 + ((item.taxRate * 1) / 100)))) : this.jnpf.numberFormat(excludingTaxAmount * (1 + ((item.taxRate * 1) / 100)))
        this.dataFormTwo.data.push({
          accountsReceivableId: '',
          ...
          receiptReturnType: item.returnDeliveryType,
          ...
          taxAmount: item.returnDeliveryType === 'delivery' ? this.jnpf.numberFormat(includingTaxAmount - excludingTaxAmount) : this.jnpf.numberFormat(includingTaxAmount - excludingTaxAmount),
