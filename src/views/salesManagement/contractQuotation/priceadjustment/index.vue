        ...
        <el-tab-pane label="单据列表" name="orderList">
          <div class="JNPF-common-layout-center JNPF-flex-main">
            ...
            <div class="JNPF-common-layout-main JNPF-flex-main">
              ...
              <JNPF-table ref="dataTable" v-loading="listLoading" :data="tableData" @sort-change="sortChange" fixedNO custom-column customKey="JNPFTableKey_345176">
                ...
                <el-table-column prop="partnerCode" label="客户编码" width="200" sortable="custom"></el-table-column>

                <el-table-column prop="partnerName" label="客户名称" width="200" sortable="custom" />
                <el-table-column prop="reason" label="调价理由" width="300"> </el-table-column>
                ...
                <el-table-column label="操作" width="180" fixed="right">
                  ...
                  <template slot-scope="scope">
                    <el-button type="text" @click="addOrUpdateHandle(scope.row.id, 'edit')" :disabled="scope.row.documentStatus == 'draft' ? false : true" size="mini">编辑</el-button>
                    <el-button type="text" :disabled="scope.row.documentStatus == 'draft' ? false : true" @click="handleDel(scope.row.id,)" class="JNPF-table-delBtn" size="mini">删除</el-button>
            ...
            <div class="JNPF-common-layout-main JNPF-flex-main">
              ...
              <JNPF-table ref="linesTableData" v-loading="listLoading" :data="linesTableData" :fixedNO="true" custom-column @sort-change="sortChange" :partentOrChild="'child'">
                <el-table-column prop="orderNo" label="单号" width="200" sortable="custom">
                  <template slot-scope="scope">
                    <el-link type="primary" @click.native="handleUserRelation(scope.row.adjustPriceId, 'look')">
                      {{ scope.row.orderNo }}
                    </el-link>
                  </template>
                </el-table-column>
                ...
                <el-table-column prop="partnerName" label="客户名称" width="200" sortable="custom" />
                <el-table-column prop="customerProductNo" label="客户物料号" width="200"  />
                ...
                <el-table-column prop="originPrice" label="原单价" width="140" />
                <el-table-column prop="newPrice" label="新单价" width="140" />
                <el-table-column prop="createTime" label="创建时间" width="180" sortable="custom" />
                <el-table-column prop="createByName" label="创建人" width="120" />
                <el-table-column label="操作" min-width="180" fixed="right">
                  <template slot-scope="scope">
                    <el-button type="text" @click="addOrUpdateHandle(scope.row.adjustPriceId, 'edit')" :disabled="scope.row.documentStatus == 'draft' ? false : true" size="mini">编辑</el-button>
                    <el-button type="text" :disabled="scope.row.documentStatus == 'draft' ? false : true" @click="handleDel(scope.row.adjustPriceId,)" class="JNPF-table-delBtn" size="mini">删除</el-button>
                    <el-dropdown hide-on-click>
                      <span class="el-dropdown-link">
                        <el-button type="text" size="mini">
                          {{ $t('common.moreBtn') }}<i class="el-icon-arrow-down el-icon--right"></i>
                      ...
                      <el-dropdown-menu slot="dropdown">
                        <el-dropdown-item @click.native="handleUserRelation(scope.row.adjustPriceId, 'look')">
                          查看详情
                        </el-dropdown-item>
                      </el-dropdown-menu>
                    </el-dropdown>
        ...
        <el-form ref="diaForm" :model="orderForm" label-width="120px" label-position="top">
          ...
          <el-col :span="12">
            <el-form-item label="创建时间">
              <el-date-picker v-model="submitDate" type="datetimerange" value-format="yyyy-MM-dd HH:mm:ss" start-placeholder="请选择开始时间" end-placeholder="请选择结束时间" style="width: 100%;" :default-time="['00:00:00', '23:59:59']">
              </el-date-picker>
            </el-form-item>
          </el-col>
        </el-form>
        ...
        <el-form ref="diaForm" :model="linesQuery" label-width="120px" label-position="top">
          ...
          <el-col :span="12">
            <el-form-item label="客户产品图号">
              <el-input v-model="linesQuery.customerProductDrawingNo" placeholder="请输入客户名称" clearable />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="产品编码">
              <el-input v-model="linesQuery.productCode" placeholder="请输入客户名称" clearable />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="产品名称">
              <el-input v-model="linesQuery.productName" placeholder="请输入客户名称" clearable />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="产品图号">
              <el-input v-model="linesQuery.productDrawingNo" placeholder="请输入客户名称" clearable />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="创建时间">
              <el-date-picker v-model="submitDatemx" type="datetimerange" value-format="yyyy-MM-dd HH:mm:ss" start-placeholder="请选择开始时间" end-placeholder="请选择结束时间" style="width: 100%;" :default-time="['00:00:00', '23:59:59']">
              </el-date-picker>
            </el-form-item>
          </el-col>
        </el-form>
...
<script>
import { getadjustPriceList, deleteAdjustPrice, getadjustPricemxList } from '@/api/salesManagement/priceAdjust'
  ...
  methods: {
    ...
    sortChange ({ prop, order }) {
      ...
      if (this.activeName == "orderList") {
        ...
        if (prop === 'partnerCode' || prop === 'partnerName' || prop === 'productName' || prop === 'productDrawingNo') {
          newProp = prop
        } else {
          newProp = prop.replace(/[A-Z]/g, match => '_' + match.toLowerCase());
        }
    ...
    initData () {
      ...
      if (this.activeName == "orderList") {
        getadjustPriceList(this.orderForm).then(res => {
        ...
        getadjustPricemxList(this.linesQuery).then(res => {
    ...
    search () {
      if (this.activeName == "orderList") {
        ...
        if (this.submitDatemx && this.submitDatemx.length > 0) {
          this.linesQuery.startTime = this.submitDatemx[0].replace(/ 0(?!0)/g, " ")
          this.linesQuery.endTime = this.submitDatemx[1].replace(/ 0(?!0)/g, " ")
        } else {
