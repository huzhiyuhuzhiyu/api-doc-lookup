  ...
  <div class="JNPF-common-layout">
          ...
          <div class="JNPF-common-layout-center JNPF-flex-main">
            ...
            <div class="JNPF-common-layout-main JNPF-flex-main">
              <JNPF-table ref="dataTable" v-loading="listLoading" :data="productionOrderList" :fixedNO="true"
                custom-column>
                <el-table-column prop="orderNo" label="订单号" min-width="180" fixed="left" :key="40"/>
                <el-table-column prop="productCode" label="产品编码" fixed="left" min-width="140" :key="41"/>
                <el-table-column prop="productName" label="产品名称" fixed="left" min-width="160" :key="42"/>
                <el-table-column prop="productDrawingNo" label="产品图号" width="160" :key="43"/>
                <el-table-column prop="orderType" label="订单类型" width="120" :key="44">
                  <template slot-scope="scope">
                    <div v-if="scope.row.orderType == 'normal'">正常订单</div>
                    <div v-if="scope.row.orderType == 'rework'">返工订单</div>
                  </template>
                </el-table-column>
                <el-table-column prop="planStartDate" label="计划开始日期" width="120" :key="45"/>
                <el-table-column prop="planEndDate" label="计划结束日期" width="120" :key="46"/>
                <el-table-column prop="productionQuantity" label="生产数量" width="120" :key="47"/>
                <el-table-column prop="completedQuantity" label="已完成数量" width="120" :key="48"/>
                <el-table-column prop="mainUnit" label="单位" width="120" :key="49"/>
                <el-table-column prop="workOrderNo" label="工作令号" width="160" />
                <el-table-column prop="createTime" label="创建时间" min-width="200" :key="50"/>
                <el-table-column label="操作" width="140" fixed="right" :key="51">
                  <template slot-scope="scope">
          ...
          <div class="JNPF-common-layout-center JNPF-flex-main">
            ...
            <div class="JNPF-common-layout-main JNPF-flex-main">
              <JNPF-table ref="detailTable" v-loading="listLoading" :data="productionWorkOrderList" :fixedNO="true"
                custom-column>
                <el-table-column prop="orderNo" label="工单号" width="180" fixed="left" :key="1"/>
                <el-table-column prop="productName" label="产品名称" fixed="left" width="160" :key="2"/>
                <el-table-column prop="productDrawingNo" label="产品图号" fixed="left" width="160" :key="3"/>
                <el-table-column prop="processCode" label="工序编码" fixed="left" width="160" :key="4"/>
                <el-table-column prop="processName" label="工序名称" width="150" :key="5"> </el-table-column>
                <el-table-column prop="productionOrderNo" label="生产订单号" width="180" :key="6"/>
                <el-table-column prop="workOrderNo" label="工作令号" width="120" :key="7"/>
                <el-table-column prop="workOrderQuantity" label="订单生产数量" width="120" :key="8"/>
                <el-table-column prop="mainUnit" label="单位" width="120" :key="9"/>
                <el-table-column prop="dispatchedQuantity" label="已派工数量" width="120" :key="10"/>
                <el-table-column prop="completedQuantity" label="完成数量" width="200" :key="11"/>
                <el-table-column prop="planStartDate" label="计划开始日期" width="160" :key="12"/>
                <el-table-column prop="planEndDate" label="计划结束日期" width="160" :key="13"/>

                <el-table-column prop="utilizationQuantity" label="是否首道工序" width="200" :key="14">
                  <template slot-scope="scope">
                    <div v-if="scope.row.firstFlag">是</div>
                    <div v-if="!scope.row.firstFlag">否</div>
                  </template>
                </el-table-column>
                <el-table-column prop="productionQuantity" label="是否报工" width="200" :key="15">
                  <template slot-scope="scope">
                    <div v-if="scope.row.reportFlag">是</div>
                    <div v-if="!scope.row.reportFlag">否</div>
                  </template>
                </el-table-column>
                <el-table-column prop="purchaseQuantity" label="是否首末道工序" width="200" :key="16">
                  <template slot-scope="scope">
                    <div v-if="scope.row.lastFlag">是</div>
                    <div v-if="!scope.row.lastFlag">否</div>
                  </template>
                </el-table-column>
                <el-table-column prop="orderType" label="工单类型" width="200" :key="17">
                  <template slot-scope="scope">
                    <div v-if="scope.row.orderType == 'normal'">正常工单</div>
                    <div v-if="scope.row.orderType == 'rework'">返工工单</div>
                  </template>
                </el-table-column>
                <el-table-column prop="createTime" label="创建时间" width="180" :key="18"/>

                <el-table-column label="操作" width="140" fixed="right" :key="19">
                  <template slot-scope="scope">
          ...
          <div class="JNPF-common-layout-center JNPF-flex-main">
            ...
            <div class="JNPF-common-layout-main JNPF-flex-main">
              ...
              <JNPF-table ref="detailTables" v-loading="listLoading" :data="productionDispatchWorkOrderList"
                ...
                <el-table-column prop="orderNo" label="派工单号" width="180" fixed="left" :key="20"/>
                <el-table-column prop="productionOrderNo" label="生产订单号" fixed="left" width="180" :key="21"/>
                <el-table-column prop="workOrderNo" label="工作令号" fixed="left" width="160" :key="22"/>
                <el-table-column prop="workNo" label="工单单号" fixed="left" width="180" :key="23
                "/>
                <el-table-column prop="workGroupName" label="班组名称" width="150" :key="24"> </el-table-column>
                <el-table-column prop="equipmentName" label="设备名称" width="160" :key="25"/>
                <el-table-column prop="personName" label="人员名称" width="120" :key="26"/>
                <el-table-column prop="productionLineName" label="产线名称" width="120" :key="27"/>
                <el-table-column prop="workstationName" label="工位名称" width="120" :key="28"/>
                <el-table-column prop="dispatchQuantity" label="派工生产数量" width="200" :key="29"/>
                <el-table-column prop="completedQuantity" label="完成数" width="120" :key="30"/>
                <el-table-column prop="planStartDate" label="计划开始时间" width="200" :key="31"/>
                <el-table-column prop="planEndDate" label="计划结束时间" width="200" :key="32"/>
                <el-table-column prop="actualStartDate" label="实际开始时间" width="200" :key="33"/>
                <el-table-column prop="actualEndDate" label="实际结束时间" width="200" :key="34"/>
                <el-table-column prop="processCode" label="工序编码" width="160" :key="35"/>
                <el-table-column prop="processName" label="工序名称" width="160" :key="36"/>
                <el-table-column prop="workOrderStatus" label="派工单状态" width="160" :key="37">
                  <template slot-scope="scope">
                ...
                <el-table-column prop="remark" label="备注" width="160" :key="38"/>

                <el-table-column prop="createTime" label="创建时间" width="180" :key="39"/>
              </JNPF-table>
    ...
    <el-dialog title="报工信息" :close-on-click-modal="false" :close-on-press-escape="false" :visible.sync="visible"
      ...
      <el-row :gutter="20">
        ...
        <JNPF-table ref="workerDialog" v-loading="listLoading" :data="workerList" border style="height:40vh">
          <el-table-column prop="productionOrderNo" label="生产订单号"  :show-overflow-tooltip="true" width="160"> </el-table-column>
          <el-table-column prop="workNo" label="工单号"  :show-overflow-tooltip="true" width="120" />
          ...
          <el-table-column prop="processName" label="工序名称" :show-overflow-tooltip="true"  width="150"></el-table-column>
          <el-table-column prop="workGroupName" label="班组名称" :show-overflow-tooltip="true"  width="120"></el-table-column>
          ...
          <el-table-column prop="reportingQuantity" label="报工数量" width="170">
            ...
            <template slot-scope="scope">
              <el-input v-model="scope.row.reportingQuantity" @input="allNumberreport(scope)"  placeholder="请输入报工数量" />
            </template>
          </el-table-column>
          <el-table-column prop="industrialWasteQuantity" label="工废数量" width="170">
            ...
            <template slot-scope="scope">
              <el-input v-model="scope.row.industrialWasteQuantity" @input="allNumberindustrial(scope)"  placeholder="请输入工废数量" />
            </template>
          </el-table-column>
          <el-table-column prop="materialWasteQuantity" label="料废数量" width="170">
            ...
            <template slot-scope="scope">
              <el-input v-model="scope.row.materialWasteQuantity" @input="allNumbermaterial(scope)"  placeholder="请输入料废数量" />
            </template>
          </el-table-column>


          <el-table-column prop="producerId" label="生产人" width="170">
            <template slot="header"><span class="required">*</span>生产人</template>
