    ...
    <div class="JNPF-preview-main org-form">
      <div :class="['JNPF-common-page-header', btnType == 'look' ? 'noButtons' : '']">
        <el-page-header @back="$emit('close')"
          :content="btnType == 'add' ? '新建金工销售报价' : btnType == 'edit' ? '编辑金工销售报价' : btnType == 'custom' ? '生成客户产品' : btnType == 'bjkh' ? '编辑客户产品' : '查看销售报价'" />
      ...
      <el-upload action="#" v-show="false" accept=".xls, .xlsx" :headers="{ token }" ref="UploadPrice"
        ...
        <!-- 选择金工产品 -->
        <ComSelect-page ref="ComSelect-product" @change="addth" :tableItems="ProductTableItems" title="选择产品" treeTitle="产品分类"
...
<script>
import { addQuotationData, editQuotationMData, getQuotationInfo, denerateQuotationMData, calculatequotationData, saleUploadData, saleUploadAmountsCount ,exportNoProduct} from "@/api/salesManagement/index";
...
import { getcategoryTree as  getproductCategoryTree} from '@/api/basicData/materialSettings' // 产品分类
...
export default {
  ...
  data() {
    return {
      ...
      ProductMethodArr: [
        { label: "物料分类", classAttribute: "material", method: getproductCategoryTree, requestObj: { classAttribute: "material" } },
  ...
  methods: {
    ...
    //       row.customerProductDrawingNo = res.data.customerDrawingNumber
      ...
      if (data.length) {
        ...
        if (this.dataFormTwo.lines.length) {
          ...
          selectArr = selectArr.filter(item1 => {
            const index = this.dataFormTwo.lines.findIndex(item2 => item2.productsId === item1.productsId);
            if (index !== -1) {
        ...
        this.dataFormTwo.lines = [...this.dataFormTwo.lines, ...selectArr,]
      }
    },
    // 联系人信息新增行
    addtable(type, data) {
    ...
    UploadProduct(data) {
      ...
      formData.append("businessCode", 'metalworking')
      //调用上传文件接口
    ...
    downLoadTemplate() {
      ...
      a.setAttribute('href', location.origin + '/static/销售报价导入模板 (金工).xlsx')
      a.click()
    },
    // 联系人信息删除当前行
    deltable(row, index) {
    ...
    init(id, btnType) {
      ...
      if (this.btnType == 'add' && !this.dataForm.id) {
        ...
        this.dataForm.quotationType = 'old_metalworking'
      }
      if (this.btnType == 'add' && this.dataForm.id) {
        const end = new Date();//获取当前的日期
        end.setTime(end.getTime())
