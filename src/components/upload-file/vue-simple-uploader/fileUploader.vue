<template>
  <div id="common-file-uploader" :class="{ 'hasDefault': !!value.length }">
    <uploader ref="uploader" :options="options" :autoStart="false" @file-added="onFileAdded" @file-success="onFileSuccess"
      @file-progress="onFileProgress" @file-error="onFileError" class="uploader-app" @complete="onComplete"
...
<script>
import { uploaderFileWithCode } from '@/api/equipment'
...
export default {
  ...
  methods: {
    ...
    handelSuccess(file) {
      ...
      uploaderFileWithCode(form).then(res => {
        // 自定义完成状态
