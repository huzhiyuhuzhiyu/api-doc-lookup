...
import { getbuyFixedPointPricingDetail, addbuyFixedPointPricing, editbuyFixedPointPricing } from '@/api/purchasingManagement/purchaseInquirySheet'  // 询价单
  ...
  data() {
    return {
      ...
      productRules: {
        ...
        bargaining: [
          { required: true, trigger: ['blur'] },
          { validator: this.formValidate({ type: 'decimal', params: [10, 2, "", (errMsg) => { this.$message.error('协议价' + errMsg) }] }), trigger: 'blur' },
          { validator: this.formValidate('noZero', '', (errMsg) => { this.$message.error('协议价不能为0') }), trigger: 'blur' }
        ],
        targetPrice: [
          { validator: this.formValidate({ type: 'decimal', params: [10, 2, "", (errMsg) => { this.$message.error('目标价格' + errMsg) }] }), trigger: 'blur' },
        ...
        if (this.busNodeConfig.childNode) {
          ...
          formNodeList = flattenedNodes.map((item, index) => {
            return {
              ...
              name: item.nodeName,
              designatedMembersId:  item.designatedMembersId ? item.designatedMembersId : '',
            }
          })
          // 抄送人
