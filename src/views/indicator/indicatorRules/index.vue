...
import { deleteIndicatorRules, getIndicatorRulesList } from "@/api/indicator";
  ...
  methods: {
    async initData(listQuery) {
      ...
      try {
        ...
        const res = await getIndicatorRulesList(this.listQuery);
    ...
    handleRemove(id) {
      this.$confirm('您确定要删除这些数据吗, 是否继续？', '提示', {
        ...
        const res = await deleteIndicatorRules(id);
        const { msg } = res
        if (msg === 'Success') {
