      ...
      <div class="JNPF-preview-main org-form">
        <div :class="['JNPF-common-page-header', type === 'look' ? 'noButtons' : '']">
          <el-page-header @back="goBack"
            :content="type == 'add' ? '新建外协收货装箱单' : type == 'edit' ? '编辑外协收货装箱单' : '查看外协收货装箱单'" />
            ...
            <el-tab-pane label="基础信息" name="jcInfo">
              ...
              <el-row :gutter="30" class="">
                <el-form ref="elForm" :model="dataForm" :rules="rules" size="small" label-width="100px"
                  ...
                  <el-col :sm="6" :xs="24">
                    <el-form-item label="外协收货单号" prop="sourceOrderNo" ref="sourceOrderNo">
                      <ComSelect-page :beforeSubmit="beforeSubmit" :renderTree="false" :isdisabled="type === 'look'"
                        ...
                        :tableItems="purchaseTableItems" :placeholder="'请选择外协收货单号'" title="选择外协收货单号"
              ...
              <el-form :model="dataFormTwo" v-bind="dataFormTwo" ref="productForm" class="tableContainer"
                ...
                <el-table style="border: 1px solid #e3e7ee;" hasNO fixedNO v-bind="dataFormTwo.data"
                  ...
                  <el-table-column prop="drawingNo" key="drawingNo" label="产品图号" min-width="200"
                    show-overflow-tooltip>
                    <template slot-scope="scope">
                  ...
                  <el-table-column prop="routingName" key="routingName" label="工艺路线名称" min-width="180"
                    show-overflow-tooltip>
                    <template slot-scope="scope">
                  ...
                  <el-table-column prop="processName" key="processName" label="工序名称" min-width="180"
                    show-overflow-tooltip>
                    <template slot-scope="scope">
  ...
  data() {
    return {
      ...
      purchaseTableItems: [
        ...
        { prop: 'purchaseOrderNo', label: '外协单号', fixed: 'left' },
      ...
      purPurchaseTableSearchList: [
        ...
        { prop: "purchaseOrderNo", label: "外协单号", type: 'input' },
      ],
      activeName: 'jcInfo',
      btnLoading: false,
      index: null,
      ...
      dataForm: {
        ...
        num: '',
        documentStatus: '',
        documentType: 'external_receiving',
        standardBox: '',                   //标准箱
        standardBoxName: '',
        oddBoxName: '',
        oddBox: '',                        // 零头箱
  ...
  methods: {
    ...
    async beforeSubmit(data, paramsObj) {
      ...
      if (paramsObj.oldData.length) {
        flag = await this.$confirm('已选择过外协收货单号，是否切换，切换后会清空供应商信息和产品信息是否继续？', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
    ...
    purchaseData(id, data) {
      ...
      if (data.length === 0) {
        ...
        detailpurPurchaseReceiptReturnGoods(this.dataForm.documentId).then(res => {
          this.dataForm.productType = res.data[0].workOrderId ? 'process' : ''
          let _data = []
          res.data.forEach(item => {
            if (item.receivingStatus === 'stopped') return // 过滤已停止的明细
            _data.push({
              waitPackingQuantity: this.jnpf.math('subtract',[item.qualifiedQuantity,item.packingQuantity]),
              totalNum: this.jnpf.math('subtract',[item.qualifiedQuantity,item.packingQuantity]),
              ...
              num: '',
              routingName: item.lastFlag && (item.reworkType === 'product' || !item.reworkType) ? '' : item.routingIdName,
              routingId: item.lastFlag && (item.reworkType === 'product' || !item.reworkType) ? '' : item.routingId,
              processName: item.lastFlag && (item.reworkType === 'product' || !item.reworkType) ? '' : item.processIdName,
              processId: item.lastFlag && (item.reworkType === 'product' || !item.reworkType) ? '' : item.processId,
              packingQuantity: 0,         // 已装箱
              packedQuantity: '',         // 待装箱
              // totalNum: item.qualifiedQuantity, // 装箱总数 = 检验合格数
