  ...
  <transition name="el-zoom-in-center">
    <div class="JNPF-preview-main org-form">
      ...
      <div :class="['JNPF-common-page-header', btnType ? 'noButtons' : '']">
        ...
        <div class="options" v-if="btnType != '查看半成品变更流程卡'">
...
export default {
  ...
  methods: {
    refreach() {
      ...
      { prop: "hoseLength", label: "软管净长", value: "", type: "input", sm: 8, readOnly: true, placeholder: ' ', itemDisabled: true },
      {
        ...
        itemRules: [{ required: true, message: '请输入正确的数字，整数不能超过8位，小数不能超过2位', trigger: "blur" },
        { validator: this.formValidate({ type: 'decimal', params: [10, 2, "", (errMsg) => { }] }), trigger: 'blur' },]
      },
      { prop: "newRightCoreLength", label: "新右芯子长", value: "", renderTree: this.dataForm.newRightCoreLength ? true : false, type: "input", sm: 8, change: this.changenewRightCoreLength, itemRules: [{ required: true, message: '请输入正确的数字，整数不能超过8位，小数不能超过2位', trigger: "blur" }, { validator: this.formValidate({ type: 'decimal', params: [10, 2, "", (errMsg) => { }] }), trigger: 'blur' }] },
      { prop: "newCompressionElongation", label: "新扣压伸长量", value: "", type: "input", sm: 8, change: this.changenewnewCompressionElongation, itemRules: [{ required: true, message: '请输入正确的数字，整数不能超过8位，小数不能超过2位', trigger: "blur" }, { validator: this.formValidate({ type: 'decimal', params: [10, 2, "", (errMsg) => { }] }), trigger: 'blur' }] },
      {
    ...
    init(id, btnType) {
      ...
      this.refreach()
      if (this.dataForm.id) {
        this.title = btnType === 'look' ? '查看半成品变更流程卡' : '编辑半成品变更流程卡'
        // 获取详情
  ...
  computed: {
    openMode() {
      return this.title === '新建半成品变更流程卡' ? '新建' : this.title === '编辑半成品变更流程卡' ? '编辑' : '只读'
    }
  },
}
</script>
