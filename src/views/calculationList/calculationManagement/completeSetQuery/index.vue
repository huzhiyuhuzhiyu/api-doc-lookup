                ...
                <el-table-column label="操作" width="200" fixed="right">
                  <template slot-scope="scope">
                    <el-button
                      ...
                      type="text"
                      :disabled="
                        scope.row.outputQuantity == 0 ||
                        scope.row.mainProductFlag
                      "
                      @click.native="retrospectFun(scope.row.id, 'produce')"
                      >追溯主产品</el-button
                    >
                    <el-button
                      size="mini"
                      type="text"
                      @click.native="complateSetFun(scope.row.id, 'produce')"
                      >齐套查询</el-button
                    >
                  </template>
                </el-table-column>
                  ...
                  <template slot-scope="scope">
                    <el-button
                      ...
                      :disabled="
                        ...
                        scope.row.mainProductFlag
                      "
                      @click.native="retrospectFun(scope.row.id, 'out')"
                      >追溯主产品</el-button
                    >
                    <el-button
                      size="mini"
                      type="text"
                      @click.native="complateSetFun(scope.row.id, 'out')"
                      >齐套查询</el-button
                    >
                  </template>
                </el-table-column>
  ...
  methods: {
    ...
    getSummaries1(param) {
      ...
      columns.forEach((column, index) => {
        ...
        const values = this.producrData.map((item) =>
          item[column.property] ? Number(item[column.property]) : ""
        );
    ...
    getSummaries3(param) {
      ...
      columns.forEach((column, index) => {
        ...
        const values = this.outData.map((item) =>
          item[column.property] ? Number(item[column.property]) : ""
        );
        if (!values.every((value) => isNaN(value))) {
          sums[index] = values.reduce((prev, curr) => {
