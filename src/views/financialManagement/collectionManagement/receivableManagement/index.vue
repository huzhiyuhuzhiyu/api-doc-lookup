    ...
    <div class="JNPF-common-layout-center JNPF-flex-main">
      <el-row class="JNPF-common-search-box" :gutter="16">
        <el-form @submit.native.prevent>
          ...
          <el-col :span="4">
            <el-form-item>
              <el-input v-model.trim="listQuery.cooperativePartnerCode" placeholder="请输入客户编码" clearable @keyup.enter.native="search()" />
            </el-form-item>
          </el-col>
          <el-col :span="4">
            <el-form-item>
              <el-input v-model.trim="listQuery.cooperativePartnerName" placeholder="请输入客户名称" clearable @keyup.enter.native="search()" />
            </el-form-item>
          </el-col>
      ...
      <div class="JNPF-common-layout-main JNPF-flex-main">
        ...
        <JNPF-table v-loading="listLoading" highlight-current-row :fixedNO="true" ref="tableForm" :data="tableDataList"
          @sort-change="sortChange" @row-dblclick="addOrUpdateHandles" customKey="JNPFTableKey_395639">
          ...
          <el-table-column prop="totalReconciliationAmount" label="应收金额" min-width="180">
            <template slot-scope="scope">
              <el-link type="primary" @click.native="addOrUpdateHandle(scope.row.cooperativePartnerId, 'receivable')">{{
                scope.row.totalReconciliationAmount ? scope.row.totalReconciliationAmount : 0
              }}</el-link>
            </template>
          </el-table-column>
          <el-table-column prop="totalPaymentAmount" label="已收金额" min-width="180">
            <template slot-scope="scope">
              <el-link type="primary" @click.native="addOrUpdateHandle(scope.row.cooperativePartnerId, 'receivable')">{{
          ...
          <el-table-column prop="totalUnpaidAmount" label="未收金额" min-width="180">
            <template slot-scope="scope">
              <el-link type="primary" @click.native="addOrUpdateHandle(scope.row.cooperativePartnerId, 'receivable')">{{
  ...
  methods: {
    ...
    addOrUpdateHandles(e){
      ...
      if (e.cooperativePartnerId) {
        this.$nextTick(() => {
          this.$refs.JNPFForm.init(e.cooperativePartnerId, 'receivable')
        })
      }
    },
    handleDel(id) {
