<template>
  <transition name="el-zoom-in-center">
    <div class="JNPF-preview-main org-form">
        ...
        <el-tabs v-model="activeName" @tab-click="handleClick" class=".el-table">
          <el-tab-pane label="订单信息" name="orderInfo">
              ...
              <el-row :gutter="30" class="custom-row">
                <el-col :sm="8" :xs="24">
                  <el-form-item label="原产品名称" prop="productName">
                    <ComSelect-page v-model="dataForm.productName" @change="submitCustomerProduct"
                      :beforeSubmit="beforeSubmit" :tableItems="ProductTableItems" dialogTitle="选择产品" treeTitle="物料分类"
                      :methodArr="ProductMethodArr" :listMethod="getProductList" :listRequestObj="listQuery"
                      :searchList="ProductTableSearchList" :isdisabled="btnType == 'look'" placeholder="请选择产品" />
                  </el-form-item>
                </el-col>
                <el-col :sm="8" :xs="24">
                  <el-form-item label="拆卸人" prop="transferBy">
                    <user-select v-model="dataForm.transferBy" placeholder="请选择拆卸人" clearable style="width: 100%"
                ...
                <el-col :sm="8" :xs="24">
                  <el-form-item label="拆卸日期" prop="transferDate">
                    <el-date-picker v-model="dataForm.transferDate" type="date" value-format="yyyy-MM-dd"
                      style="width: 100%" placeholder="请选择拆卸日期" :disabled="btnType == 'look' ? true : false">
                    </el-date-picker>
                  </el-form-item>
                </el-col>
                <el-col :sm="8" :xs="24">
                ...
                <el-col :sm="8" :xs="24">
                  <el-form-item label="原箱条码" prop="boxBarcode">
                    <ComSelect-page :placeholder="'请选择原箱条码'" :isdisabled="btnType === 'look' || !dataForm.shelfSpaceName
                      " v-model="dataForm.boxBarcode" :dialogTitle="'选择条码'" ref="ComSelect-pagebox" @change="orginBox"
                      :tableItems="orginTableItems" :renderTree="false" :listMethod="TransferBarCode"
                      :searchList="orginTableSearchList" :listRequestObj="orginRequestObj" :paramsObj="{}"
                      :elementShow="true" />
                  </el-form-item>
                </el-col>
                <el-col :sm="8" :xs="24">
                ...
                <el-col :sm="8" :xs="24">
                  <el-form-item label="数量" prop="num">
                    <el-input v-model="dataForm.num" placeholder="请输入数量" :disabled="btnType == 'look' ? true : false"
                      maxlength="11" />
                  </el-form-item>
                </el-col>
                <el-col :sm="8" :xs="24">
            ...
            <el-form :model="dataFormTwo" ref="productForm" class="data-form">
              <el-table class="TableForm table" ref="product" :data="dataFormTwo.data"
                @selection-change="handeleProductInfoData" style="border: 1px solid #e3e7ee">
                ...
                <el-table-column prop="productName" label="目标产品名称" width="200" show-overflow-tooltip />
                <el-table-column prop="productDrawingNo" label="目标产品图号" width="300" show-overflow-tooltip />
                <el-table-column prop="productSpec" label="目标产品规格型号" width="200" show-overflow-tooltip />
                ...
                <el-table-column prop="shelfSpaceId" label="目标库位" width="260">
                  ...
                  <template slot-scope="scope">
                    <el-form-item :prop="'data.' + scope.$index + '.' + 'shelfSpaceName'"
                      :rules="productRules.shelfSpaceName">
                      <ComSelect-list :dialogTitle="'请选择库位'" v-model="scope.row.shelfSpaceName" :requestObj="kwstock"
                        :isdisabled="btnType === 'look'" :method="stockDisassemblykw" placeholder="请选择目标库位"
                        :paramsObj="{ scope }" @change="changeWarehousekw"></ComSelect-list>
                    </el-form-item>
                  </template>
                </el-table-column>
                <el-table-column prop="batchNumber" label="目标批次号" width="230" show-overflow-tooltip>
                  <template slot-scope="scope">
                    ...
                    <el-select v-model.trim="scope.row.batchNumber" filterable remote reserve-keyword placeholder="请输入批次号"
                      style="width: 100%" clearable :disabled="btnType == 'look'" :remote-method="remoteMethod"
                      :loading="loading" @focus="actionbat">
                  ...
                  <template slot-scope="scope">
                    <el-form-item :prop="'data.' + scope.$index + '.' + 'boxBarcode'" :rules="productRules.boxBarcode">
                      <ComSelect-page :placeholder="'请选择目标箱条码'" :isdisabled="!scope.row.shelfSpaceId || btnType === 'look'
                        " :value="scope.row.boxBarcode" :dialogTitle="'选择条码'" ref="ComSelect-pageBar"
                        @change="targetBox" :tableItems="targetTableItems" :renderTree="false"
                        :listMethod="TransferBarCode" :listRequestObj="targetBoxbarCodeRequestObj(scope.$index)
                ...
                <el-table-column prop="num" label="数量" width="160">
                  ...
                  <template slot-scope="scope">
                    <el-form-item :prop="'data.' + scope.$index + '.' + 'num'" :rules="productRules.num">
                      <el-input v-model="scope.row.num" :disabled="btnType === 'look'" maxlength="11" placeholder="请输入数量"
                        @input="watchnums(scope.row, scope.$index)" style="width: 135px">{{ scope.row.num }}
                      </el-input>
                    </el-form-item>
                  </template>
                </el-table-column>
      ...
      <ComSelect-page ref="ComSelect-page1" @change="submitCustomerProduct1" :tableItems="ProductTableItems1"
        dialogTitle="选择产品" treeTitle="物料分类" :methodArr="ProductMethodArr" :listMethod="getProductList"
        :listRequestObj="ProductListRequestObj" :searchList="ProductTableSearchList1" :elementShow="false" multiple />
    </div>
  </transition>
</template>
<script>
...
import { getWarehouseList, getProductionLotList } from "@/api/basicData/index"; // 仓库树
  ...
  methods: {
    ...
    submitCustomerProduct1(selectedIds, selectedList) {
      selectedList.forEach((item) => {
        ...
        item.all.productSpec = item.all.spec
