                      ...
                      <el-dropdown hide-on-click>
                        ...
                        <el-dropdown-menu slot="dropdown">
                          ...
                          <el-dropdown-item v-if="scope.row.sourceType === 'outbound_sale_send'"
                            @click.native="printOrder(scope.row.id)">
                            打印发货清单
                          </el-dropdown-item>
                          <el-dropdown-item @click.native="exportFile(scope.row.id, 'downLoad')">
                            下载发货清单
                          </el-dropdown-item>
                        </el-dropdown-menu>
                      </el-dropdown>
...
import { getWarehouseList, deleteWarehouseData, getWarehouseLinesList, exportOutboundOrder, WarehousePrintData } from '@/api/warehouseManagement/inboundAndOutbound'
...
export default {
  components: { Form, PrintForm, OutboundOrdersForm },
  data() {
    return {
  ...
  methods: {
    ...
    printOrder(id) {
      ...
      WarehousePrintData(id).then(res => {
        ...
        this.printForm = res.data.stockMove
        // 复制数据测试 打印分页
        // for (var i = 0; i < 6; i++) {
