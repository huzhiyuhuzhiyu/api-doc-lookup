      ...
      <div class="JNPF-preview-main org-form">
        <div :class="['JNPF-common-page-header', type === 'look' ? 'noButtons' : '']">
          <el-page-header @back="goBack"
            :content="type == 'add' ? '新建采购收货装箱单' : type == 'edit' ? '编辑采购收货装箱单' : '查看采购收货装箱单'" />
            ...
            <el-tab-pane label="基础信息" name="jcInfo">
              ...
              <el-form :model="dataFormTwo" v-bind="dataFormTwo" ref="productForm" class="tableContainer"
                ...
                <el-table style="border: 1px solid #e3e7ee;" hasNO fixedNO v-bind="dataFormTwo.data"
                  ...
                  <!-- <el-table-column prop="routingName" key="routingName" label="工艺路线名称" min-width="200"
                    show-overflow-tooltip>
                    <template slot-scope="scope">
                  ...
                  <el-table-column prop="processName" key="processName" label="工序名称" min-width="200"
                    show-overflow-tooltip>
                    <template slot-scope="scope">
                  ...
                  <el-table-column v-if="dataForm.packingType === 'single'" prop="packing" label="标准箱数量"
                    min-width="180">
                    <template slot="header">
  ...
  methods: {
    ...
    showTipBox(type, oldType) {
      if (type === 'single') {
        if (this.mixedTableData.length) {
          this.showType = true
          this.$confirm('已有装箱信息，更换装箱类型，将清空装箱信息，是否继续？', '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning'
          }).then(() => {
    ...
    async handleClick(e) {
      if (e.name === 'boxCode' && this.type === 'add') {
        if (this.dataForm.packingType === 'single') {
          ...
          //     this.tableData[this.tableData.length - 1].totalNum = this.oddprice
          //   }
          // })
    ...
    async beforeSubmit(data, paramsObj) {
      ...
      if (paramsObj.oldData.length) {
        flag = await this.$confirm('已选择过采购收货单号，是否切换，切换后会清空供应商信息和产品信息是否继续？', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
      ...
      this.$nextTick(() => {
        ...
        if (!this.dataForm.id) {
          ...
          detailSalePackingList(this.dataForm.id).then(res => {
            ...
            this.dataForm.sourceNoDeliverDate = res.data.sourceNoDeliverDate
            // 明细列表
            let _data = []
            ...
            // 装箱单条码信息
            if (this.dataForm.packingType === 'single') {
              res.data.barcodes.forEach(box => {
                box.sourceNoDeliverDate = this.dataForm.sourceNoDeliverDate
                box.documentType = this.dataForm.documentType
              })
