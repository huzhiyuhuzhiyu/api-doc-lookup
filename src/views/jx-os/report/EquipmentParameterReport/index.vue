<template>
    <div class="JNPF-common-layout">
        <div class="JNPF-common-layout-center">
                ...
                <el-form @submit.native.prevent size="medium">
                    <el-col :span="8">
                        <el-form-item label="时间" prop="">
                            <el-date-picker v-model="res[tabIndex].timeData" type="datetimerange" range-separator="-"
                                            style="width: 90%;"
                                            start-placeholder="开始日期" end-placeholder="结束日期" clearable
                                            size="small" @change="timeChange" :default-time="['00:00:00', '12:00:00']"
                            >
                            </el-date-picker>
                        </el-form-item>
                    </el-col>
            ...
            <el-tabs v-model="editableTabsValue" type="card" closable @tab-remove="removeTab" @tab-click="clickTab" style="height:1100px;width: 100%;overflow:auto;">
                <el-tab-pane v-for="(item, index) in res" :key="index" :label="item.name" :name="item.name"></el-tab-pane>
               <div style="">
                   <div id="chart" style="width: 98%;min-height: 500px;max-height: 500px;padding-top: 1vw;"></div>
                   <div class="testbox">
                       <el-table v-if="resDataList.length >0." :data="resDataList[tabIndex]" stripe height="300px" border>
            ...
            <div class="demo-drawer__content">
                <el-form :model="query" ref="ruleForm" label-position="right" label-width="120px">
                    <el-form-item label="时间" prop="timeData" :rules="[{ required: true, message: '时间不能为空'}]">
                        <el-date-picker v-model="query.timeData" type="datetimerange" range-separator="-"
                                        style="width: 90%;"
                                        start-placeholder="开始日期" end-placeholder="结束日期" clearable
                ...
                if (valid) {
                    if (this.regularCheck()) {
                        ...
                        selectEquipmentParameter(res).then(res => {
                            ...
                            //this.codeNameList.splice(this.tabIndex, 1, codeNames)
                            //this.dateList.splice(this.tabIndex, 1, date)
                            //this.nameList.splice(this.tabIndex, 1, _data.name)
                            //this.resDataList.splice(this.tabIndex, 1, arr)//[this.tabIndex] = arr;
...
.el-table{height: 600px;width: 100%;overflow: auto;}
