...
<template>
  <div class="popupSelect-container">
    <div class="el-select" @click.stop="openDialog" @keyup.enter="openDialog">
      <div :class="['el-select__tags', tagMultipleLine ? '' : 'singleLine']" v-if="multiple" ref="tags" :style="{ 'max-width': inputWidth - 32 + 'px', width: '100%', cursor: 'pointer' }">
      ...
      <el-input ref="reference" v-model="innerValue" type="text" :placeholder="currentPlaceholder" :disabled="isdisabled" readonly :validate-event="false" :tabindex="(multiple) ? '-1' : null" @mouseenter.native="inputHovering = true" @mouseleave.native="inputHovering = false">
    ...
    <el-dialog :title="computedDialogTitle" :close-on-click-modal="false" :visible.sync="visible" class="JNPF-dialog JNPF-dialog_center transfer-dialog" lock-scroll append-to-body width="800px" :modal-append-to-body="false" @close="onClose">
      <div class="transfer__body">
        <div class="transfer-pane">
          <div class="transfer-pane__tools">
            ...
            <Search-input placeholder="请输入关键词查询" v-model="keyword" @keyup.enter.native="search" clearable class="search-input" maxlength="20" @buttonClick="search" />
          </div>
          <div class="transfer-pane__body" v-loading="loading">
            <el-tree :data="treeData" :props="props" check-on-click-node :expand-on-click-node="false" default-expand-all @node-click="handleNodeClick" class="JNPF-common-el-tree" node-key="id" ref="tree" show-checkbox @check-change="handleCheckChange" :filter-node-method="filterNode">
  ...
  methods: {
    handleCheckChange(data, checked, indeterminate) {
      if (checked) {
        if (!data.childrenList.length) {
          const boo = this.rSelectData.some(o => o.id === data.id)
          if (boo) return
          this.rSelectData.push(data)
        }
      } else {
        if (!data.childrenList.length) {
          const boo = this.rSelectData.filter(o => o.id !== data.id)
          this.rSelectData = boo
        }
      }
    },
    async openDialog() {
    ...
    async confirm() {
      ...
      for (let i = 0; i < this.rSelectData.length; i++) {
        selectedData.push({
          id: this.rSelectData[i].id,
          name: this.rSelectData[i].name,
          all: this.rSelectData[i]
        })
      }
