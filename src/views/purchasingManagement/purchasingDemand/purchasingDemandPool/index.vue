          ...
          <el-table-column prop="source" label="来源" min-width="140" sortable="custom">
            <template slot-scope="scope">
              ...
              <el-link type="primary" @click.native="getPoolSourceList(scope.row.id,)"
                v-if="scope.row.source == 'procure'">请购单</el-link>
              <el-link type="primary" @click.native="getPoolSourceList(scope.row.id,)"
                v-if="scope.row.source == 'mrp'">MRP下发</el-link>
              <el-link type="primary" @click.native="getPoolSourceList(scope.row.id,)"
                v-if="scope.row.source == 'order_distribute'">订单分配</el-link>
...
export default {
  ...
  methods: {
    ...
    initData() {
      ...
      purProcurementDemandPoolList(this.listQuery).then(res => {
    ...
    batchQuiry() {
      if (this.selectData.length === 0) {
        ...
        if (quirtFlag) {
          ...
          quiryData = Object.values(quiryData.reduce((acc, item) => {
