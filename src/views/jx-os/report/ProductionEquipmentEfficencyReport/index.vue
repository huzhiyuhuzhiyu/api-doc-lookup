    ...
    <div class="JNPF-common-layout">
        ...
        <div class="JNPF-common-layout-center">
                ...
                <el-form @submit.native.prevent size="medium">
                    ...
                    <el-col :span="6">
                        <el-form-item label="稼动率">
                            <JNPFTreeSelect v-model="query.method" placeholder="请选择" :options="efficencyOptions"
                                            :props="efficencyProps"
                            />
                        </el-form-item>
                    </el-col>
                    <el-col :span="5">
            ...
            <div class="JNPF-common-layout-main JNPF-flex-main">
                ...
                >
                    <el-table-column prop="index" type="index" width="60" label="序号"
                                     align="center" fixed
                    />
                    <el-table-column prop="name" label="设备名称" align="left" min-width="180" fixed/>
                    <el-table-column v-for="(item,index2) in times" :key="item" :label="item"
                                     style="background-color: aqua;"
                    >
                        <template slot-scope="scope">
                            <span><strong>{{ (scope.row.results[index2].data * 100).toFixed(0) }}%</strong></span>
                        </template>
                    </el-table-column>
...
import { getEquipmentEfficency_v2 } from '@/api/report'
...
export default {
    ...
    data() {
        return {
            name: 'ProductionEquipmentEfficencyReport',
            expandObj: {},
            columnOptions: [],
            mergeList: [],
            columnList,
    ...
    methods: {
                ...
                for (var i = 0; i < row.results.length; i++) {
                    if (i == columnIndex - 2) {
                        ...
                        for (var j = 0; j < this.normList.length; j++) {
                            ...
                            let _data = (row.results[i].data * 100).toFixed(0)
        ...
        initData() {
            ...
            //_query.params.start = new Date(moment((new Date(this.query.time[0]).getTime())).format('YYYY-MM-DD 00:00:00')).getTime()
            //_query.params.stop =  new Date(moment((new Date(this.query.time[1]).getTime())).format('YYYY-MM-DD 23:59:59')).getTime()

            getEquipmentEfficency_v2(_query).then(res => {
                var _list = []
