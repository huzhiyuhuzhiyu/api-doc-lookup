    ...
    <template v-if="popupType==='dialog'">
      <el-dialog :title="popupTitle" :close-on-click-modal="false" :visible.sync="visible"
        ...
        <JNPF-table v-loading="listLoading" :data="list" :border="false" highlight-current-row
          @row-click="rowClick" :hasNO="false" customKey="JNPFTableKey_618226">
          ...
          <template v-for="(item,i) in columnOptions">
              <el-table-column :prop="value" :label="item.label" v-if="item.type==='img'" >
                  <template slot-scope="scope">
                      <!-- {{scope.row.backgroundUrl}} -->
                    <img height="80" :src="scope.row.backgroundUrl"/>
                    <!-- <el-image :src="scope.row.value"></el-image> -->
                  </template>
                </el-table-column>
              <el-table-column :prop="item.value" :label="item.label" v-else />
    ...
    <template v-if="popupType ==='drawer'">
      <el-drawer :title="popupTitle" :visible.sync="visible" :wrapperClosable="false" ref="drawer"
        ...
        <div class="JNPF-flex-main">
          ...
          <JNPF-table v-loading="listLoading" :data="list" :border="false" highlight-current-row
            @row-click="rowClick" :hasNO="false" customKey="JNPFTableKey_917553">
            <el-table-column width="35">
              <template slot-scope="scope">
                <el-radio :label="scope.row[propsValue]" v-model="checked">&nbsp;</el-radio>
              </template>
