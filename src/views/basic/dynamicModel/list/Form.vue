  ...
  methods: {
    ...
    init(formConf, modelId, id, isPreview, useFormPermission) {
      ...
      this.$nextTick(() => {
        ...
        // var x = JSON.parse(this.dataForm.data)
        // x.bean_content.forEach(function(item,index){
        //     this.formConf.fields.push(JSON.parse(item.replace("\\","")))
        // })




        // this.formConf.fields.forEach(function(item,index){
        //     if(item.__vModel__=="bean_content"){

        //           console.log(item);
        //           console.log(item['__config__'].defaultValue);

        //           // var config = JSON.parse(item['__config__']).replace("\\",""))
        //           // console.log(config);
        //         // var config = JSON.parse(JSON.stringify(item.__config__).replace("\\",""))
        //         // console.log(typeof config,config.defaultValue);
        //     }
    ...
    submitForm(data, callback) {
      ...
      for(var p in data){//遍历json对象的每个key/value对,p为key
          if(p.indexOf("bean_")>-1 && p!="bean_content"){
              TempJson[p.replace("bean_","")] = data[p]
          }
      }
      // console.log(TempJson);
      data.bean_content = JSON.stringify(TempJson);
      // End **************************************************************************************************
