...
export default {
  ...
  computed: {
    ...
    isAutoRecommend() {
      return this.formType === 'autoRecommend';
    },
    totalNum() {
  ...
  methods: {
    ...
    async getInitBatchPackingList(id) {
      ...
      try {
        ...
        if (msg === 'Success') {
          ...
          this.linesList = this.isPacking ? distributionList : batchNumberList;
    ...
    getTitle() {
      ...
      const actionText = actionMap[this.btnType] || '';

      const baseTitle = this.isPacking ? '装箱清单' : '智能推荐批次';
      this.collapseTitle = this.isPacking ? '产品分配信息' : '批次推荐信息';
    ...
    async handleSubmit() {
      if (this.isAutoRecommend && (!this.linesList || this.linesList.length === 0)) {
        this.$message.warning('未进行批次推荐，请勿提交');
        return;
      }
      ...
      const valid = await this.$refs['tableForm'].$refs.main.validate().catch(() => false);
      if (!valid) {
      ...
      const linesFieldName = this.isAutoRecommend ? 'stockPickedLines' : 'stockPackingListLines';
      const mainFieldName = this.isAutoRecommend ? 'stockPicked' : 'stockPackingList';

      const linesData = this.isPacking
            ...
            <div class="main" v-loading="loading" ref="main">
              <el-collapse v-model="activeNames" style="margin-top: 5px;" @change="refreshTableHeight">
                <el-collapse-item :title="isTypeAdd ? '库存信息' : '基本信息'" name="basicInfo" :class="isTypeAdd ? 'productInfo' : 'orderInfo'" ref="dataFormRegion">
                  ...
                  <template v-else>
                    <JNPF-col v-model="dataForm" :tabContent="basicFormSchema" ref="dataForm" btnType="look"/>
                  </template>
                </el-collapse-item>
                    ...
                    <template slot="actions">
                      <el-table-column label="操作" width="100" fixed="right">
                        <template slot-scope="scope">
                          <el-button type="text" class="JNPF-table-delBtn" @click="linesDelete(scope)">
                            {{ $t(`common.delBtn`) }}
                          </el-button>
                        </template>
                      </el-table-column>
