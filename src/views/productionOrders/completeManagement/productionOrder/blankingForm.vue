            ...
            >
              <template slot-scope="scope">
                ...
                <el-button
                  ...
                  class="underline-button"
                  :disabled="scope.row.processingType != 'self_produced'"
                  >{{scope.row.equipmentId ?scope.row.equipmentCode? scope.row.equipmentName+"("+scope.row.equipmentCode+")":scope.row.equipmentName : "请选择设备"}}</el-button
                >
              </template>
            </el-table-column>
            <el-table-column
...
export default {
  ...
  methods: {
    ...
    selectResouce(item, type) {
      ...
      if (type == "设备") {
        this.blankingForm.blankingList[this.index].equipmentId = item.split("_")[0];
        this.blankingForm.blankingList[this.index].equipmentName = item.split("_")[1]+"("+item.split("_")[2]+")";

        // this.blankingForm.blankingList[this.index].routingProResMap.device.forEach((item) => {
        //   if (
        //     item.resourceId ==
          ...
          res.data.workOrderList.forEach((item) => {
            ...
            if (item.firstFlag) {
              response.data.forEach((items) => {
                ...
                items.equipmentCode = item.equipmentCode;
