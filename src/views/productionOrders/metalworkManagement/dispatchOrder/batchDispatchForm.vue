          ...
          ">
                ...
                <el-form ref="dataForms" :model="dispatchForm" :rules="dataRule" label-width="160px" label-position="top">
                    <el-row :gutter="30" class="custom-row">
                        <el-col :sm="12" :xs="24">
                            <el-form-item label="派工单号" prop="orderNo">
                                <el-input readonly placeholder="请输入派工单号" disabled
                                    v-model="dispatchForm.orderNo"></el-input>
                            </el-form-item>
                        </el-col>

                        <el-col :sm="12" :xs="24">
                            <el-form-item label="产品编码" prop="productCode">
                                <el-input placeholder="请输入产品编码" disabled v-model="dispatchForm.productCode"></el-input>
                            </el-form-item>
                        </el-col>
                        <el-col :sm="12" :xs="24">
                            <el-form-item label="产品名称" prop="productName">
                                <el-input placeholder="请输入产品名称" disabled v-model="dispatchForm.productName"></el-input>
                            </el-form-item>
                        </el-col>

                        <el-col :sm="12" :xs="24">
                            <el-form-item label="产品图号" prop="productDrawingNo">
                                <el-input placeholder="请输入产品图号" disabled v-model="dispatchForm.productDrawingNo"></el-input>
                            </el-form-item>
                        </el-col>
                        <el-col :sm="12" :xs="24">
                            <el-form-item label="工序编码" prop="processCode">
                                <el-input placeholder="请输入工序编码" disabled v-model="dispatchForm.processCode"></el-input>
                            </el-form-item>
                        </el-col>
                        <el-col :sm="12" :xs="24">
                            <el-form-item label="工序名称" prop="processName">
                                <el-input placeholder="请输入工序名称" disabled v-model="dispatchForm.processName"></el-input>
                            </el-form-item>
                        </el-col>
                        <el-col :sm="12" :xs="24">
                            <el-form-item label="派工数量" prop="dispatchQuantity">
                                <el-input placeholder="请输入派工数量" disabled v-model="dispatchForm.dispatchQuantity"></el-input>
                            </el-form-item>
                        </el-col>
                        <el-col :sm="12" :xs="24">
                            <el-form-item label="已完成数量" prop="completedQuantity">
                                <el-input placeholder="请输入已完成数量" disabled v-model="dispatchForm.completedQuantity"></el-input>
                            </el-form-item>
                        </el-col>
                        <el-col :sm="12" :xs="24">
                  <el-form-item label="已报工废数量" prop="industrialWasteQuantity">
                    <el-input placeholder="请输入已报工废数量" disabled v-model="dispatchForm.industrialWasteQuantity"></el-input>
                  </el-form-item>
                </el-col>
                        <el-col :sm="12" :xs="24">
                            <el-form-item label="班组" prop="workGroupId">
                                <el-select v-model="dispatchForm.workGroupId" placeholder="" class="applySelect" disabled style="width: 70%; display: none" >
                                <el-option v-for="(item, index) in dispatchForm.routingProResMap ? dispatchForm.routingProResMap.work_group : []" :key="index" :label="item.resourceName" :value="item.resourceId"></el-option>
                            </el-select>
                        ...
                        <el-col :sm="12" :xs="24">
                            <el-form-item label="设备" prop="equipmentId">
                                ...
                                <el-option v-for="(item, index) in dispatchForm.routingProResMap? dispatchForm.routingProResMap.device: []" :key="index" :label="item.resourceName + '(' + item.resourceCode + ')'"
                                    :value="item.resourceId"></el-option>
                            </el-select>
                            <el-button @click="selectDeviceFun(dispatchForm)" type="text" class="underline-button" :disabled="dispatchForm.processingType != 'self_produced' ">
                                {{dispatchForm.equipmentId ? dispatchForm.equipmentName+"("+dispatchForm.equipmentCode+')' : "请选择设备"}}
                            </el-button>
                            </el-form-item>
                        </el-col>
                        <el-col :sm="12" :xs="24">
                            <el-form-item label="人员" prop="personId">
                                <el-select v-model="dispatchForm.personId" placeholder="" clearable
                                style="width: 60%; display: none" class="applySelect" disabled>
                                <el-option v-for="(item, index) in dispatchForm.routingProResMap ? dispatchForm.routingProResMap.personnel: []" :key="index" :label="item.resourceName" :value="item.resourceId"></el-option>
                            </el-select>
    ...
    methods: {
        ...
        selectResouce(item, type) {
            ...
            if (type == "人员") {
               ...
               this.dispatchForm.routingProResMap.personnel.forEach((item) => {
            ...
            if (type == "班组") {
               ...
               this.dispatchForm.routingProResMap.work_group.forEach((item) => {
            ...
            if (type == "设备") {
                ...
                // this.dispatchForm.equipmentName = item.split("_")[1]+"("+item.split("_")[2]+")";
        ...
        selectWorkgroupFun(scope) {
            ...
            if (this.dispatchForm.routingProResMap) {
                if (this.dispatchForm.routingProResMap.work_group) {
                    this.index = scope.$index;
                    this.currentWorkgroupId = this.dispatchForm.workGroupId;
                    let workgroupData = this.dispatchForm.routingProResMap.work_group;
        ...
        selectDeviceFun(scope) {
            ...
            if (this.dispatchForm.routingProResMap) {
                if (this.dispatchForm.routingProResMap.device) {
                    this.index = scope.$index;
                    this.currentDeviceId = this.dispatchForm.equipmentId ? this.dispatchForm.equipmentId : "";
                    let deviceDataData = this.dispatchForm.routingProResMap.device;
                    this.routingProResMapDiaFlag = true;
        ...
        selectPersonnelFun(scope) { 
            ...
            if (this.dispatchForm.routingProResMap) {
                if (this.dispatchForm.routingProResMap.personnel) {
                    this.currentPersonId = this.dispatchForm.personId;
                    this.index = scope.$index;
                    let personnelData = this.dispatchForm.routingProResMap.personnel;
                    this.routingProResMapDiaFlag = true;
      ...
      detailDispatchData(id).then(res => {
        console.log("派工单详情",res);
        this.dispatchForm=res.data.dispatch
            ...
            prodOrderInfo(obj).then((res) => {
                ...
                if(item.processId==this.dispatchForm.processId){
                    ...
                    if (item.routingProResMap) {
                        this.dispatchForm.routingProResMap=item.routingProResMap
                    this.dispatchForm.processingType=item.processingType
                    this.dispatchForm.firstFlag=item.firstFlag
                        // this.dispatchForm.routingProResMap=
                        if (item.routingProResMap.personnel) {
                            this.dispatchForm.personId = item.routingProResMap.personnel[0].resourceId;
                            this.dispatchForm.personName = item.routingProResMap.personnel[0].resourceName;
                        }
                        console.log("6666", typeof item.routingProResMap.personnel);
                        if (
                             this.dispatchForm.routingProResMap.work_group &&
                             this.dispatchForm.routingProResMap.work_group.length == 1
                        ) {
                            this.dispatchForm.workGroupId = item.routingProResMap.work_group[0].resourceId;
                            this.dispatchForm.workGroupName = item.routingProResMap.work_group[0].resourceName;
                        }
                        if (
                            this.dispatchForm.routingProResMap.device &&
                            this.dispatchForm.routingProResMap.device.length == 1
                        ) {
                            this.dispatchForm.equipmentId = item.routingProResMap.device[0].resourceId;
                            this.dispatchForm.equipmentName = item.routingProResMap.device[0].resourceName;
                            this.dispatchForm.equipmentCode= item.routingProResMap.device[0].resourceCode;
                        }
                    } else {
                        this.dispatchForm.routingProResMap=[]
                    }
