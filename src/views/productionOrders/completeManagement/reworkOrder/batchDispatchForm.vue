        ...
        <div class="JNPF-preview-main org-form">
            ...
            <div class="main" v-loading="formLoading">
                ...
                <el-table ref="product" :data="workerList" :fixedNO="false" style="width:100%;" border class="objName">
                    ...
                    <el-table-column prop="workGroupId" label="班组" min-width="150">
                        ...
                        <template slot-scope="scope">
                            <el-select v-model="scope.row.workGroupId" placeholder="请选择班组" clearable class="applySelect"
                                disabled style="width:70%;display:none" @clear="clearGroup(scope)">
                                <el-option
                    ...
                    scope.row.workGroupId ? scope.row.workGroupName : '请选择班组' }}</el-button>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column prop="personId" label="人员" min-width="100">
                        ...
                        <template slot-scope="scope">
                            <el-select v-model="scope.row.personId" placeholder="请选择人员" style="width:70%;display:none"
                            ...
                            <el-button @click="selectPersonnelFun(scope)" type="text" class="underline-button"
                                :disabled="scope.row.processingType != 'self_produced'">{{
                    scope.row.personId ? scope.row.personName : '请选择人员' }}</el-button>
                        </template>
                    </el-table-column>
            ...
            <el-dialog :title="routingProResMapDiaTitle" :close-on-click-modal="false" :close-on-press-escape="false"
                ...
                <el-row class="row-bg" :gutter="0" style="margin:12px 0;" v-if="routingProResMapDiaTitle == '人员资源'">
                    ...
                    .replace('-', '.') }}
                                    {{ totalData[item][0].completedQuantity }}/{{ totalData[item][0].dispatchQuantity
                                    }}个
                                ...
                                <p style="margin-top:4px; font-size:12px">
                                    ...
                                    {{ totalData[item][1].completedQuantity }}/{{ totalData[item][1].dispatchQuantity
                                    }}个
                                </p>
                            </div>
                    ...
                    totalData[item][0].planEndDate.match(/-(\d{2}-\d{2})/)[1].replace('-', '.') }}
                            ...
                            <p v-if="totalData[item].length" style="margin-top:4px; font-size:12px">派工截止：{{
                    totalData[item][totalData[item].length -
                        1].planEndDate.match(/-(\d{2}-\d{2})/)[1].replace('-', '.') }} <el-button
                                    class="elbutton" type="text" size="mini"
                ...
                <el-row class="row-bg" :gutter="0" style="margin:12px 0;" v-if="routingProResMapDiaTitle == '班组资源'">
                    <el-col :span="4" class="personBox"
                        ...
                        <div class="grid-content bg-purple" :style="!totalData[item].length ? 'height:100%' : ''"
                            ...
                            <div v-if="totalData[item].length > 1">
                                <p style="margin-top:4px; font-size:12px">
                                    {{ totalData[item][totalData[item].length -
                    2].planStartDate.match(/-(\d{2}-\d{2})/)[1]
                    .replace('-', '.') }}-
                                    {{ totalData[item][totalData[item].length -
                    2].planEndDate.match(/-(\d{2}-\d{2})/)[1]
                    .replace('-', '.') }}
                    ...
                    totalData[item][totalData[item].length - 2].dispatchQuantity }}个
                                ...
                                <p style="margin-top:4px; font-size:12px">
                                    {{ totalData[item][totalData[item].length -
                    1].planStartDate.match(/-(\d{2}-\d{2})/)[1].replace('-', '.') }}-
                                    {{ totalData[item][totalData[item].length -
                    1].planEndDate.match(/-(\d{2}-\d{2})/)[1]
                    .replace('-', '.') }}
            ...
            <el-dialog title="派工单数据" :close-on-click-modal="false" :close-on-press-escape="false" append-to-body
                ...
                <el-table ref="product" :data="detailDataList"  max-height="600">
                        ...
                        <el-table-column prop="orderNo" label="派工单号" width="200"></el-table-column>
...
import { getprodOrderList, prodOrderInfo, addProdOrder, updateprodOrderFinished, prodOrderDispatch, dispatchListMap } from '@/api/productOrdes/finishedProductOrders'
import { getProductionLineList, getProductionLineInfo, excelExport } from '@/api/basicData/index'
        ...
        return {
            ...
            productRules: {
                ...
                dispatchQuantity: [
                    ...
                    { validator: this.formValidate({ type: 'decimal', params: [20, 4, "", (errMsg, index) => { this.$message.error('第' + (index + 1) + '行：派工数量' + errMsg) }] }), trigger: 'blur' },
                    { validator: this.formValidate('noZero', '', (errMsg, index) => { this.$message.error('第' + (index + 1) + '行：派工数量不能为0') }), trigger: 'blur' }
    ...
    methods: {
        ...
        selectResouce(item, type) {
            ...
            if (type == "设备") {
                ...
                //         this.workerList[this.index].equipmentName = item.resourceName + '(' + item.resourceCode + ')'
                //     }
                // })
            }
            ...
            if (scope.row.routingProResMap) {
                if (scope.row.routingProResMap.personnel) {
                    ...
                    dispatchListMap(obj).then(res => {
                        ...
                        this.personnelData = Object.keys(res.data);
                        this.totalData = res.data
                ...
                if (valid) {
                    ...
                    if (!valid_2) {
                        ...
                        for (let index = 0; index < this.dataFormTwo.prodOrderList.length; index++) {
                            ...
                            if (!item.blankingProductsId) {
                                console.log(123, Number(item.dispatchQuantity) + Number(item.dispatchedQuantity), item.productionQuantity);
                        ...
                        for (let index = 0; index < this.workerList.length; index++) {
                            ...
                            if (!item.workGroupId && !item.equipmentId && !item.personId && !(this.dataFormTwo.prodOrderList)[0].blankingProductsId && item.processingType == 'self_produced') {
                                submitFlag = false
                                this.btnLoading = false
                                this.$message({
