<template>
  <div>
    ...
    <el-form :model="JNPFColTableData" ref="main">
      <el-table class="TableForm table" :data="JNPFColTableData.data" hasNO fixedNO hasC
        ...
        <el-table-column label="操作" width="180" :fixed="hasFixed ? 'right' : false">
          <template slot-scope="scope">
            <el-button size="mini" type="text" class="JNPF-table-delBtn" :disabled="openMode === '只读'"
              @click="deleteth(scope)">删除</el-button>
            <el-button size="mini" type="text" @click="handlerOpenSource(scope)" :style="{color:scope.row.allocated?'#67c23a':'#1890ff'}">货位设置</el-button>
          </template>
        </el-table-column>
