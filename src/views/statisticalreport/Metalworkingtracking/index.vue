  ...
  <div class="JNPF-common-layout">
    <div class="JNPF-common-layout-center JNPF-flex-main">
      ...
      <div class="JNPF-common-layout-main JNPF-flex-main">
        <JNPF-table v-loading="listLoading" highlight-current-row :fixedNO="true" ref="dataTable" :data="tableDataList" @sort-change="sortChange" custom-column customKey="JNPFTableKey_775814">
          <el-table-column prop="createDate" label="日期" min-width="130" />
          <el-table-column prop="equipmentCode" label="设备编码" min-width="140" sortable="custom" />
          ...
          <el-table-column prop="pickingStatus" label="领料状态" min-width="110">
            <template slot-scope="scope">
              <div v-if="scope.row.pickingStatus == 'not_picked'">未领料</div>
              <div v-else-if="scope.row.pickingStatus == 'picking'">领料中</div>
              <div v-else-if="scope.row.pickingStatus == 'picked'">已领料</div>
            </template>
          </el-table-column>
...
export default {
  ...
  created() {
    ...
    this.listQuery.deliverDateArr = this.nearMonth()  //近3个月的日期区间
    this.initData()
  },
  methods: {
    // 导出
    ...
    nearMonth() {
      ...
      startDate = new Date(nowYear, nowMonth, nowDay - 90);
      endDate = new Date(nowYear, nowMonth, nowDay);
      return [formatDate(startDate), formatDate(endDate)]
    },
    sortChange({ prop, order }) {
      let newProp;
      if (prop === 'productDrawingNo' || prop === 'equipmentCode' || prop === 'processName') {
        newProp = prop
      } else {
