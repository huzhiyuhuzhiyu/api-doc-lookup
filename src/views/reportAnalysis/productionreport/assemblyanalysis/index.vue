    ...
    <div class="JNPF-common-layout-center JNPF-flex-main">
      <el-row class="JNPF-common-search-box" :gutter="16">
        <el-form @submit.native.prevent>
          <el-col :span="4">
            <el-form-item>
              <el-date-picker v-model="deliveryDate" type="daterange" value-format="yyyy-MM-dd" :clearable="false" start-placeholder="交货开始日期" end-placeholder="交货结束日期" @change="changePicker" :picker-options="pickerOptions">
              </el-date-picker>
            </el-form-item>
          </el-col>
          <el-col :span="4">
      ...
      <div class="JNPF-common-layout-main JNPF-flex-main">
        <JNPF-table ref="dataTable" show-summary :summary-method="getSummaries" v-loading="listLoading" :data="tableData" custom-column :fixedNO="true" customKey="JNPFTableKey_741714">
          ...
          <el-table-column prop="ordersNum" label="销售订单数量" min-width="140" />
          <el-table-column prop="completedQuantity" label="已完工数量" min-width="140" />
          <el-table-column prop="notCompletedQuantity" label="未完工数量" min-width="140">
            <template slot-scope="scope">
              <div>{{ scope.row.notCompletedQuantity > 0 ? scope.row.notCompletedQuantity : 0 }}</div>
            </template>
          </el-table-column>
          ...
          <el-table-column prop="calcState" label="分析状态" min-width="120">
            <template slot-scope="scope">
              <div v-if="scope.row.calcState=='not_calculated'">未分析</div>
              <div v-else-if="scope.row.calcState=='calculated'">已分析</div>
            </template>
          </el-table-column>
        </JNPF-table>
        ...
        <el-form :model="dataForm" label-width="120px" label-position="top">
          ...
          <el-col :span="12">
            <el-form-item label="订单日期">
              <el-date-picker v-model="orderDate" type="daterange" value-format="yyyy-MM-dd" :clearable="false" start-placeholder="订单开始日期" end-placeholder="订单结束日期">
              </el-date-picker>
            </el-form-item>
          </el-col>
          <el-col :span="12">
...
<script>
import { getmateriallist } from '@/api/reportAnalysis/saleReport'
import ExportForm from '@/components/no_mount/ExportBox/index'
import { excelExport } from '@/api/basicData/index'
export default {
  name: 'assemblyanalysis',
  components: { ExportForm },
  data() {
    return {
      analyzestatusList: [
        { label: "未分析", value: "not_calculated" },
        { label: "已分析", value: "calculated" }
      ],
      orderDate: [],
  ...
  created() {
    ...
    this.deliveryDate = this.nearMonth()  //近一个月的日期区间
    this.initData()
  },
  methods: {
    // 合计处理
    ...
    initData() {
      this.jnpf.searchTimeFormat(this.dataForm, this.deliveryDate, 'deliveryStartDate', 'deliveryEndDate')
      this.jnpf.searchTimeFormat(this.dataForm, this.orderDate, 'orderStartDate', 'orderEndDate')
      // if (this.orderDate && this.orderDate.length > 0) {
      //   this.dataForm.orderStartDate = this.orderDate[0]
      //   this.dataForm.orderEndDate = this.orderDate[1]
      // } else {
      ...
      getmateriallist(this.dataForm).then(res => {
