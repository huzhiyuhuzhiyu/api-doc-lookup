<template>
    <div class="JNPF-common-layout">
                ...
                <JNPF-table v-loading="listLoading" :data="list" row-key="id" default-expand-al :key="1"
                                    ...
                                    <template v-if="scope.row.rowEdit">
                                        ...
                                        <template v-else-if="item.jnpfKey==='time'">
                                            <el-time-picker v-model="scope.row[item.prop]" style="width:100%"
                                                ...
                                                :format="item.format" :readonly="item.readonly"
                                                :disabled="item.disabled">
                                            </el-time-picker>
                                        </template>
                                        ...
                                        <template v-else-if="['comInput','textarea'].includes(item.jnpfKey)">
                                            <el-input v-model="scope.row[item.prop]" :placeholder="item.placeholder"
                                                ...
                                                <template slot="prepend" v-if="item.__slot__ && item.__slot__.prepend">{{item.__slot__.prepend}}</template>
                                                <template slot="append" v-if="item.__slot__ && item.__slot__.append">{{item.__slot__.append}}</template>
                                            </el-input>
                                        </template>
                    ...
                    <template v-else>
                        <template v-for="(item, i) in columnList">
                            ...
                            <el-table-column :prop="item.prop" :label="item.label" :align="item.align"
                                :width="item.width" :key="i" :sortable="item.sortable?'custom':item.sortable" v-else />
                        </template>
                    </template>
        ...
        <SuperQuery v-if="superQueryVisible" ref="SuperQuery" :columnOptions="columnOptions" @superQuery="superQuery" />
        <candidate-form :visible.sync="candidateVisible" :candidateList="candidateList" :branchList="branchList"
            taskId="0" :formData="workFlowFormData" @submitCandidate="submitCandidate" :isCustomCopy="isCustomCopy" />
    </div>
</template>
    ...
    export default {
        ...
        methods: {
            async init() {
                ...
                console.log("this.columnData111",this.columnData);
                if (this.columnData.type === 3) {
                    this.columnData.columnList = this.columnData.columnList.filter(o => o.prop != this.columnData
                        .groupField)
                }
                if (this.config.webType == 3) {
                    this.flowTemplateJson = this.config.flowTemplateJson ? JSON.parse(this.config
                        .flowTemplateJson) : {}
                    this.isCustomCopy = this.flowTemplateJson.properties && this.flowTemplateJson.properties
                        .isCustomCopy
                }
                ...
                getDataInterfaceRes('408600087489414149').then(res => {
                    let data = res.data
                    this.prpupFileds = data;
                    this.getColumnList()
                })
            ...
            initData() {
                ...
                getModelList(this.modelId, this.listQuery).then(res => {
                    ...
                    for (let i = 0; i < res.data.list.length; i++) {
                        ...
                        // _data.bean_content = JSON.parse(_data.bean_content.replace("\\",""))
                        // for (let j = 0; j < _data.bean_content.length; j++) {
                        //     var x = _data.bean_content[j].fieldName
                        //     _data[x] = _data.bean_content[j].value
                        // }
                        // delete _data["bean_content"];
            ...
            getColumnList() {
                ...
                // for (let i = 0; i < columnPermissionList.length; i++) {
                //     if (columnPermissionList[i].prop == "bean_content") {
                //         columnPermissionList.splice(i, 1);
                //     }
                // }


                // for (let i = 0; i < this.prpupFileds.length; i++) {
                //     columnPermissionList.push(JSON.parse(this.prpupFileds[i].content))
                // }
            ...
            handleNodeClick(data, node) {
                ...
                if (this.columnData.treeDataSource === "organize") {
                    ...
                    queryJson = {
                        [this.columnData.treeRelation]: currValue
                    }
                } else {
                    queryJson = {
            ...
            submitForRowEdit(row) {
                ...
                Candidates(0, {
                    formData: this.workFlowFormData
                }).then(res => {
                    let data = res.data
            ...
            addOrUpdateHandle(id) {
                ...
                if (this.config.webType == 3) {
                    ...
                    this.$nextTick(() => {
                        this.$refs.Form.init(this.formData, this.modelId, id, this.isPreview, this.columnData
                            .useFormPermission)
                    })
                }
            },
            ...
            goDetail(id, row) {
                if (this.config.webType == 3) {
                    ...
                    this.$nextTick(() => {
                        this.$refs.Detail.init(this.formData, this.modelId, id, this.columnData
                            .useFormPermission)
                    })
                }
            },
