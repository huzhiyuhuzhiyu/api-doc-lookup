...
<script>
import { getpurPurchaseReceiptReturnGoodsdetail, detailpurPurchaseReceiptReturnGoods } from '@/api/purchasingManagement/purchaseInquirySheet'  // 单据详情
import { addInspectionData, updateInspectionData, detailInspectionData, detailInspectionDataByBizid } from '@/api/inspectionManagement/index' // 产品检验项目列表
...
export default {
  ...
  data() {
    return {
      ...
      inspectionType: "",
      dataForm: {},
      dataFormItems: [/* 通过 this.setDataFormItems() 动态更改 */],
      // 流转记录
      flowCardItems: [
      ...
      linesList: [],
      linesListItems: [/* 通过 this.setLinesListItems() 动态更改 */],
      spaceLines: [],
      productionLotList: [],
  ...
  methods: {
    ...
    setDataFormItems() {
      this.dataFormItems = [
        ...
        {
          prop: "inspectorId", label: "检验人", value: undefined, type: "custom", customComponent: "user-select", itemRules: [{ required: true, trigger: 'change' }],
    ...
    setLinesListItems() {
      this.linesListItems = [
        ...
        { prop: "productSpec", label: "规格型号", value: "", type: 'view', minWidth: 140, render: ['procure', 'finished', 'sale_back', 'external'].includes(this.inspectionType) },
        { prop: "productCategoryName", label: "产品分类", value: "", type: 'view', minWidth: 140, render: ['procure', 'finished', 'sale_back', 'process', 'back_material', 'produce', 'external'].includes(this.inspectionType) },
        ...
        { // 检验结果
          prop: "inspectionResults", label: "检验结果", value: undefined, type: "select", clearable: false,
          options: [{ label: '合格', value: 'qualified' }, { label: '不合格', value: 'unqualified' }], itemRules: [{ required: true, trigger: 'change' }], width: 150,
          change: this.inspectionResultsChange, itemDisabled: (rowIndex) => this.linesList[rowIndex].inspectionMethod === 'exempt' || this.openMode === '只读'
        },
        { // 检验数量
          prop: "samplingQuantity", label: "检验数量", value: "", type: "input",
          itemRules: [
            { validator: this.formValidate({ type: 'noEmtry', params: ["", (errMsg, index) => { this.$message.error(`产品信息第${index + 1}行：检验数量${errMsg}`) }] }), trigger: 'blur' },
            { required: true, trigger: 'blur' },
            { validator: this.formValidate({ type: 'decimal', params: [20, 4, "", (errMsg, index) => { this.$message.error(`产品信息第${index + 1}行：检验数量${errMsg}`) }] }), trigger: 'blur' },
            { validator: this.formValidate('positiveNumber', false, (errMsg, index) => { this.$message.error(`产品信息第${index + 1}行：检验数量${errMsg}`) }), trigger: 'blur' },
            { validator: this.formValidate({ type: 'calc', params: [(rowIndex) => Number(this.linesList[rowIndex].samplingQuantity) <= Number(this.linesList[rowIndex].inspectionQuantity), "不能超过对应报检数量", (errMsg, index) => { this.$message.error(`产品信息第${index + 1}行：检验数量${errMsg}`) }] }), trigger: 'blur' },
          ], minWidth: 180, itemDisabled: (rowIndex) => this.linesList[rowIndex].inspectionMethod === 'all' || this.linesList[rowIndex].inspectionMethod === 'exempt' || this.openMode === '只读'
        },
        { // 不合格数量
          ...
          itemRules: [
            ...
            { validator: this.formValidate({ type: 'calc', params: [(rowIndex) => Number(this.linesList[rowIndex].unqualifiedQuantity) <= Number(this.linesList[rowIndex].samplingQuantity), "不能超过对应检验数量", (errMsg, index) => { this.$message.error(`产品信息第${index + 1}行：不合格数量${errMsg}`) }] }), trigger: 'blur' },
          ], minWidth: 180, itemDisabled: (rowIndex) => this.linesList[rowIndex].inspectionResults === 'qualified' || this.openMode === '只读'
        },
      ]
    ...
    async init(row, readOnly, inspectionType, type, businessCode) {
      ...
      if (typeof id === 'object' && inspectionType === 'process') { // 生产巡检
        ...
        return
      } else if (typeof id === 'object' && inspectionType === 'finished_batch') { // 批量完工检验
        let selectedData = id
      ...
      if (res.data) {
      ...
      } else if (inspectionType === 'produce') { // 生产退料
        ...
        let tempLinesList = [{
          ...
          routingName: rowData.routingName,
          routingLineId: rowData.routingLineId,
          inspectionQuantity: this.jnpf.math('subtract', [rowData.completedQuantity, rowData.qualifiedQuantity, rowData.underInspectionQuantity]), // 检验数量 = 生产数量 - 合格数量 - 检验中数量
          inspectionResults: '',
          returnQuantity: 0,
    ...
    linesListFormat(linesList) {
      linesList.forEach(line => {
        ...
        if (line.inspectionMethod === 'exempt') { // 免检
          line.inspectionResults = 'qualified'
          line.samplingQuantity = line.inspectionQuantity
          line.unqualifiedQuantity = 0
        } else if (line.inspectionMethod === 'all') { // 全检
          line.samplingQuantity = line.inspectionQuantity
        } else if (line.inspectionMethod === 'spot_check') { // 抽检
          if (line.spotCheckRatio && line.spotCheckRatio != '0') { // 有抽检比例自动设置检验数量
  ...
  computed: {
    ...
    openMode() {
      ...
      if (this.title.includes('生产巡检')) { return this.title === "生产巡检" ? '新建' : '只读' }
