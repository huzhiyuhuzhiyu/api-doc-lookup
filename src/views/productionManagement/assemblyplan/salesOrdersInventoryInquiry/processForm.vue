      ...
      <div class="JNPF-common-layout-center JNPF-flex-main">
        ...
        <div class="JNPF-common-layout-main JNPF-flex-main">
          <JNPF-table v-loading="listLoading" :data="tableDataList" :fixedNO="true" @selection-change="selectProcess"
            v-if="showFlag" @row-click="handleRowClick" hasC ref="processRef" customKey="JNPFTableKey_909808">
            ...
            <el-table-column prop="technicalRequirement" label="技术要求" width="120" show-overflow-tooltip
              v-if="isTechnicalSwitch === '1'">

            </el-table-column>
            <el-table-column prop="inspectionInformation" label="检验信息" width="120" show-overflow-tooltip
              v-if="isCheckingSwitch === '1'">

            </el-table-column>
            <el-table-column prop="workOrderFlag" label="是否生成工单" width="120">
              <template slot-scope="scope">
...
import { getBimProcessList, deleteBimProcess, getBimProcessDetail } from '@/api/bimProcess/index'
export default {
  data() {
    return {
  ...
  methods: {
    ...
    submitFun() {
      ...
      this.selectArr.forEach((item, index) => {
        ...
        getBimProcessDetail(item.id).then(res => {
          console.log(res);
          const groupedByResourceType = res.data.resourceList.reduce((acc, item) => {
            const key = item.resourceType; // 获取 resourceType 作为键  
            if (!acc[key]) {
              acc[key] = []; // 如果该键不存在，则初始化为一个空数组  
            }
          ...
          let obj = {
            ...
            routingProResList: res.data.resourceList,
            routingProResMap: groupedByResourceType
          }
            if (obj.routingProResMap) {
            if (obj.routingProResMap.personnel) {
              this.$set(obj, 'personId', obj.routingProResMap.personnel[0].resourceId)
              this.$set(obj, 'personName', obj.routingProResMap.personnel[0].resourceName)
            }
            if (obj.routingProResMap.work_group) {
              this.$set(obj, 'workGroupId', obj.routingProResMap.work_group[0].resourceId)
              this.$set(obj, 'workGroupName', obj.routingProResMap.work_group[0].resourceName)
            }
            if (obj.routingProResMap.device) {
              this.$set(obj, 'equipmentId', obj.routingProResMap.device[0].resourceId)
              this.$set(obj, 'equipmentName', obj.routingProResMap.device[0].resourceName)
            }
          } else {
          }
          this.selectArr[index] = obj
        })
      });
      setTimeout(() => {
    ...
    getProcessList() {
      ...
      getBimProcessList(this.form).then(res => {
        ...
        res.data.records.forEach(item => {
          ...
          this.$set(item, 'processCode', item.code)
        });
