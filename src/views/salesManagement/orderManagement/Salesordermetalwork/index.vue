<template>
  <div class="JNPF-common-layout">
          ...
          <div class="JNPF-common-layout-center JNPF-flex-main">
            <el-row class="JNPF-common-search-box" :gutter="16">
              <el-form @submit.native.prevent>
                ...
                <el-col :span="4">
                  <el-form-item>
                    <el-input v-model="orderForm.cooperativePartnerCode" @keyup.enter.native="search()" placeholder="请输入客户编码" clearable />
                  </el-form-item>
                </el-col>
                <el-col :span="4">
                  <el-form-item>
                    <el-input v-model="orderForm.cooperativePartnerName" @keyup.enter.native="search()" placeholder="请输入客户名称" clearable />
                  </el-form-item>
                </el-col>
                <el-col :span="6">
                  <el-form-item>
            ...
            <div class="JNPF-common-layout-main JNPF-flex-main">
              ...
              <JNPF-table ref="dataTable" v-loading="listLoading" :data="tableData" :fixedNO="true" @sort-change="sortChange" custom-column>
                ...
                <!-- <el-table-column prop="deliveryCompletionDate" label="发货完成时间" width="160" sortable="custom"></el-table-column> -->
                ...
                <el-table-column prop="documentStatus" fixed="right" label="单据状态" width="120" sortable="custom" :showOverflowTooltip="false" align="center">
                  <template slot-scope="scope">
                    <div v-if="scope.row.documentStatus == 'draft'"><el-tag type="warning">草稿</el-tag> </div>
                    <div v-if="scope.row.documentStatus == 'submit'"><el-tag type="success">提交</el-tag></div>
                  </template>
                </el-table-column>
                <!-- <el-table-column prop="approvalStatus" fixed="right" label="审批状态" width="120" sortable="custom" :showOverflowTooltip="false" align="center">
                  <template slot-scope="scope">
                    ...
                    <div v-if="scope.row.approvalStatus == 'rebut' && scope.row.documentStatus == 'submit'"><el-tag type="danger">审批拒绝</el-tag></div>
                  </template>
                </el-table-column> -->
              ...
              <pagination :total="total" :page.sync="orderForm.pageNum" :limit.sync="orderForm.pageSize" @pagination="initData">
                <div><span>合计数量:{{totalNum}}</span></div>
              </pagination>
            </div>
          </div>
        </el-tab-pane>
          ...
          <div class="JNPF-common-layout-center JNPF-flex-main">
            <el-row class="JNPF-common-search-box" :gutter="16">
              <el-form @submit.native.prevent>
                <el-col :span="4">
                  <el-form-item>
                    <el-input v-model="orderDetailForm.orderNo" @keydown.enter.native="searchDetail()" placeholder="请输入订单号" clearable />
                  </el-form-item>
                </el-col>
                <el-col :span="4">
                  <el-form-item>
                    <el-input v-model="orderDetailForm.cooperativePartnerCode" @keydown.enter.native="searchDetail()" placeholder="请输入客户编码" clearable />
                  </el-form-item>
                </el-col>
                <el-col :span="4">
                  <el-form-item>
                    <el-input v-model="orderDetailForm.cooperativePartnerName" @keydown.enter.native="searchDetail()" placeholder="请输入客户名称" clearable />
                  </el-form-item>
                </el-col>
                <el-col :span="6">
                  <el-form-item>
                ...
                <el-button style="float: right;margin-right: 20px;" size="mini" type="primary" icon="el-icon-search" @click="moreQueriesDetail()">更多查询</el-button>
              </el-form>
            </el-row>
            <div class="JNPF-common-layout-main JNPF-flex-main">
              <div class="JNPF-common-head">
                ...
                <div class="JNPF-common-head-right">
                  <el-tooltip effect="dark" :content="$t('common.refresh')" placement="top">
                    <el-link icon="icon-ym icon-ym-Refresh JNPF-common-head-icon" :underline="false" @click="initDataDetail()" />
                  </el-tooltip>
                </div>
              </div>

              <JNPF-table ref="detailTableData" v-loading="listLoading" :data="detailTableData" :fixedNO="true" @sort-change="sortChangeDetail" custom-column :partentOrChild="'child'">
                <el-table-column prop="orderNo" label="订单号" width="180" sortable="custom">
                  <template slot-scope="scope">
                ...
                <el-table-column prop="cooperativePartnerCode" label="客户编码" width="140" sortable="custom" />
                <el-table-column prop="cooperativePartnerName" label="客户名称" width="140" sortable="custom" />
                ...
                <el-table-column prop="orderType" label="订单类型" :showOverflowTooltip="false" width="130">
                  <template slot-scope="scope">
                    <div v-for="(item, index) in orderList" :key="index">
                      <span v-if="item.value == scope.row.orderType">{{ item.label }}</span>
                    </div>
                ...
                <el-table-column label="操作" width="180" fixed="right">
                  <template slot-scope="scope">
                    ...
                    <el-button size="mini" type="text" :disabled="scope.row.documentStatus == 'draft' ? false : true" @click.stop="addOrUpdateHandle(scope.row.ordersId, 'edit')">编辑</el-button>
                    <el-button size="mini" type="text" class="JNPF-table-delBtn" :disabled="scope.row.documentStatus == 'draft' ? false : true" @click.stop="handleDetailDel(scope.row.ordersId)">删除</el-button>
                    <el-dropdown hide-on-click>
                      <span class="el-dropdown-link">
                        <el-button type="text" size="mini">
                          {{ $t('common.moreBtn') }}<i class="el-icon-arrow-down el-icon--right"></i>
              ...
              <pagination :total="detailTotal" :page.sync="orderDetailForm.pageNum" :limit.sync="orderDetailForm.pageSize" @pagination="initDataDetail" />
        ...
        <el-form ref="diaForm" :model="orderForm" label-width="120px" label-position="top">
          ...
          <!-- <el-col :span="12">
            <el-form-item label="发货状态">
              <el-select v-model="orderForm.shipmentStatus" placeholder="请选择发货状态" clearable>
          ...
          <!-- <el-col :span="12">
            <el-form-item label="发货完成时间">
              <el-date-picker v-model="CompletionDate" type="daterange" value-format="yyyy-MM-dd" style="width: 100%;" start-placeholder="开始时间" end-placeholder="结束时间" clearable>
              </el-date-picker>
            </el-form-item>
          </el-col> -->
          <el-col :span="12">
            <el-form-item label="订单日期">
              <el-date-picker v-model="orderDateArr" type="daterange" value-format="yyyy-MM-dd" style="width: 100%;" :picker-options="pickerOptions" start-placeholder="开始日期" end-placeholder="结束日期">
              </el-date-picker>
            </el-form-item>
          ...
          <el-col :span="12">
            <el-form-item label="创建时间">
              <el-date-picker v-model="createTimeArr" type="datetimerange" format="yyyy-MM-dd hh:mm:ss" value-format="yyyy-MM-dd hh:mm:ss" style="width: 100%;" start-placeholder="开始时间" end-placeholder="结束时间" clearable>
              </el-date-picker>
            </el-form-item>
          </el-col>
    ...
    <el-dialog :title="title" :close-on-click-modal="false" :close-on-press-escape="false" :visible.sync="detailVisible" lock-scroll class="JNPF-dialog JNPF-dialog_center" width="800px">
      <el-row :gutter="20">
        ...
        <el-form ref="diaForm" :model="orderDetailForm" label-width="120px" label-position="top">
          ...
          <el-col :span="12">
            <el-form-item label="交货日期">
              <el-date-picker v-model="deliveryDateArr" type="daterange" value-format="yyyy-MM-dd" style="width: 100%;" :picker-options="pickerOptions" start-placeholder="开始日期" end-placeholder="结束日期" clearable>
              </el-date-picker>
            </el-form-item>
          </el-col>
          <el-col :span="12">
  ...
  methods: {
    ...
    getAttributeline () {
      getAttributeline('product').then(res => {
        console.log("getAttributeline res", res);
        this.customList = []
            ...
            children.forEach(resp => {
              ...
              if (resp.enCode == "paymentCycle") {
                ...
                getDictionaryDataList(id, obj).then(response => {
                  ...
                  console.log("paymentCycleList", this.paymentCycleList);
                })
              }
