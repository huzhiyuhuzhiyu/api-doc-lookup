...
export default {
  ...
  computed: {
    ...
    dialogRequestObj() {
      ...
      let warehouseIds = this.$parent.dataForm.sourceType === "outbound_transfer" ? [this.$parent.dataForm.transferOutWarehouseId] : undefined
      return { productsId, routingLineId, warehouseType: 'normal', warehouseIds }
    },
    oddOutboundFlag() { // 是否支持非整箱出库
      let flag = false
      try {
        const sourceType = this.$parent.dataForm.sourceType
        flag = sourceType === 'outbound_pick_out' || (sourceType === 'outbound_stock_up' && this.scope.row.productType !== 'pt001')
      } catch (error) { }
      return flag
    }
  },
  methods: {
    ...
    refeshLinesListItems() {
      this.linesListItems = [
        ...
        { prop: "batchNumber", label: "批次号", value: "", type: 'view', minWidth: 180 },
        {
          ...
          listMethod: lineStockBoxBarcode
        },
        { prop: "boxPackingNum", label: "装箱数量", value: 0, type: "view", minWidth: 120, render: this.openMode !== '只读' },
        { prop: "occupyAvailableQuantity", label: "占用数量", value: "", type: 'view', minWidth: 120, render: this.openMode !== '只读' },
        { prop: "availableQuantity", label: "总可用数量", value: "", type: 'view', minWidth: 120, render: this.openMode !== '只读' },
        {
          ...
          itemRules: [
            { required: true, trigger: 'blur' },
            {
              validator: this.formValidate({
                type: 'calc',
                params: [
                  (rowIndex, value) => Number(value) <= Number(this.linesList[rowIndex].boxPackingNum),
                  "出库不能超过对应装箱数量",
            ...
            },
            {
              validator: this.formValidate({
                ...
                params: [
                  (rowIndex, value) => {
                    // 如果待出库数量比占用数量多
                    if (Number(this.scope.row.num) > Number(this.linesList[rowIndex].occupyAvailableQuantity)) {
                      return Number(value) >= Number(this.linesList[rowIndex].occupyAvailableQuantity)
                    } else {
                      return Number(value) >= Number(this.scope.row.num)
                    }
                  },
                  "占用库存必须优先出库",
                  (errMsg, index) => { this.$message.error(`库位信息第${index + 1}行：${errMsg}`) }]
          ...
          ], minWidth: 120, itemDisabled: !this.oddOutboundFlag || this.openMode === '只读'
        },
    ...
    barCodeListChange(val, data, paramsObj) {
      ...
      if (!data || !data.length) {
        ...
        //   }
        // })
        // const oldLineTotalNum = this.linesList[index].boxPackingNum // 记录上一次调整条码箱后的行总数
        // this.linesList[index].boxPackingNum = totalNum

        // // 智能设置出库数量
        // if (this.$parent.dataForm.sourceType !== 'outbound_pick_out') this.linesList[index].num = totalNum
        // else {
        //   this.$nextTick(() => {
        //     if (this.nowNum == '0' && !oldLineTotalNum) {
        //     } else if (this.linesList[index].boxPackingNum > this.nowNum + oldLineTotalNum) {
        //       this.linesList[index].num = this.nowNum + oldLineTotalNum
        //     } else {
        ...
        // 智能设置出库数量
        if (!this.oddOutboundFlag) this.linesList[index].num = totalNum
        else {
          this.linesList[index].num = 0
          this.$nextTick(() => {
            if (this.nowNum == '0') { }
    ...
    autoDistribute() {
      ...
      if (this.scope.row.num != 0) {
        let requestObj = {
          ...
          documentObjId: this.$parent.dataForm.id, // 单据ID
          // occupyObjId: this.scope.row.occupyObjId, // 占用对象ID
          occupyObjIdStr: this.scope.row.occupyObjIdStr || this.scope.row.occupyObjId, // 占用对象ID
          routingLineId: this.scope.row.routingLineId // 工艺路线明细ID
        }
        let autoDistributeMethod = this.autoDistributeMethod

        if (typeof autoDistributeMethod === 'object') {
          autoDistributeMethod = this.autoDistributeMethod.method
          requestObj = {
            model: {
              documentObjId: this.$parent.dataForm.id, // 单据ID
              num: this.scope.row.num,
              // occupyObjId: 0, // 占用对象ID
              // ordersLineId: 0,
              productsId: this.scope.row.productsId,
              routingLineId: this.scope.row.routingLineId // 工艺路线明细ID
            },
            warehouseIds: [this.$parent.dataForm.transferOutWarehouseId]
          }
        }

        autoDistributeMethod(requestObj).then(res => {
          this.linesList = []
          let resList = Array.isArray(res.data) ? res.data : res.data.lines
          resList.forEach(line => {
            line.documentObjId ? line.availableQuantity = this.jnpf.math('add', [line.availableQuantity, line.occupyAvailableQuantity]) : ''
          })

          // 由于占用和非占用，需要合并同库位同批次数据，并记录占用数量
          resList = resList.reduce((result, obj) => {
            let existingObj = result.find(item => item.shelfSpaceId === obj.shelfSpaceId && item.batchNumber === obj.batchNumber);
            if (existingObj) {
              ...
              if (obj.occupyAvailableQuantity) {
          ...
          this.addOrDelLinesItem(resList, 'auto')
          this.loading = false
        }).catch(err => {
          this.loading = false
        })
    ...
    async saveSettings() {
      ...
      if (submitFlag) {
        ...
        if (flag) {
          ...
          effective = effective.filter(line => line.boxList.length)
    ...
    addOrDelLinesItem(data, addType) {
      ...
      if (type === 'Object') { this.linesList.splice(data.$index, 1) }
        ...
        if (addType === 'auto') { tempList = data ? data.map(item => { return { ...item, id: '', num: 0, boxList: [] } }) : data }
        else {
          for (let i = 0; i < data.length; i++) {
            ...
            item.id = ""
            item.boxList = []
            const hasFlag = this.linesList.some(i => item.shelfSpaceId === i.shelfSpaceId && item.batchNumber === i.batchNumber)
            if (hasFlag) { hasItemList.push(`${item.shelfSpaceName} - ${item.batchNumber}`) }
