      ...
      <div class="JNPF-preview-main org-form">
        <div :class="['JNPF-common-page-header', type === 'look' ? 'noButtons' : '']">
          <el-page-header @back="goBack"
            :content="type == 'add' ? '新建工序外协收货通知单' : type == 'edit' ? '编辑工序外协收货通知单' : type == 'look' ? '查看工序外协收货通知单' : '新建工序外协收货通知单'" />
            ...
            <el-tab-pane label="基础信息" name="jcInfo">
              ...
              <el-row :gutter="15" class="">
                <el-form ref="elForm" :model="dataForm" :rules="rules" size="small" label-width="100px"
                  ...
                  <el-col :sm="8" :xs="24">
                    <el-form-item label="收货日期" prop="deliverDate" ref="deliverDate">
                      <el-date-picker v-model="dataForm.deliverDate" type="date" value-format="yyyy-MM-dd"
                        style="width: 100%;" placeholder="请选择收货日期" :disabled="type == 'look' ? true : false">
                      </el-date-picker>
                    </el-form-item>
                  </el-col>
                  ...
                  <el-col :sm="8" :xs="24" v-if="type === 'look'">
                    <el-form-item label="收货完成时间" prop="deliveryCompletionDate" ref="deliveryCompletionDate">
                      <el-time-select style="width: 100%;" :disabled="type == 'look' ? true : false"
                        v-model="dataForm.deliveryCompletionDate" placeholder="收货完成时间">
                      </el-time-select>
                    </el-form-item>
                  </el-col>
                  <el-col :span="8" v-if="type === 'look'">
                  ...
                  <!-- <el-col :span="12" v-if="type === 'look'">
                    <el-form-item label="驳回理由" prop="reasonRejection" ref="reasonRejection">
                      <el-input type="textarea" :row="3" v-model="dataForm.reasonRejection" placeholder="请输入驳回理由"
              ...
              <el-form :model="dataFormTwo" v-bind="dataFormTwo" ref="productForm">
                <el-table style="border: 1px solid #e3e7ee;" hasNO fixedNO v-bind="dataFormTwo.data"
                  ...
                  <el-table-column prop="productCode" label="产品编码" min-width="200" key="productCode" fixed="left" show-overflow-tooltip>
                  ...
                  <el-table-column prop="productName" label="产品名称" min-width="200" key="productName" fixed="left" show-overflow-tooltip>
                    <template slot-scope="scope">
                  ...
                  <el-table-column prop="productDrawingNo" label="产品图号" min-width="200" key="productDrawingNo" fixed="left"
                  ...
                  <el-table-column prop="routingIdName" label="工艺路线名称" min-width="200" key="routingIdName">
                    <template slot-scope="scope">
                      <el-form-item :prop="'data.' + scope.$index + '.' + 'routingIdName'">

                        <div class="viewData">
                          <span>{{ scope.row.routingIdName }}</span>
                        </div>
                      </el-form-item>
                    </template>
                  </el-table-column>
                  <el-table-column prop="processIdName" label="工序名称" min-width="200" key="processIdName">
                    <template slot-scope="scope">
                      <el-form-item :prop="'data.' + scope.$index + '.' + 'processIdName'">

                        <div class="viewData">
                          <span>{{ scope.row.processIdName }}</span>
                        </div>
                      </el-form-item>
                    </template>
                  </el-table-column>
                  ...
                  <el-table-column prop="mainUnit" label="单位(主)" min-width="120" show-overflow-tooltip key="mainUnit">
                    <template slot-scope="scope">
                  ...
                  <el-table-column prop="purchaseQuantity" label="外协订单数量" min-width="200" key="purchaseQuantity">
                  ...
                  <el-table-column prop="receiptQuantity" label="已入库数量" min-width="180" key="receiptQuantity">
                    <template slot-scope="scope">
                  ...
                  <el-table-column prop="requiredReceivedQuantity" label="待入库数量" min-width="180" key="requiredReceivedQuantity" v-if="type !== 'look'">
                    <template slot-scope="scope">
                      <el-form-item prop="requiredReceivedQuantity">
                        <div class="viewData">
                          <span>{{ scope.row.requiredReceivedQuantity ? scope.row.requiredReceivedQuantity : 0 }}</span>
                        </div>
                      </el-form-item>
                    </template>
                  </el-table-column>
                  <el-table-column prop="receivedQuantity" label="收货数量" min-width="180" key="receivedQuantity">
                    <template slot="header">
                  ...
                  <el-table-column prop="qualifiedQuantity" label="合格数量" min-width="120" key="qualifiedQuantity" v-if="type === 'look'" />
                  <el-table-column prop="unqualifiedQuantity" label="不合格数量" min-width="120" key="unqualifiedQuantity" v-if="type === 'look'" />
...
export default {
  ...
  data() {
    return {
      ...
      dataForm: {
        ...
        notificationType: "external_process",             //	通知单类型 外协通知 external、销售通知 sale,可用值:external,sale           
        purchaseOrderId: '',              // 采购单id
        receiptReturnType: "receipt",            //类型（收货 receipt、退货 return）
      ...
      rules: {
        ...
        deliverDate: [{ required: true, message: '请选择收货日期', trigger: ['change'] }],
      },
      productRules: {
        // warehouse: [{ required: true, trigger: ['change'] },],
        receivedQuantity: [
          { required: true, trigger: ['blur'] },
          { validator: this.formValidate({ type: 'decimal', params: [20, 4, "", (errMsg) => { this.$message.error('收货数量：' + errMsg) }] }), trigger: 'blur' },
          { validator: this.formValidate('noZero', '', (errMsg) => { this.$message.error('收货数量：不能为0') }), trigger: 'blur' }
  ...
  methods: {
    ...
    //     let msg = `当前第${index * 1 + 1}行：收货数量不能超过采购数量`
          ...
          getpurPurchaseReceiptReturnGoodsdetail(this.dataForm.id).then(res => {
            ...
            if (this.type === 'copy') {
              ...
              purPurchaseOrderdetail(this.dataForm.purchaseOrderId).then(res => {
                ...
                linesData = res.data.purchaseOrderLineVOList
                detailpurPurchaseReceiptReturnGoods(this.dataForm.id).then(res => {
                  ...
                  for (let i = 0; i < linesData.length; i++) {
                    for (let j = 0; j < _data.length; j++) {
                      if (linesData[i].id === _data[j].ordersLineId && linesData[i].receivingStatus === "receiving") {
                        resultArray.push(_data[j]);
                      }
                    }
                  }
                  linesData.forEach(item => {
                    if (resultArray.length) {
                      resultArray.forEach(item2 => {
                        item2.inspectionResults = ""
                        item2.billStatus = 'no_billing'
                        item2.receivingStatus = 'receiving'
                        if (item.id === item2.ordersLineId) {
                          item2.purchaseQuantity = item.purchaseQuantity ? item.purchaseQuantity : 0
                          item2.receiptQuantity = item.receiptQuantity ? item.receiptQuantity : 0
                          item2.receivedQuantity = Number(item.purchaseQuantity - item.receiptQuantity - item.requiredReceivedQuantity) > 0 ? item.purchaseQuantity - item.receiptQuantity - item.requiredReceivedQuantity : 0
                          item2.requiredReceivedQuantity = item.requiredReceivedQuantity ? item.requiredReceivedQuantity : 0
                          item2.id = ''
                          item.routingIdName = item.lastFlag ? '' : item.routingIdName
                          item.processIdName = item.lastFlag ? '' : item.processIdName
                          item2.routingId = item2.lastFlag ? '' : item2.routingId
                          item2.processId = item2.lastFlag ? '' : item2.processId
                          item2.routingLineId = item2.lastFlag ? '' : item2.routingLineId
                        }
                      })
                    }
                  })

                  this.dataFormTwo.data = resultArray
              ...
              detailpurPurchaseReceiptReturnGoods(this.dataForm.id).then(res => {
                ...
                this.dataFormTwo.data.forEach(item => {
                  item.routingIdName = item.lastFlag ? '' : item.routingIdName
                  item.processIdName = item.lastFlag ? '' : item.processIdName
                  item.routingId = item.lastFlag ? '' : item.routingId
                  item.processId = item.lastFlag ? '' : item.processId
                  item.routingLineId = item.lastFlag ? '' : item.routingLineId
                })
              })
            }
          })
    ...
    async request(type) {
      ...
      if (!this.dataFormTwo.data.length) {
        ...
        this.$message.error('至少有一条收货明细')
        return
      }
      if (this.dataFormTwo.data.length) {
        let stopedFlag = this.dataFormTwo.data.every(item => item.receivingStatus === 'receiving')
        if (!stopedFlag) {
          hasCostPrice = false
          this.btnLoading = false
          this.$message.error('已停止或已完成的订单产品，不允许再次收货！')
          return
        }
      }
      ...
      if (this.type === 'add' || this.type === 'copy' || this.type === 'edit') {
        _dataFormTwo = this.dataFormTwo.data.map(item => {
          return {
            ...item,
            receiptQuantity: 0,
          }
      ...
      purPurchaseOrderdetail(this.dataForm.purchaseOrderId).then(res => {
        ...
        this.dataFormTwo.data = this.dataFormTwo.data.map(item => {
          return {
            ...
            routingIdName: item.lastFlag ? '' : item.routingName,
            processIdName: item.lastFlag ? '' : item.processName,
            routingId: item.lastFlag ? '' : item.routingId,
            processId: item.lastFlag ? '' : item.processId,
            routingLineId: item.lastFlag ? '' : item.routingLineId,
            receivedQuantity: Number(item.purchaseQuantity - item.receiptQuantity - item.requiredReceivedQuantity) > 0 ? item.purchaseQuantity - item.receiptQuantity - item.requiredReceivedQuantity : 0,
            requiredReceivedQuantity: item.requiredReceivedQuantity ? item.requiredReceivedQuantity : 0,
          }
        })
        this.dataFormTwo.data = this.dataFormTwo.data.filter(item => item.receivingStatus == 'receiving')
      })
    },
