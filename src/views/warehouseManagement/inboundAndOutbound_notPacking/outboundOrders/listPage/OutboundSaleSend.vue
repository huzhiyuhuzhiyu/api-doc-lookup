                  ...
                  <template slot-scope="scope">
                    <tableOpts @edit="addOrUpdateHandle(scope.row.id)"
                      ...
                      <el-button slot="left" type="text" size="mini"
                        @click="addOrUpdateHandle(scope.row.stockMoveId, true, 'stock_look')" v-else>查看备货</el-button>
...
import OutboundOrdersForm from '@/views/warehouseManagement/inboundAndOutbound/outboundOrders/Form.vue'
import DetailForm from '../../../inboundAndOutbound/outboundOrders/detailForm/OutboundSaleSend.vue'
export default {
  components: { Form, DetailForm, OutboundOrdersForm },
  props: {
    originTypeList: {
      type: Array,
      default: () => []
  ...
  methods: {
    ...
    addOrUpdateHandle(id, readOnly, type) {
      if (readOnly && !type) {
        ...
        this.$nextTick(() => { this.$refs.OutboundOrdersForm.init(id, readOnly, 'outbound_stock_up', id) })
      } else {
