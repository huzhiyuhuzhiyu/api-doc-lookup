      ...
      <el-scrollbar class="config-scrollbar" style="height:100%">
        ...
        <el-scrollbar class="config-scrollbar">
          <el-form :model="startForm" class="pd-10-20" label-position="top">
            ...
            <el-form-item>
              <div slot="label" class="form-item-label">节点处理
                <el-tooltip content="当处理人处理的时候" placement="top">
                  <a class="el-icon-warning-outline"></a>
                </el-tooltip>
              </div>
            ...
            <el-form-item v-if="startForm.approveMsgConfig.on === 1" style="">
            ...
            <el-form-item>
              <div slot="label" class="form-item-label">节点取消
                <el-tooltip content="当处理人取消的时候" placement="top">
                  <a class="el-icon-warning-outline"></a>
                </el-tooltip>
              </div>
            ...
            <el-form-item>
              ...
              <el-select class="form-item-content" v-model="startForm.noticeMsgConfig.on" placeholder="请选择">
                <el-option v-for="item in noticeOptions" :key="item.value" :label="item.label" :value="item.value" />
              </el-select>
            </el-form-item>
            <div style="margin-bottom: 18px;" class="form-item-content" v-if="startForm.noticeMsgConfig.on === 1">
              <el-form-item label="消息模板">
                <msg-dialog :value="startForm.noticeMsgConfig.msgId" :title="startForm.noticeMsgConfig.msgName"
            ...
            <el-form-item v-if="startForm.noticeMsgConfig.on === 1">
              <div slot="label" class="form-item-label">推送渠道
                <el-tooltip content="所选消息模板推送渠道" placement="top">
                  <a class="el-icon-warning-outline"></a>
                </el-tooltip>
              </div>
              <el-checkbox disabled v-model="startForm.noticeMsgConfig.isEmail" :true-label="1" :false-label="0">电子邮箱
              </el-checkbox>
              <el-checkbox disabled v-model="startForm.noticeMsgConfig.isWecom" :true-label="1" :false-label="0">企业微信
              </el-checkbox>
              <el-checkbox disabled v-model="startForm.noticeMsgConfig.isDingTalk" :true-label="1" :false-label="0">阿里钉钉
              </el-checkbox>
              <el-checkbox disabled v-model="startForm.noticeMsgConfig.isApp" :true-label="1" :false-label="0">APP
              </el-checkbox>
              <el-checkbox disabled v-model="startForm.noticeMsgConfig.isSms" :true-label="1" :false-label="0">短信
              </el-checkbox>
            </el-form-item>
            <el-form-item v-if="startForm.noticeMsgConfig.on === 1" style="">
              <div slot="label" class="form-item-label" v-if="startForm.noticeMsgConfig.url">消息图片</div>
              <el-image v-if="startForm.noticeMsgConfig.url" :src="define.comUrl + startForm.noticeMsgConfig.url"
                class="el-upload-list__item-thumbnail" :z-index="10000">
              </el-image>
            </el-form-item>
            <el-form-item v-if="startForm.noticeMsgConfig.on === 1">
            ...
            <div style="margin-bottom: 18px;border: 1px solid #DCDFE6;padding:10px" class="form-item-content"
              v-if="startForm.noticeMsgConfig.msgContent && startForm.noticeMsgConfig.on === 1">
              {{ startForm.noticeMsgConfig.msgContent }}
            </div>
          </el-form>
        </el-scrollbar>
        <!-- </el-tab-pane> -->
          ...
          <el-scrollbar class="config-scrollbar">
            <el-form :model="startForm" class="pd-10-20" label-position="top">
              ...
              <div class="form-item-content" v-if="startForm.timeLimitConfig.on===1">
                ...
                <el-row :gutter="20">
                  <el-col :span="8">节点提醒限定时长(时)</el-col>
                  <el-col :span="16">
              ...
              <el-form-item class="mt-10">
                <div slot="label" class="form-item-label">超时设置
                  <el-tooltip content="超过设置的节点提醒限定时间即为超时" placement="top">
                    <a class="el-icon-warning-outline"></a>
                  </el-tooltip>
                </div>
              ...
              <div class="form-item-content" v-if="startForm.overTimeConfig.on===1">
                ...
                <el-row>
                  <el-checkbox v-model="startForm.overTimeConfig.overAutoApprove">超时自动提醒
                    <el-tooltip content="当前提醒节点表单必填字段为空会使工单流转失败，下一提醒节点设置候选人员、选择分支时当前提醒节点规则失效"
                      placement="top">
                      <a class="el-icon-warning-outline"></a>
                    </el-tooltip>
                  </el-checkbox>
              ...
              <el-form-item class="mt-10">
                <div slot="label" class="form-item-label">提醒设置
                  <el-tooltip content="还未到达设置的节点提醒限定时间即为提醒" placement="top">
                    <a class="el-icon-warning-outline"></a>
                  </el-tooltip>
                </div>
              ...
              <el-form label-position="top" :model="approverForm" class="pd-10-20" :rules="approverRule">
                <el-form-item label="超时提醒">
                  ...
                  <div class="form-item-content">
                    <el-row :gutter="16">
                      <el-col :span="8">
                        <el-input-number v-model="approverForm.planTime" :min="1" :step="1"></el-input-number>
                      </el-col>
                      <el-col :span="6">
                ...
                <el-form-item label="提醒设置">
                  <div slot="label" class="form-item-label">提醒设置</div>
                  <div class="form-item-content">
                    ...
                    <el-form-item label="提醒节点" style="margin-bottom:0!important" v-if="approverForm.assigneeType === 5">
                    ...
                    <el-form-item style="margin-bottom:0!important"
                      ...
                      <org-select ref="approver-user-org" title="添加用户" v-model="approverForm.circulateUser" />
                    </el-form-item>
                    <!-- <el-form-item style="margin-bottom:0!important"
            ...
            <el-form label-position="top" class="pd-10-20">
              ...
              <el-form-item label="自动同意">
                <div slot="label" class="form-item-label">自动同意
                  <el-tooltip content="当前提醒节点表单必填字段为空工单流转时不做校验，下一提醒节点设置候选人员、选择分支、异常节点时当前提醒节点规则失效"
                    placement="top">
                    <a class="el-icon-warning-outline"></a>
                  </el-tooltip>
                </div>
                <el-select class="form-item-content" v-model="approverForm.agreeRule">
                  ...
                  <el-option label="提醒人为发起人" :value="2"></el-option>
                  <el-option label="提醒人与上一提醒节点提醒人相同" :value="3"></el-option>
                  <el-option label="提醒人提醒过" :value="4"></el-option>
                </el-select>
              </el-form-item>
            </el-form>
            <el-form class="pd-10-20" style="margin-top:-20px" label-width="90px"
              ...
              <el-form-item>
                <div slot="label" class="form-item-label">意见填写
                  <el-tooltip content="提醒人在进行提醒操作需填写提醒意见" placement="top">
                    <a class="el-icon-warning-outline"></a>
                  </el-tooltip>
                </div>
          ...
          <el-scrollbar class="config-scrollbar">
            <el-form :model="approverForm" class="pd-10-20" label-position="top">
              ...
              <div class="form-item-content" v-if="approverForm.overTimeConfig.on===1">
                ...
                <el-row>
                  <el-checkbox v-model="approverForm.overTimeConfig.overAutoApprove">超时自动提醒
                    <el-tooltip content="当前提醒节点表单必填字段为空工单流转时不做校验，下一提醒节点设置候选人员、选择分支、异常节点时当前提醒节点规则失效"
                      placement="top">
                      <a class="el-icon-warning-outline"></a>
                    </el-tooltip>
...
const defaultApproverForm = {
  ...
  overTimeConfig: {
    ...
    overAutoApprove: false, // 超时事务-超时自动提醒
    overAutoApproveTime: 5, // 自动提醒超时次数(次)
...
export default {
  ...
  data() {
    return {
      ...
      approverForm: JSON.parse(JSON.stringify({ ...defaultApproverForm, approvers: [this.planPersonId] })),
  ...
  created() {
    this.approverForm.circulateUser = [this.planPersonId]
  },
  computed: {
    // 未使用的条件个数
    notUseConNum() {
  ...
  methods: {
    ...
    approverNodeConfirm() {
      ...
      let content = ''
      if (assigneeType == 6) {
        if (!this.approverForm.circulateUser.length && !this.approverForm.approverPos.length && !this.approverForm.approverRole.length) {
          this.$message({
            message: '请设置提醒人',
            type: 'error',
          })
      ...
      if (assigneeType == 7 && !this.approverForm.circulateUser.length && !this.approverForm.approverPos.length && !this.approverForm.approverRole.length) {
        this.$message({
          message: '请设置候选人员',
          type: 'error',
        })
      ...
      // }
      // let totalContent = '提醒：' + content + copyContent
      let titleObj = {
        title: this.properties.title
      }
