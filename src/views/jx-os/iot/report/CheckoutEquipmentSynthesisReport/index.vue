<template>
        ...
        <div class="JNPF-common-layout-center">
            ...
            <div class="JNPF-common-layout-main JNPF-flex-main">
                ...
                <JNPF-table v-loading="listLoading" :data="list" height="100%" @sort-change="sortChange"
                    ...
                    <el-table-column prop="" label="检验总数" align="left" min-width="120">
                        <template slot-scope="{row}">
                            {{row.countOK.max }}
                        </template>
                    </el-table-column>
                    <el-table-column prop="countOK.max" label="合格数量" align="left" min-width="120" />
                    <el-table-column prop="countNG1.name" label="不良类型" align="left" min-width="100">
                    </el-table-column>
                    <el-table-column prop="countNG1.max" label="不良数量" align="left" min-width="120" />
                    <el-table-column prop="countNG2.name" label="不良类型" align="left" min-width="120" />
                    <el-table-column prop="countNG2.max" label="不良数量" align="left" min-width="120" />
                    <el-table-column prop="countNG3.name" label="不良类型" align="left" min-width="120" />
                    <el-table-column prop="countNG3.max" label="不良数量" align="left" min-width="120">
                        <template slot-scope="{row}">
                            <span> {{ row.countNG3.name != undefined?row.countNG3.max:'' }}</span>
                        </template>
                    </el-table-column>
                    <el-table-column prop="countNG4.name" label="不良类型" align="left" min-width="120" />
                    <el-table-column prop="countNG4.max" label="不良数量" align="left" min-width="120">
                        <template slot-scope="{row}">
                            <span> {{ row.countNG4.name != undefined?row.countNG4.max:'' }}</span>
                        </template>
                    </el-table-column>

                    <!-- <el-table-column prop="sbfh" label="设备负荷" align="center">
                        <el-table-column prop="stateTime.onlineDate" label="开机时间" align="left" min-width="120"/>
                        <el-table-column prop="stateTime.offlineDate" label="关机时间" align="left" min-width="120"/>
                        <el-table-column prop="fhl" label="负荷率(%)" align="left" min-width="100"/>
                    </el-table-column>

                    <el-table-column prop="equipmentWorkpieceRatio" label="稼动率" align="center">
                        <el-table-column prop="timeRate" label="时间稼动率(%)" align="center" min-width="130">
                            <template slot-scope="{row}">
                                {{ row.equipmentWorkpieceRatio.timeRate }}%
                            </template>
                        </el-table-column>
                        <el-table-column prop="performanceRate" label="性能稼动率(%)" align="center" min-width="130">
                            <template slot-scope="{row}">
                                {{ row.equipmentWorkpieceRatio.performanceRate }}%
                            </template>
                        </el-table-column>
                        <el-table-column prop="realCropRate" label="实质稼动率(%)" align="center" min-width="130">
                            <template slot-scope="{row}">
                                {{ row.equipmentWorkpieceRatio.realCropRate }}%
                            </template>
                        </el-table-column>
                        <el-table-column prop="velocityRate" label="速度稼动率(%)" align="center" min-width="130">
                            <template slot-scope="{row}">
                    ...
                    <el-table-column prop="cl" label="产量" align="center">
                        ...
                        <el-table-column prop="thisWeekOutput.sum" label="本周产量" align="left" min-width="100"/>
                        <el-table-column prop="thisMonthOutput.sum" label="本月产量" align="left" min-width="100"/>
                        <el-table-column prop="totalOutput.max" label="累计产量" align="left" min-width="100"/>
 ...
 -->
        ...
        <BiaoZhun v-if="formVisible" ref="JNPFForm" :objectOptions="objectOptions" @refresh="refresh"
            @setNormList="setNormList"></BiaoZhun>
...
<script>
    import {
        getProductionEquipmentSynthesisReport,
        getProductionCheckoutEquipmentSynthesisReport
    } from '@/api/report'
    ...
    export default {
        ...
        methods: {
                        ...
                        for (var j = 0; j < normList.length; j++) {
                            ...
                            if (_norm.object == column.property) {
                                ...
                                switch (propertyNames.length) {
                                    case 1:
                                        // console.log(row.[propertyNames[0]]);
                                        backgroundcolor = this.calculateValue(_norm, row[propertyNames[0]])
                                        break
                                    case 2:
                                        //console.log(row.[propertyNames[0]].[propertyNames[1]]);
                                        backgroundcolor = this.calculateValue(_norm, row[propertyNames[0]][
                                            propertyNames[1]
                                        ])
                                        break
                                    ...
                                    case 4:
                                        ...
                                        backgroundcolor = this.calculateValue(_norm, row[propertyNames[0]][
                                            propertyNames[1]
                                        ][propertyNames[2]][propertyNames[3]])
                                        break
            ...
            initData() {
                ...
                console.log(_query)
                getProductionCheckoutEquipmentSynthesisReport(_query).then(res => {
                    var _list = []
                    for (let i = 0; i < res.data.list.length; i++) {
                        let _data = res.data.list[i]

                        if (_data.result.mapping.mappingLines.length > 0) {
                            for (var j = 0; j < _data.result.mapping.mappingLines.length; j++) {
                                let _mapping = _data.result.mapping.mappingLines[j];
                                if (_mapping.dictionary.code == _data.countNG1.code) {
                                    _data.countNG1.name = _mapping.name;
                                } else
                                if (_mapping.dictionary.code == _data.countNG2.code) {
                                    _data.countNG2.name = _mapping.name;
                                } else
                                if (_mapping.dictionary.code == _data.countNG3.code) {
                                    _data.countNG3.name = _mapping.name;
                                } else
                                if (_mapping.dictionary.code == _data.countNG4.code) {
                                    _data.countNG4.name = _mapping.name;
                                }
                ...
                // request({url: `/api/equipment/Equipment/getList`,method: 'post',data: _query}).then(res => {
                //   var _list = [];
                //   for (let i = 0; i < res.data.list.length; i++) {
                //     let _data = res.data.list[i];
                //     _list.push(_data)
