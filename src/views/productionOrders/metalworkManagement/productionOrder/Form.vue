    ...
    <div class="JNPF-preview-main org-form">
      ...
      <div class="main" v-loading="formLoading">
        ...
        <el-tabs v-model="activeName" @tab-click="handleClick">
          ...
          <el-tab-pane label="进度跟踪" name="progress" v-if="btnType === 'look'">
            <el-row class="JNPF-common-search-box" :gutter="16">
              <el-form @submit.native.prevent>
                <el-col :span="4">
                  <el-form-item>
                    <el-input v-model="progressForm.workNo" @keydown.enter.native="getProgressFun()" placeholder="请输入工单号"
                      clearable />
                  </el-form-item>
                </el-col>
                <el-col :span="4">
                  <el-form-item>
                    <el-input v-model="progressForm.processCode" @keydown.enter.native="getProgressFun()"
                      placeholder="请输入工序编码" clearable />
                  </el-form-item>
                </el-col>
                <el-col :span="4">
                  <el-form-item>
                    <el-input v-model="progressForm.processName" @keydown.enter.native="getProgressFun()"
                      placeholder="请输入工序名称" clearable />
                  </el-form-item>
                </el-col>
                <el-col :span="6">
                  <el-form-item>
                    <el-button type="primary" size="mini" icon="el-icon-search" @click="getProgressFun()">
                      {{ $t('common.search') }}</el-button>
                    <el-button size="mini" icon="el-icon-refresh-right" @click="reset()">{{ $t('common.reset') }}
                    </el-button>
                    <el-button type="text" icon="el-icon-download" @click="exportForm('progressForm')">导出</el-button>
                  </el-form-item>
                </el-col>

              </el-form>
            </el-row>
            <JNPF-table ref="progressForms" v-loading="listLoading" :data="progressList" :fixedNO="true" >
              <el-table-column prop="orderNo" label="工单号" width="180"> </el-table-column>
              ...
              <el-table-column prop="workOrderQuantity" label="任务生产数量" width="160" />
              <el-table-column prop="dispatchedQuantity" label="已派工/外协数量" width="160"></el-table-column>
              <el-table-column prop="firstInspectionStatus" label="首检是否通过" width="160">
                <template slot-scope="scope">
                  <div v-if="scope.row.firstInspectionStatus == '0'">否</div>
                  <div v-if="scope.row.firstInspectionStatus == '1'">是</div>
                </template>

              </el-table-column>
              <el-table-column prop="completedQuantity" label="已完成数量" width="160"></el-table-column>
              <el-table-column prop="industrialWasteQuantity" label="工废数量" min-width="130" />
              <el-table-column prop="schedule" label="生产进度" width="160">              

                <template slot-scope="scope">
                    <el-progress 
                       :percentage="Number(((scope.row.completedQuantity*1+scope.row.industrialWasteQuantity*1) / (scope.row.workOrderQuantity) * 100).toFixed(2))"></el-progress>
                  </template>
              </el-table-column>
              <el-table-column prop="stockFlag" label="是否入库" width="160">
                <template slot-scope="scope">
                  <div v-if="scope.row.stockFlag == '0'">否</div>
                  <div v-if="scope.row.stockFlag == '1'">是</div>
                </template>
              </el-table-column>
              <el-table-column prop="packingQuantity" label="已装箱数量" width="160"></el-table-column>
              <el-table-column prop="receivedQuantity" label="已入库数量" width="160">

              </el-table-column>
              <!-- <el-table-column prop="planStartDate" label="计划开始日期" width="160" />
              <el-table-column prop="planEndDate" label="计划结束日期" width="160" /> -->
              <el-table-column prop="actualStartDate" label="实际开始日期" width="160">

              </el-table-column>
              <el-table-column prop="actualEndDate" label="实际结束日期" width="160"></el-table-column>


            </JNPF-table>
            <pagination :total="total" :page.sync="progressForm.pageNum" :limit.sync="progressForm.pageSize"
              @pagination="getProgressFun">
            </pagination>
          </el-tab-pane>
        </el-tabs>
      </div>
