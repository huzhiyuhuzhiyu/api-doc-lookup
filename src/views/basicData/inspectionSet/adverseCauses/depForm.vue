<template>
  <el-dialog :title="!dataForm.id ? '新建不良原因' :'编辑不良原因'" :close-on-click-modal="false" :close-on-press-escape="false"
...
<script>
import { addAdverseCausesData,getAdverseCausesInfo,editAdverseCausesData,checkadverseCausesCode } from "@/api/basicData/index";
        ...
        code: [
          ...
          { validator: formValidate('enCode'), trigger: 'blur' },
          {
            validator: (rule, value, callback) => {
              ...
              checkadverseCausesCode(value, this.dataForm.id).then(res => {
                console.log('res===>', res);
                if (res.data) {
