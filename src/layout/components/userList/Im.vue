<template>
    <el-dialog :title="info.realName+'/'+info.account" class="JNPF-dialog_center dialog1" v-drag-dialog :visible.sync="visible" :append-to-body="true"
    ...
    :close-on-click-modal ="false">
        ...
        <div class="im-container" v-if="visible">
            <!-- @mousemove="move" @mousedown="oMove" @mouseup="cMove('up')" @mouseout="cMove('out')" @mouseleave="cMove('leave')" -->
...
export default {
  ...
  methods: {
    oMove(e) {
        ...
        this.clientX_start = e.clientX;
        this.clientY_start = e.clientY;

        this.dom = document.getElementsByClassName("header")[0].parentElement
        this.domX_start = window.getComputedStyle(this.dom,null).getPropertyValue("right").replace("px",'') * 1
        this.domY_start = window.getComputedStyle(this.dom,null).getPropertyValue("bottom").replace("px","") * 1

        this.ismove = true


        console.log(this.clientX_start,this.clientY_start,this.domX_start,this.domY_start );
    ...
    move(e){
        if ( this.ismove) {
            // console.log(this.clientX_start - e.clientX + this.domX_start);
            var right = this.clientX_start - e.clientX + this.domX_start;

            var bottom = this.clientY_start - e.clientY + this.domY_start;
            ...
            if (right > this.screenWidth - 610) {
                right = this.screenWidth - 610
            }

            if (bottom > this.screenHeight - 540) {
                bottom = this.screenHeight - 540
            }
...
<style>
    .dialog1>.el-dialog{width: 600px;}
    .dialog1>.el-dialog>.el-dialog__header{padding: 10px;}
    .dialog1>.el-dialog>.el-dialog__body{padding: 0;}
    .dialog1 {
