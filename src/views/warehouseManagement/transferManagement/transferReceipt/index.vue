              ...
              <JNPF-table v-loading="listLoading" ref="dataTable" :data="tableData" @sort-change="sortChange"
                ...
                <el-table-column prop="receiveStatus" label="接收状态" width="120" sortable="custom" align="center"
                  ...
                  <template slot-scope="scope">
                    <el-tag type="warning" v-if="scope.row.receiveStatus == 'not_received'">待接收</el-tag>
                    <el-tag type="success" v-else-if="scope.row.receiveStatus == 'received'">已接收</el-tag>
                  </template>
                </el-table-column>
                <el-table-column label="操作" width="140" fixed="right">
                  <template slot-scope="scope">
                    <tableOpts @edit="addOrUpdateHandle(scope.row.id, false)" :editText="'接收'"
                      :editDisabled="scope.row.receiveStatus === 'received' || scope.row.canReceive == false"
                      :hasDel="false">
          ...
          <div class="JNPF-common-layout-center JNPF-flex-main">
            ...
            <div class="JNPF-common-layout-main JNPF-flex-main">
              <JNPF-table v-loading="listLoading" ref="linesDataTable" :data="linesTableData" @sort-change="sortChange"
                ...
                <el-table-column label="操作" width="140" fixed="right">
                  <template slot-scope="scope">
                    <tableOpts @edit="addOrUpdateHandle(scope.row.pickingId, false)" :editText="'接收'"
                      :editDisabled="scope.row.receiveStatus === 'received' || scope.row.canReceive == false"
                      :hasDel="false">
                      <el-dropdown hide-on-click>
                        <span class="el-dropdown-link">
