...
export default {
      ...
      systemSearchView: [{
        ...
        conditionJson: { // 视图内容配置*
          condition: [ // 视图查询条件（自动根据绑定表格的列顺序排序）
            ...
            { prop: 'sourceNo', symbol: 'like', fixed: true },
            {
              prop: 'orderDate', // 属性*
  ...
  methods: {
    ...
    handleButtonClick(type) {
      switch ( type ) {
        case 'splitCard':
          ...
          this.processCardFormData = this.selectedRow[0]
          this.processCardFormVisible = true
          break;
        default:
      }
    },
    ...
    close(isInitData = true) {
      this.processCardFormVisible = false
...
<template>
  <div class="JNPF-common-layout">
    ...
    <processCardForm :visible.sync="processCardFormVisible" :formData="processCardFormData" @initData="initData"/>
  </div>
</template>
