...
export default {
  ...
  methods: {
    ...
    init(id, btnType) {
      ...
      this.refreach()
      if (this.dataForm.id) {
        this.title = btnType === 'look' ? '查看变更流程卡' : '编辑变更流程卡'
        // 获取详情
        detailFinishChange(this.dataForm.id).then(res => {
          this.formLoading = false
          console.log(res);
          this.dataForm = res.data.change
          this.dataForm.productionOrderNo = res.data.change.orderNo
        })
  ...
  computed: {
    openMode() {
      return this.title === '新建变更流程卡' ? '新建' : this.title === '编辑变更流程卡' ? '编辑' : '只读'
    }
  },
}
</script>
