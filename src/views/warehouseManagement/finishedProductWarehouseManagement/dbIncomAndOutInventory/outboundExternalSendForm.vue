<template>
  <!-- 外协发料 通知单  outbound_external_send--> 
                        ...
                        <el-row :gutter="30" class="custom-row">
                          ...
                          <el-col :sm="6" :xs="24">
                            <el-form-item label="仓库" prop="warehouseName">
                              <ComSelect-list :requestObj="{ type: 'normal',   state: 'enable', projectId: isProjectSwitch === '1' ? userInfo.projectId || '' : '' }" :dialogTitle="'选择仓库'"
...
{ label: "外协退货", value: "outbound_external" }, -->
            ...
            <div class="JNPF-common-layout-main JNPF-flex-main">
              <JNPF-table v-loading="listLoading" :data="productList" hasC :fixedNO="true"
                ...
                min-width="160" />
                <el-table-column prop="mainUnit" :label="mainUnitFlag == 1 ? '单位(主)' : '单位'" min-width="120" />
                <el-table-column prop="deliveryQuantity" :label="mainUnitFlag == 1 ? '数量(主)' : '数量'" min-width="160">
                </el-table-column>
                <el-table-column prop="deputyUnit" label="单位(副)" min-width="120" v-if="mainUnitFlag == 1" />
...
export default {
  ...
  methods: {
    ...
    searchProductFun() {
      ...
      this.orderForm.notifyType = "external"
    ...
    watchNum(row, index) {
      ...
      if (this.mainUnitFlag == 1) {
        if (row.calculationDirection == 'multiplication') {
          this.$set(row, 'deputyNum', this.jnpf.numberFormat(this.jnpf.math('multiply', [row.deliveryQuantity, row.ratio]), 6))
        } else {
          this.$set(row, 'deputyNum', this.jnpf.numberFormat(this.jnpf.math('divide', [row.deliveryQuantity, row.ratio]), 6))
        }
      }
        ...
        if (valid) {
          ...
          if (submitFlag) {
            ...
            this.productData.forEach(item => {
              let ordersLineId=JSON.parse(JSON.stringify(item.outShipmentListId))
              let ordersId=JSON.parse(JSON.stringify(item.ordersLineId))
              item.ordersLineId=ordersLineId
