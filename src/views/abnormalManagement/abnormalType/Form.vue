  ...
  methods: {
    setDataFormItems(){
      this.dataFormItems = [
        ...
        { prop: "name", label: "类型名称", value: "", type: "input", itemRules: [{ required: true, trigger: "blur" }], sm: 24 },
      ]
    },
    init(row, btnType) {
      this.visible = true
      ...
      if (row) {
        this.dataForm = { ...row, parentId: '-1' }
        this.autoCode = row.code
        this.formLoading = false
        this.title = this.btnType === 'look' ? '查看异常类型' : '编辑异常类型'
      } else {
        this.title = '新建异常类型'
        this.formLoading = false
      }
