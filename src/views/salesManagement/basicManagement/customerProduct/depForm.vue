<template>
  <transition name="el-zoom-in-center">
    <div class="JNPF-preview-main org-form">
                  ...
                  <el-row :gutter="30" class="custom-row">
                    ...
                    <el-col :sm="6" :xs="24">
                      <el-form-item label="客户" prop="cooperativePartnerIdText">
                        <ComSelect-page key="partner" ref="ComSelect-page" :value="dataForm.cooperativePartnerIdText" @change="partnerChange" :tableItems="partnerTableItems"
                          dialogTitle="选择客户" treeTitle="客户分类" placeholder="请选择客户" :methodArr="{ method: getcategoryTrees, requestObj: { type: 'customer' } }"
                          :listMethod="getCooperativeData" :listRequestObj="partnerRequestObj" :searchList="partnerSearchList" :treeNodeClick="yxPartnerTreeNodeClick"
              ...
              <el-collapse-item title="产品信息" name="productInfo" class="productInfo">
                ...
                <el-form :model="dataFormTwo" v-bind="dataFormTwo" ref="productForm" class="data-form">
                  <JNPF-table customKey="customerProduct" ref="product" :data="dataFormTwo.lines" @selection-change="handeleProductInfoData" v-bind="customStyleData" fixedNO :has-c="btnType === 'add'"  custom-column>
                    ...
                    <el-table-column prop="customerProductDrawingNo" label="客户型号" min-width="160">
                      <template slot-scope="scope">
                        <el-form-item>
                          <el-input v-model="scope.row.customerProductDrawingNo" placeholder="请输入" :disabled="btnType === 'look'">
                          </el-input>
                        </el-form-item>
                      </template>
                    </el-table-column>
                    ...
                    <el-table-column prop="productDrawingNo" label="品名规格" min-width="330">
                      ...
                      <template slot-scope="scope">
                        <el-autocomplete v-model="scope.row.productDrawingNo" :fetch-suggestions="querySearchAsync" placeholder="请输入" prefix-icon="el-icon-search"
                          ...
                          @select="handleSelect(scope.row, scope.$index, $event)"></el-autocomplete>
                    ...
                    <el-table-column prop="taxRate" label="税率" width="140">
                      ...
                      <template slot-scope="scope">
                        <el-select v-model="scope.row.taxRate" placeholder="税率" style="width: 100%;" :disabled="btnType === 'look'"
                    ...
                    <el-table-column v-if="btnType === 'look' ? ['2','0'].includes(includedTaxFlagConfig) : true" prop="excludingTaxPrice" label="单价(不含税)" width="150"
          ...
          <el-collapse-item title="基本信息" name="basicInfo" class="orderInfo">
            <el-form ref="dataForm" :model="dataForm" :rules="dataRule" label-width="160px" label-position="top">
              <el-row :gutter="30" class="custom-row">
                <el-col :sm="6" :xs="24">
                  <el-form-item label="客户" prop="cooperativePartnerIdText">
                    <ComSelect-page key="partner" ref="ComSelect-page" v-model="dataForm.cooperativePartnerIdText" @change="partnerChange" :tableItems="partnerTableItems"
                      dialogTitle="选择客户" treeTitle="客户分类" placeholder="请选择客户" :methodArr="{ method: getcategoryTrees, requestObj: { type: 'customer' } }"
                      :listMethod="getCooperativeData" :listRequestObj="partnerRequestObj" :searchList="partnerSearchList" :treeNodeClick="yxPartnerTreeNodeClick"
                      :isdisabled="btnType === 'look'"/>
                  </el-form-item>
      ...
      <el-dialog title="导入数据" append-to-body :close-on-click-modal="false" :close-on-press-escape="false" :visible.sync="uploadVisib" lock-scroll
        class="JNPF-dialog JNPF-dialog_center" width="400px">
        <el-upload cass="upload-demo" action="#" accept=".xls, .xlsx" :multiple="false" :auto-upload="false" :limit="1" :on-preview="handlePreview" drag :on-remove="handleRemove"
          :on-change="handleFileChange" ref="uploadRef">
      ...
      <el-dialog title="提示" append-to-body :close-on-click-modal="false" :close-on-press-escape="false" :visible.sync="tipsvisible" lock-scroll
      ...
      <el-dialog title="历史价格" :close-on-click-modal="false" :close-on-press-escape="false" :visible.sync="historyVisiblt" lock-scroll
      ...
      <el-dialog title="选择产品" :close-on-click-modal="false" :close-on-press-escape="false" :visible.sync="allProVisible" lock-scroll
        class="JNPF-dialog JNPF-dialog_center selectPro" width="70%" append-to-body>
        ...
        <div class="JNPF-common-layout" style="height: 68vh;overflow: auto;">
          <div class="JNPF-common-layout-left">
            ...
            <el-scrollbar class="JNPF-common-el-tree-scrollbar" v-loading="treeLoading">
              <el-tree ref="treeBox" :data="ProductTreeData" :props="defaultProps" :default-expand-all="expands" highlight-current :expand-on-click-node="false" node-key="id"
                @node-click="handleNodeAllProduct" class="JNPF-common-el-tree" v-if="refreshTree" :filter-node-method="filterNodeAllProduct">
...
import {getQuotationmxLists, saleUploadProductData} from "@/api/salesManagement/index";
import {
  addPartnerOrProductData,
  excelExport,
  getBimBusinessDetail,
  getCooperativeData,
  getcooperativeproductInfo,
  getOrderFiledMap,
  updatePartnerOrProductData
} from '@/api/basicData/index'
import {getcategoryTree as productTree} from '@/api/basicData/materialSettings' // 产品分类 编排属性值
import {getcategoryTrees} from '@/api/salesManagement/assemblyOrders'
import {getDetailByDrawNo, getProducts} from '@/api/masterDataManagement/index.js' // 产品列表
...
export default {
    ...
    return {
      ...
      productRules: {
        ...
        customerProductNo: [
          {
            validator: this.formValidate({
              type: 'noEmtry', params: [" 客户料号不能为空", (errMsg, index) => {
        ...
        num: [
          {
            validator: this.formValidate({
              type: 'noEmtry', params: ["数量不能为空", (errMsg, index) => {
                this.$message.error(`产品信息第${ index + 1 }行：${ errMsg }`)
              }]
            }), trigger: 'blur'
          },
          {required: true, trigger: 'blur'},
          {
            validator: this.formValidate('positiveNumber', '数量必须大于0', (errMsg, index) => {
              this.$message.error(`产品信息第${ index + 1 }行：${ errMsg }`)
            }), trigger: 'blur'
          }
        ],
        ...
        price: [
          {
            validator: this.formValidate({
              type: 'noEmtry', params: ["单价(含税)不能为空", (errMsg, index) => {
                this.$message.error(`产品信息第${ index + 1 }行：${ errMsg }`)
              }]
            }), trigger: 'blur'
          },
          {required: true, trigger: 'blur'},
          {
            validator: this.formValidate('positiveNumber', '单价(含税)必须大于0', (errMsg, index) => {
              this.$message.error(`产品信息第${ index + 1 }行：${ errMsg }`)
            }), trigger: 'blur'
          },
          {
            validator: this.formValidate({
              type: 'decimal', params: [18, 6, "", (errMsg, index) => {
                this.$message.error(`产品信息第${ index + 1 }行：单价(含税)${ errMsg }`)
              }]
            }), trigger: 'blur'
          }
        ],
  ...
  watch: {
    activeName(val) {
      ...
      if (val === 'orderInfo' && JSON.stringify(this.customStyleData) !== '{}') this.$nextTick(() => {
  ...
  methods: {
    ...
    partnerChange(val, data, paramsObj) {
      this.$nextTick(() => {
        this.$refs['dataForm'].validateField('cooperativePartnerIdText')
      }) // 校验操作的元素(name是组件绑定的value)
