...
<script>
import { delRubberhoselength, getRubberhoselengthList, } from '@/api/financialManagement/index'
...
export default {
  ...
  methods: {
    //区间规则
    hoseLengthRangefun(val) {
      let a = null
      if (val.endVal && val.startVal) {
        a = val.startVal + val.startSymbol + ' ? ' + val.endSymbol + val.endVal
      } else if (val.endVal && !val.startVal) {
        a = ' ? ' + val.endSymbol + val.endVal
      } else if (!val.endVal && val.startVal) {
        a = val.startVal + val.startSymbol + ' ? '
      }
      return a
    },
    // 关闭新建、编辑页面
    ...
    initData() {
      getRubberhoselengthList(this.tableQuery).then(res => {
    ...
    handleDel(id) {
      this.$confirm(this.$t('删除该长度区间，将会把计件价格表中的价格一起删除，是否继续？'), this.$t('common.tipTitle'), {
        type: 'warning'
      }).then(() => {
        delRubberhoselength(id).then(res => {
          this.initData()
          this.$message({
            type: 'success',
            message: "删除成功",
