<template>
  <div>
        ...
        <div class="main">
          <el-tabs v-model="activeName">
            <el-tab-pane label="基础信息" name="jcInfo">
                ...
                <el-form ref="elForm" :model="dataForm" :rules="rules" size="small" label-width="100px"
                  ...
                  <el-col :span="12">
                    <el-form-item label="供应商编码" prop="cooperativePartnerCode" ref="cooperativePartnerCode">
                      <el-input v-model="dataForm.cooperativePartnerCode" placeholder="供应商编码" disabled>
                      </el-input>
                    </el-form-item>
                  </el-col>
              ...
              <el-form :model="dataFormTwo" ref="productForm">
                <el-table style="border: 1px solid #e3e7ee;" @selection-change="handeleProductInfoData" hasC hasNO fixedNO
                  ...
                  <el-table-column prop="productsCode" key="productsCode" label="产品编码" min-width="200"
                    show-overflow-tooltip>
                    <template slot-scope="scope">
                      <el-form-item :prop="'data.' + scope.$index + '.' + 'productsCode'"
                        :rules='productRules.productsCode'>
                        <div class="viewData">
                  ...
                  <el-table-column prop="productsName" key="productsName" label="产品名称" min-width="200"
                    show-overflow-tooltip>
                    <template slot-scope="scope">
                  ...
                  <el-table-column prop="drawingNo" key="drawingNo" label="产品图号" min-width="200" show-overflow-tooltip>
                    <template slot-scope="scope">
                  ...
                  <el-table-column prop="bargaining" key="bargaining" label="协议价" min-width="140">
                    <template slot="header">
                      <span class="required">*</span>协议价
                    </template>
                    <template slot-scope="scope">
                      <el-form-item :prop="'data.' + scope.$index + '.' + 'bargaining'" :rules='productRules.bargaining'>
                        <el-input v-model="scope.row.bargaining" @input="bargainingChange($event, scope)"
                          :disabled="type === 'look'" maxlength="20" placeholder="请输入协议价">{{ scope.row.bargaining }}
                        </el-input>
                      </el-form-item>
                    ...
                    <template slot-scope="scope">
                      <el-form-item :prop="'data.' + scope.$index + '.' + 'taxRate'" :rules='productRules.taxRate'>
                        <el-input oninput="value = value.replace(/\D/g,'')" v-model="scope.row.taxRate"
                          @input="taxRateChange($event, scope)" :disabled="type === 'look'" maxlength="2"
                          placeholder="请输入税率">{{ scope.row.taxRate }}
                        </el-input>
                      </el-form-item>
                    </template>
                  </el-table-column>
                  <!-- <el-table-column prop="excludingTaxPrice" key="excludingTaxPrice" label="不含税价" min-width="180">
                    <template slot="header">
                  ...
                  <el-table-column prop="excludingTaxPrice" key="excludingTaxPrice" label="不含税价" min-width="200"
                    show-overflow-tooltip>
                    <template slot-scope="scope">
                      <el-form-item :prop="'data.' + scope.$index + '.' + 'excludingTaxPrice'">
                        <div class="viewData">
                  ...
                  <el-table-column prop="targetPrice" key="targetPrice" label="目标价" min-width="180">
                    <template slot-scope="scope">
                      <el-form-item :prop="'data.' + scope.$index + '.' + 'targetPrice'"
                        :rules='productRules.targetPrice'>
                        <el-input v-model="scope.row.targetPrice" :disabled="type === 'look'" maxlength="20"
                          :placeholder="type == 'look' ? '' : '请输入目标价'">{{
                  ...
                  <el-table-column prop="historicalPrice" key="historicalPrice" label="历史价格" min-width="180"
                    show-overflow-tooltip />
                    ...
                    <template slot-scope="scope">
                      <el-form-item :prop="'data.' + scope.$index + '.' + 'effectiveTimeStart'"
                        ...
                        <el-date-picker v-model="scope.row.effectiveTimeStart" type="date" value-format="yyyy-MM-dd"
                          style="width: 100%;" placeholder="请选择有效起始时间" :disabled="type == 'look' ? true : false">
                        </el-date-picker>
                      </el-form-item>
                    </template>
                  </el-table-column>
                    ...
                    <template slot-scope="scope">
                      <el-form-item :prop="'data.' + scope.$index + '.' + 'effectiveTimeEnd'"
                        ...
                        <el-date-picker v-model="scope.row.effectiveTimeEnd" type="date" value-format="yyyy-MM-dd"
                          style="width: 100%;" placeholder="请选择有效结束时间" :disabled="type == 'look' ? true : false">
                        </el-date-picker>
                      </el-form-item>
                    </template>
                  </el-table-column>

                  <!-- <el-table-column prop="contrastProductsName" key="contrastProductsName" label="对比物料" min-width="200">
                    <template slot-scope="scope">
                      <el-form-item :prop="'data.' + scope.$index + '.' + 'contrastProductsName'">
                        <el-input v-model="scope.row.contrastProductsName" readonly
                          @focus="openContrastProducts(scope.$index)" :disabled="type === 'look'" maxlength="20"
                          placeholder="请选择对比物料">
                          <span slot="suffix" v-show="scope.row.contrastProductsName && type !== 'look'"
                            @click="btnClearable(scope.$index, scope.row.contrastProductsName)">
                  ...
                  <!-- <el-table-column prop="contrastProductsPrice" key="contrastProductsPrice" label="对比物料价格"
                    min-width="200">
    ...
    <Product-dialog v-if="contrastProductsVisibled" ref="contrastProductsRef"
      @productSubmit="productSubmit"></Product-dialog>
...
import { getbuyFixedPointPricingDetail, addbuyFixedPointPricing, editbuyFixedPointPricing,buyFixedPointPricingUploadData} from '@/api/purchasingManagement/purchaseInquirySheet'  // 询价单
...
export default {
  ...
  data() {
    return {
      ...
      ProductMethodArr: [
        { label: "物料分类", classAttribute: "material", method: productCategoryTree, requestObj: { classAttribute: "material" } },
      ], // 产品选择弹出框树状列表
      ...
      productRules: {
        ...
        effectiveTimeEnd: [{ required: true, message: '请选择有效结束日期', trigger: ['change'] },
        { validator: this.checkDate2(), trigger: 'change' },
        ],
        materialPrice: [{ required: true, trigger: ['blur'] }, { validator: this.formValidate({ type: 'decimal', params: [20, 8, "", (errMsg) => { this.$message.error('物料价格：' + errMsg) }] }), trigger: 'blur' }],
        taxRate: [
          ...
          {
            validator: this.checktaxRate(), trigger: 'blur'
          },
        ],
        excludingTaxPrice: [{ required: true, trigger: ['blur'] },
        { validator: this.formValidate({ type: 'decimal', params: [20, 8, "", (errMsg) => { this.$message.error('不含税价' + errMsg) }] }), trigger: 'blur' },
        { validator: this.formValidate('noZero', '', (errMsg) => { this.$message.error('不含税价不能为0') }), trigger: 'blur' }
        ],
        bargaining: [
          { required: true, trigger: ['blur'] },
          { validator: this.formValidate({ type: 'decimal', params: [20, 4, "", (errMsg) => { this.$message.error('协议价' + errMsg) }] }), trigger: 'blur' },
          { validator: this.formValidate('noZero', '', (errMsg) => { this.$message.error('协议价不能为0') }), trigger: 'blur' }
        ],
        targetPrice: [
          { validator: this.formValidate({ type: 'decimal', params: [20, 4, "", (errMsg) => { this.$message.error('目标价格' + errMsg) }] }), trigger: 'blur' },
          { validator: this.formValidate('noZero', '', (errMsg) => { this.$message.error('目标价格不能为0') }), trigger: 'blur' }
        ],
  ...
  methods: {
    // 谈价更改
    bargainingChange(val, scope) {
      scope.row.excludingTaxPrice = this.jnpf.numberFormat(val / (1 + (scope.row.taxRate / 100)))
    },
    // 税率更改
    taxRateChange(val, scope) {
      scope.row.excludingTaxPrice = this.jnpf.numberFormat(scope.row.bargaining / (1 + (val / 100)))
    },
    checkDate() {
    ...
    addth(id, data) {
     ...
     UploadProduct(data) {
      ...
      //调用上传文件接口
      buyFixedPointPricingUploadData(formData).then(res => {
        ...
        this.dataFormTwo.data.map(item=>{
          return item.excludingTaxPrice = this.jnpf.numberFormat(item.bargaining / (1 + (item.taxRate / 100)))
        })
    ...
    // 对比物料弹窗
    openContrastProducts(index) {
      console.log(index);
      this.index = index
      this.contrastProductsVisibled = true
      this.$nextTick(() => {
        this.$refs.contrastProductsRef.initData2()
      })
    },
    // 打开选择供应商弹窗
    openDialog() {
            ...
            if (this.dataFormTwo.data.length === 0) {
              ...
              if (!valid_2) {
                ...
                if (!this.dataForm.id) {
                  ...
                  editbuyFixedPointPricing(_data).then((res) => {
