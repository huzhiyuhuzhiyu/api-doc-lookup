    ...
    <div class="JNPF-common-layout-center JNPF-flex-main">
      ...
      <div class="JNPF-common-layout-main JNPF-flex-main">
        ...
        <JNPF-table v-loading="listLoading" highlight-current-row :fixedNO="true" ref="tableForm" :data="tableDataList"
          @sort-change="sortChange" custom-column border @selection-change="handleSelectionChange" hasC customKey="JNPFTableKey_212369">
          ...
          <el-table-column prop="blankProductsName" label="毛坯产品名称" min-width="150" show-overflow-tooltip
            sortable="custom" />
          <el-table-column prop="blankProductsDrawingNo" label="毛坯产品图号" min-width="150" show-overflow-tooltip />
          <el-table-column prop="blankProductNo" label="毛坯料号" min-width="150" show-overflow-tooltip />
          <el-table-column prop="grossWeight" label="毛坯重(kg)" min-width="120" />
          <el-table-column prop="materialCost" label="材料成本(元)" min-width="150" sortable="custom" />
          <el-table-column prop="productWeight" label="成品重(kg)" min-width="140" />
          <el-table-column prop="drillWorkingHours" label="钻孔工时(s)" min-width="110" />
          <el-table-column prop="totalWorkingHours" label="总工时(s)" min-width="120" sortable="custom" />
          <el-table-column prop="totalLaborCost" label="总人工费用(元)" min-width="180" sortable="custom" />
          <el-table-column prop="feeAmount" label="总制造费用(元)" min-width="180" sortable="custom" />
          <el-table-column prop="totalCost" label="成本合计(元)(不含税)" min-width="200" sortable="custom" />
          <el-table-column prop="updateTime" label="最新成本更新时间" min-width="180"  >

          <template scope="scope">
              <div>{{ scope.row.updateTime?scope.row.updateTime.substring(0,10):'' }}</div>
            </template>
          </el-table-column>
          <el-table-column prop="updateByName" label="最新成本更新人" min-width="150" />
        ...
        <el-form ref="diaForm" :model="dataForm" label-width="120px" label-position="top">
          ...
          <el-col :span="12">
            <el-form-item label="毛坯产品名称">
              <el-input v-model="dataForm.blankProductsName" placeholder="请输入毛坯产品名称" width="180" clearable maxlength="20">
              </el-input>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="毛坯产品图号">
              <el-input v-model="dataForm.blankProductsDrawingNo" placeholder="请输入毛坯产品图号" width="180" clearable
                maxlength="20"> </el-input>
            </el-form-item>
          </el-col>

          <el-col :span="12">
            <el-form-item label="毛胚重量">
              <el-select placeholder="请选择毛胚重量" v-model="dataForm.hasGrossWeight" clearable>
                <el-option v-for="(item, index) in hasGrossWeightList" :key="index" :label="item.label"
          ...
          <el-col :span="12">
            <el-form-item label="成品重量">
              <el-select placeholder="请选择成品重量" v-model="dataForm.costWeight" clearable>
                <el-option v-for="(item, index) in costWeightList" :key="index" :label="item.label"
                  :value="item.value"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="总工时">
              <el-select placeholder="请选择总工时" v-model="dataForm.hasTotalWorkingHours" clearable>
                <el-option v-for="(item, index) in hasTotalWorkingHoursList" :key="index" :label="item.label"
                  :value="item.value"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
          ...
          <el-col :span="12">
            <el-form-item label="最新成本更新时间" prop="costDateArr">
...
<script>
import { getfinSelfMadePricingList, updatetfinSelfMadePricing } from '@/api/financialManagement/index'
