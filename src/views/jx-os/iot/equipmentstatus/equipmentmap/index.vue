<template>
    <div class="JNPF-common-layout">
        <!-- 弹出框 -->
        <Detailspages v-if="!flags" :equipmentId="equipmentId" @handle="handles"/>
        <div v-show="flags" v-loading="loading" class="JNPF-common-layout-center">
                    ...
                    <el-col :span="5">
                        <el-form-item label="车间">
                            <JNPFTreeSelect v-model="query.mrpWorkshopId" placeholder="请选择"
                                            ...
                                            @change="mrpWorkshopIdChange" clearable
                            />
                        </el-form-item>
                    </el-col>
                    <el-col :span="4">
            ...
            >
                <!-- 五个小方块 -->
                <div class="text" style="width: 100%;display: flex;margin-top: 10px;font-size: ; padding-left: 10px;">
                    <div style="display: flex; ">
                        ...
                        >
                            ...
                            <!-- <div class="text" :style="{backgroundColor:item.backgroundColor,color:item.color}"
                            style="width: 100%;display: flex;align-items: center;padding-top: 0.5vw;padding-bottom: 0.5vw;box-shadow: 3px 3px 3px gray;">
                            <span style="margin-left: 5px;transform: scale(0.8);">{{item.name}}：{{item.number}}</span>
                            <div style="flex: 1;"></div>
                            <span style="margin-right: 5px;transform: scale(0.8);">{{item.ratio}}</span>
                        </div> -->

                            <div class="tags" :style="{ backgroundColor: item.bgc }">
                            ...
                            <div style="text-align: center;" :style="{ width: settings.rightWidth }">
                                ...
                                <p style="line-height: 22px; ">{{ item.ratio }}</p>
                            </div>

                            <!-- <div id="equipmentstatus" class="text"
                            style="width: 100%;background-color: #1ABE6C;display: flex;align-items: center;justify-content: center;padding-top: 0.5vw;padding-bottom: 0.5vw;box-shadow: 3px 3px 3px gray;">
                            <div>
                                故障
                            </div>
                            ：
                            <span :class="'so'+index">0</span>
                        </div>
 -->
                        </div>
                    </div>
                    ...
                    <div style="padding: 4px 0; width:140px; display: flex;align-items: center;margin: 0 10px;">
                        <el-select v-model="sizeFlag" placeholder="请选择">
                            <el-option v-for="(item, index) in flagOption" :key="item.value" :label="item.label"
                                       :value="item.value"
                            >
                            </el-option>
                        </el-select>
                        <!--  <screenfull style="margin: 0 10px;" isContainer/> -->
                    ...
                    >
                        ...
                        >
                            ...
                            <div class="mingcheng" style="">
                                ...
                                <div class="text fontc">状态持续时间:{{ item.stateTime }}</div>
                            </div>
                        </div>
...
<script>
import { getColorListApi, getListState } from '@/api/equipment'
...
import { selectmrpWorkshopId } from '@/views/jx-os/equipmentstatus/equipmentmap/leafNode'
...
export default {
    ...
    created() {
        ...
        getColorListApi(colorList => {
            ...
            localStorage.setItem('colorList', JSON.stringify(this.stateList))
            // 拿到颜色之后 在请求设备列表
            this.initData(true)
        })
    ...
    methods: {
        ...
        mrpWorkshopIdChange(e) {
            let lista = [];
           let list = selectmrpWorkshopId(e,lista);
            console.log(lista)
        },
        ...
        multiselectionfilter() {
            ...
            let conditions = []
            for (let kets in this.query) {
                // console.log(kets.substr(kets.length-2))
                if (kets == 'customer_id' && this.query[kets] != undefined) {
                    let objects = {
                        key: kets,
                        value: parseInt(this.query[kets]) || null,
