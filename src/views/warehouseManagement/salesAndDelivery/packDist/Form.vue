...
import {getOrderDetail, getsaleOrderDetailList} from "@/api/salesManagement/assemblyOrders";
...
import {addQuotationsendlist, confirmOrdersNotice, editQuotationMsendlist} from "@/api/salesManagement";
...
export default {
  ...
  data() {
    return {
      ...
      apiMethodActions: {
        ...
        confirm: {
          api: confirmOrdersNotice,
          deliveryStatus: this.dataForm.deliveryStatus === 'ready' ? 'arranged' : '',
        }
      }
    }
  },
  ...
  methods: {
    ...
    async defaultInit(prefillData) {
      ...
      this.linesList = prefillData.orderLines.map(item => ({
        ...item,
        ordersNum: item.num,
        productDrawingNo: item.drawingNo,
        deliveryQuantity: item.waitDeliverNum
      })) || []
      this.fileList = this.fileListMap('', prefillData.attachmentList || [])
    },
            ...
            <div class="JNPF-common-page-header">
              ...
              <div class="options">
                <template v-if="activeType">
                  <template v-if="btnType === 'add' || btnType === 'edit'">
                  ...
                  <template v-else-if="btnType === 'confirm'">
                    <el-button type="primary" :loading="btnLoading" @click="handleSubmit('submit')">
                      确认
                    </el-button>
                  </template>
                  <template v-else-if="btnType === 'packDist'">
