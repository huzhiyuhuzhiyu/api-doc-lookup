...
import {addBimPackagingMaterials, getBimPackagingMaterials, updateBimPackagingMaterials} from "@/api/packagingMaterials";
...
export default {
  ...
  methods: {
    async init(id = '', type, packagingType) {
      this.btnType = type
      this.dataForm.packagingType = packagingType
    ...
    async getDetail(id) {
      this.loading = true
      try {
        const res = await getBimPackagingMaterials(id)
        const {msg, data} = res
        if (msg === 'Success') {
          this.dataForm = data
          this.loading = false
    ...
    async handleSubmit() {
      ...
      let MSG = '提交成功'
      try {
        const apiMethod = this.dataForm.id ? updateBimPackagingMaterials : addBimPackagingMaterials
