<template>
  <div :class="[elementShow ? 'popupSelect-container' : '']">
    ...
    <el-dialog :title="'选择' + getTitle" :close-on-click-modal="false" :visible="visible" lock-scroll
...
export default {
  ...
  computed: {
    ...
    getTitle() {
      let title = ''
      if (this.selectClassifyType === 'customer') {
        title = '客户'
  ...
  methods: {
    ...
    async getData() {
      ...
      this.$nextTick(() => {
        ...
        if (this.selectClassifyType) {
          ...
          this.$message.warning('请先选择所属合作伙伴')
        }
      })
    },
    // 获取列表数据
    initData() {
      ...
      getCooperativeData(this.listQuery).then(async listRes => {
