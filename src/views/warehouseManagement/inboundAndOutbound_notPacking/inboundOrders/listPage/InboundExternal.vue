          ...
          <div class="JNPF-common-layout-center JNPF-flex-main">
            ...
            <div class="JNPF-common-layout-main JNPF-flex-main">
              <div class="JNPF-common-head">
                <el-page-header @back="$emit('close', true)" :content="'外协收货入库'" />
              ...
              <JNPF-table ref="dataTable" v-loading="listLoading" :data="tableData" :fixedNO="true"
                ...
                <el-table-column prop="orderNo" label="收货单号" width="200" fixed="left" sortable="custom">
                  <template slot-scope="scope">
                    <el-link type="primary"
                      @click.native="addOrUpdateHandle(scope.row.id, true, scope.row.notificationType)">
                      {{ scope.row.orderNo }}
                    </el-link>
                  </template>
                </el-table-column>
                <el-table-column prop="purchaseOrderNo" label="外协单号" width="200" fixed="left" sortable="custom" />
                <el-table-column prop="salesman" label="业务员" width="120" sortable="custom" />
                    ...
                    <tableOpts @edit="addOrUpdateHandle(scope.row.id)" :editText="'入库'" :hasDel="false">
                      <el-dropdown hide-on-click>
                        ...
                        <el-dropdown-menu slot="dropdown">
                          <el-dropdown-item
                            @click.native="addOrUpdateHandle(scope.row.id, true, scope.row.mainNotificationType)">
                            查看详情
                          </el-dropdown-item>
                        </el-dropdown-menu>
                      </el-dropdown>
            ...
            <div class="JNPF-common-layout-main JNPF-flex-main">
              ...
              <JNPF-table ref="linesTableData" v-loading="listLoading" :data="linesTableData" :fixedNO="true"
                ...
                <el-table-column prop="orderNo" label="发货单号" width="190" fixed="left" sortable="custom">
                  <template slot-scope="scope">
                    <el-link type="primary"
                      @click.native="addOrUpdateHandle(scope.row.returnDeliveryNoticeId, true, scope.row.mainNotificationType)">{{
                        scope.row.orderNo
                      }}</el-link>
                  </template>
                </el-table-column>
...
import DetailForm from '../../../inboundAndOutbound/inboundOrders/detailForm/InboundExternal.vue'
export default {
  ...
  data() {
    return {
      ...
      initListQuery: {
        ...
        receivingStatus: 'receiving', //	收货状态,可用值:received,receiving,returned,returning
  ...
  methods: {
    ...
    sortChange({ prop, order }) {
      ...
      if (this.activeName == "dataTable") {
        if (['purchaseOrderNo', 'partnerCode', 'partnerName', 'shipperName', 'createByName', 'productName', 'productDrawingNo', 'routingName', 'processName'].includes(prop)) { newProp = prop }
        else { newProp = prop.replace(/[A-Z]/g, match => '_' + match.toLowerCase()); }
        this.listQuery.orderItems[0].asc = order !== "descending"
        this.listQuery.orderItems[0].column = order === null ? "" : newProp
      } else {
        if (['orderNo', 'partnerName', 'salesman', 'productName', 'productDrawingNo'].includes(prop)) { newProp = prop }
        else { newProp = prop.replace(/[A-Z]/g, match => '_' + match.toLowerCase()); }
    ...
    addOrUpdateHandle(id, readOnly, notificationType) {
      if (readOnly) {
        this.detailFormVisible = true
        this.$nextTick(() => { this.$refs.DetailForm.init(id, 'look', notificationType) })
      } else {
        this.formVisible = true
        this.$nextTick(() => { this.$refs.Form.init(id, undefined, "inbound_external") })
      }
    },
  }
}
