    ...
    <transition name="fade">
      <div v-show="systemShow" class="system">
        <div class="system-header" :class="headClass" :style="{ backgroundColor: head }">
          ...
          <span @click="closeSystem" style="line-height: 60px; cursor: pointer; font-size: 18px;">
          ...
          <span class="ant-input-group-wrapper ant-input-search">
            <span class="ant-input-wrapper ant-input-group">
              <span class="ant-input-affix-wrapper">
                <input @input="search(searchValue)" @compositionstart="onCompositionStart"
                  @compositionend="onCompositionEnd" v-model="searchValue" placeholder="搜索：请输入关键词" type="text"
                  class="ant-input" />
                <span class="ant-input-suffix">
                  <span @click="clearSearch" v-show="searchValue" role="button" aria-label="close-circle" tabindex="-1"
                    class="  ant-input-clear-icon-hidden ant-input-clear-icon-has-suffix ant-input-clear-icon">
                    <svg focusable="false" class="" data-icon="close-circle" width="1em" height="1em"
              ...
              <span class="ant-input-group-addon">
                <button class="ant-btn ant-input-search-button ant-btn-icon-only" type="button">
...
import { setMajor, getSystem } from '@/api/permission/userSetting'
import getProjectList from '@/mixins/generator/getProjectList'
import { getProjectList as getProjectLists } from '@/api/system/projectManagement'
export default {
  computed: {
  ...
  methods: {
    changeMajor(majorId, majorType, name) {
      //切换系统
      // localStorage.setItem("qhxt",true)
      if (name === this.projectName) return
      console.log(majorId, 'oo')
      this.autoProjectId = majorId
      localStorage.setItem('autoProjectId', majorId)
      this.$store.commit('user/SET_USERINFO_PROJECTID', majorId)

      this.$router.push({ name: 'commonPage' })
      location.reload()
    },
    showSystem() {
      if (this.userInfo.userProjectId !== '1') return
    ...
    async searchData() {
      if (this.searchValue) {
        ...
        let res = await getProjectLists(this.systemQuery)
