                  ...
                  <el-table-column label="操作" width="180" fixed="right">
                    <template slot-scope="scope">
                      ...
                      <el-button size="mini" type="text" :disabled="sourceDisabled"
                        @click="handlerOpenSource(scope.$index, 'source')">配置发料清单</el-button>
...
export default {
  ...
  methods: {
    ...
    handlerOpenSource(index, type) {
      ...
      this.index = index
      // 通过需求池id 获取明细的数据
      purProcurementDemandPoolexternal(this.dataFormTwo.data[index].id).then(res => {
    ...
    init(data, demandDelivery) {
      ...
      console.log(this.demandDelivery);
