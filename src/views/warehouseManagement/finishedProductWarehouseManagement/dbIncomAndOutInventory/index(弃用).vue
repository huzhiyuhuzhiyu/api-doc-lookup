    ...
    <div class="JNPF-common-layout-center JNPF-flex-main" v-if="!visibleForm">
      ...
      <div class="JNPF-common-layout-main JNPF-flex-main" v-loading="listLoading">
        ...
        <JNPF-table :partentOrChild="'fhtabForm'" v-loading="listLoading" :data="fhTableList" @sort-change="sortChange"
          ...
          <el-table-column prop="partnerCode" label="客户编码" width="200" sortable="custom" v-if="userInfo.roleCode.split(',').includes('show_warehouse_data')&& userInfo.roleCode.split(',').includes('show_cooperativePartnerIdName_data')" />
          <el-table-column prop="partnerName" label="客户名称" min-width="140" sortable="custom" v-if="userInfo.roleCode.split(',').includes('show_warehouse_data') && userInfo.roleCode.split(',').includes('show_cooperativePartnerIdName_data')"  />
        ...
        <JNPF-table :partentOrChild="'cgthtabForm'" v-loading="listLoading" @sort-change="sortChange"
          ...
          <el-table-column prop="partnerName" label="供应商名称" min-width="140" sortable="custom" v-if="userInfo.roleCode.split(',').includes('show_warehouse_data') && userInfo.roleCode.split(',').includes('show_cooperativePartnerIdName_data')" />
          <el-table-column prop="partnerCode" label="供应商编码" width="200" sortable="custom" v-if="userInfo.roleCode.split(',').includes('show_warehouse_data') && userInfo.roleCode.split(',').includes('show_cooperativePartnerIdName_data')" />
        ...
        <JNPF-table :partentOrChild="'wxflOrdertabForm'" v-loading="listLoading" @sort-change="sortChange" key="3"
          ...
          <el-table-column prop="productName" label="产品名称" v-if="isProductNameSwitch === '1'" min-width="160"
            sortable="custom" />
          <el-table-column prop="drawingNo" label="型号" min-width="300" sortable="custom"></el-table-column>
        ...
        <JNPF-table :partentOrChild="'dataTableProductRef'" ref="dataTableProductRef" v-loading="listLoading"
          ...
          <el-table-column label="操作" width="100" fixed="right">
            <template slot-scope="scope">
              <el-button size="mini" type="text"
                @click="productInbound(scope.row, 'inbound_order_production')">入库</el-button>
            </template>
          </el-table-column>
        </JNPF-table>

        <!-- 翻库入库 -->
        <JNPF-table :partentOrChild="'dataTableFilpRef'" ref="dataTableFilpRef" v-loading="listLoading"
          v-if="isProjectSwitchFlag" :data="filpData" :fixedNO="true" custom-column :setColumnDisplayList="filpColumns"
  ...
  methods: {
          ...
          detailpurchaseOrderList(this.purchaseForm).then(res => {
            if (this.mainUnitFlag == 1) {
              res.data.records.forEach(item => {
                ...
                if (item.calculationDirection == 'multiplication') {
                  this.$set(item, 'deputyNum', this.jnpf.numberFormat(this.jnpf.math('multiply', [item.requiredReceivedQuantity, item.ratio]), 6))
                } else {
                  this.$set(item, 'deputyNum', this.jnpf.numberFormat(this.jnpf.math('divide', [item.requiredReceivedQuantity, item.ratio]), 6))
                }
              });
            }
    ...
    advancedQueryFun() {
      ...
      if (this.categoryType == 'outbound_sale_send' && this.saleFlag) {
        ...
        this.advancedQueryFuns('deliveryDate')
