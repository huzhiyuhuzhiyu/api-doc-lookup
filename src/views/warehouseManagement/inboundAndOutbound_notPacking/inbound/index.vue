<template>
  <div class="JNPF-common-layout">
          ...
          <div class="JNPF-common-layout-center JNPF-flex-main">
            <el-row class="JNPF-common-search-box" :gutter="16">
              <el-form @submit.native.prevent>
                <el-col :span="4">
                  <el-form-item>
                    <el-input v-model="listQuery.orderNo" placeholder="请输入入库单号" clearable
            ...
            <div class="JNPF-common-layout-main JNPF-flex-main">
              ...
              <JNPF-table v-loading="listLoading" :data="tableData" fixedNO @sort-change="sortChange" custom-column
                ref="dataTable">
                <el-table-column prop="orderNo" label="入库单号" min-width="200" sortable="custom">
                  <template slot-scope="scope">
                    <el-link type="primary" @click.native="addOrUpdateHandle(scope.row.id, true, scope.row.sourceType, scope.row.sourceNo)">
                      {{ scope.row.orderNo }}
                    </el-link>
                  </template>
                </el-table-column>
                    ...
                    <tableOpts @edit="addOrUpdateHandle(scope.row.id)" @del="handleDel(scope.row.id)"
                      ...
                      <el-dropdown hide-on-click>
                        ...
                        <el-dropdown-menu slot="dropdown">
                          <el-dropdown-item @click.native="addOrUpdateHandle(scope.row.id, true, scope.row.sourceType, scope.row.sourceNo)">
                            查看详情
                          </el-dropdown-item>
                        </el-dropdown-menu>
                      </el-dropdown>
          ...
          <div class="JNPF-common-layout-center JNPF-flex-main">
            <el-row class="JNPF-common-search-box" :gutter="16">
              <el-form @submit.native.prevent>
                <el-col :span="4">
                  <el-form-item>
                    <el-input v-model="linesQuery.orderNo" placeholder="请输入入库单号" clearable
                      @keyup.enter.native="search()" />
                  </el-form-item>
                </el-col>
                <el-col :span="4">
            ...
            <div class="JNPF-common-layout-main JNPF-flex-main">
              ...
              <JNPF-table v-loading="listLoading" :data="linesTableData" fixedNO @sort-change="sortChange" custom-column
                ...
                <el-table-column prop="orderNo" label="入库单号" min-width="200" sortable="custom">
                  <template slot-scope="scope">
                    <el-link type="primary" @click.native="addOrUpdateHandle(scope.row.moveId, true, scope.row.sourceType, scope.row.sourceNo)">
                      {{ scope.row.orderNo }}
                    </el-link>
                  </template>
                </el-table-column>
                  ...
                  <template slot-scope="scope">
                    <tableOpts @edit="addOrUpdateHandle(scope.row.moveId)" @del="handleDel(scope.row.moveId)"
                      ...
                      <el-dropdown hide-on-click>
                        ...
                        <el-dropdown-menu slot="dropdown">
                          <el-dropdown-item @click.native="addOrUpdateHandle(scope.row.moveId, true, scope.row.sourceType, scope.row.sourceNo)">
                            查看详情
                          </el-dropdown-item>
                        </el-dropdown-menu>
                      </el-dropdown>
    ...
    <Form v-if="formVisible" ref="Form" @refreshDataList="initData" @close="closeForm" :originTypeList="originTypeList" />    <InboundOrdersForm v-if="inboundOrdersFormVisible" ref="InboundOrdersForm" @close="closeForm"
...
import { getWarehouseList, deleteWarehouseData, getWarehouseLinesList } from '@/api/warehouseManagement/inboundAndOutbound'
import originTypeList from "../../inboundAndOutbound/originTypeList.js"
import Form from './Form.vue'
import InboundOrdersForm from '../inboundOrders/Form.vue'
export default {
  components: { Form, InboundOrdersForm },
  data() {
    return {
      ...
      formVisible: false,
      inboundOrdersFormVisible: false,
  ...
  methods: {
    sortChange({ prop, order }) {
      ...
      if (this.activeName === 'dataTable') {
        ...
        if (['partnerCode', 'partnerName', 'productName', 'createTime'].includes(prop)) { newProp = prop }
        else { newProp = prop.replace(/[A-Z]/g, match => '_' + match.toLowerCase()); }
    ...
    addOrUpdateHandle(id, btnType, sourceType, orderNo) {
      console.log(id, btnType, sourceType)
      if (['inbound_order_production'].includes(sourceType) && btnType && !orderNo ) { // 批量入库的数据
        this.inboundOrdersFormVisible = true
        this.$nextTick(() => { this.$refs.InboundOrdersForm.init(id, btnType, sourceType + '_batch', id) })
      } else if (['inbound_order_production'].includes(sourceType)) {
        this.inboundOrdersFormVisible = true
        this.$nextTick(() => { this.$refs.InboundOrdersForm.init(id, btnType, sourceType, id) })
      } else {
    ...
    closeForm(isRefresh) {
      ...
      this.inboundOrdersFormVisible = false
