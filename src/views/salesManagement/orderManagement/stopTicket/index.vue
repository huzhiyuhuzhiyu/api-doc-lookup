          ...
          <div class="JNPF-common-layout-center JNPF-flex-main">
            <el-row class="JNPF-common-search-box" :gutter="16">
              <el-form @submit.native.prevent>
                <el-col :span="4">
                  <el-form-item>
                    <el-input v-model="dataForm.orderNo" @keyup.enter.native="search()" placeholder="请输入停止单号"
                      clearable />
                  </el-form-item>
                </el-col>
                ...
                <el-col :span="4">
                  <el-form-item>
                    <el-input v-model="dataForm.saleOrderNo" @keyup.enter.native="search()" placeholder="请输入订单号"
                      clearable />
                  </el-form-item>
                </el-col>
                <el-col :span="4">
            ...
            <div class="JNPF-common-layout-main JNPF-flex-main">
              ...
              <JNPF-table ref="dataTable" v-loading="listLoading" :data="tableData" @sort-change="sortChange"
                custom-column>
                <el-table-column prop="orderNo" label="停止单号" min-width="200" sortable="custom">
                ...
                <el-table-column prop="saleOrderNo" label="订单号" width="180" sortable="custom">

                </el-table-column>


                <el-table-column prop="applyReason" label="申请理由" min-width="150" />

                <el-table-column prop="reasonRejection" label="驳回理由" min-width="150" />
          ...
          <div class="JNPF-common-layout-center JNPF-flex-main">
            <el-row class="JNPF-common-search-box" :gutter="16">
              <el-form @submit.native.prevent>
                <el-col :span="4">
                  <el-form-item>
                    <el-input v-model="linesQuery.orderNo" @keyup.enter.native="search()" placeholder="请输入停止单号"
                      clearable />
                  </el-form-item>
                </el-col>
                ...
                <el-col :span="4">
                  <el-form-item>
                    <el-input v-model="linesQuery.saleOrderNo" @keyup.enter.native="search()" placeholder="请输入订单号"
                      clearable />
                  </el-form-item>
                </el-col>
            ...
            <div class="JNPF-common-layout-main JNPF-flex-main">
              ...
              <JNPF-table ref="linesTableData" v-loading="listLoading" :data="linesTableData" @sort-change="sortChange"
                :fixedNO="true" custom-column :partentOrChild="'child'" >
                <el-table-column prop="orderNo" label="停止单号" min-width="200" sortable="custom" fixed="left">
                  <template slot-scope="scope">
                ...
                <el-table-column prop="ordersNo" label="订单号" width="180" fixed="left">
                </el-table-column>
                <el-table-column prop="customerProductDrawingNo" label="客户产品图号" width="180" sortable="custom"
                  fixed="left">
                </el-table-column>
                <el-table-column prop="productCode" label="产品编码" width="180" sortable="custom" fixed="left">
                </el-table-column>
                <el-table-column prop="productName" label="产品名称" width="180" sortable="custom" fixed="left">
                </el-table-column>
                <el-table-column prop="productDrawingNo" label="产品图号" width="180" sortable="custom" fixed="left">
                </el-table-column>
                <el-table-column prop="num" label="订单数量" width="180"> </el-table-column>
                <!-- <el-table-column prop="stopReason" label="停止原因" width="180"> </el-table-column>
                <el-table-column prop="stopDate" label="停止日期" width="180" sortable="custom"> </el-table-column> -->
                <el-table-column prop="remark" label="备注" min-width="150"></el-table-column>
                <el-table-column prop="createTime" label="创建时间" width="180" sortable="custom"></el-table-column>
                <el-table-column prop="createByName" label="创建人" width="120"></el-table-column>
                ...
                <el-table-column label="操作" width="180">
                  <template slot-scope="scope">
                    <el-button size="mini" type="text" :disabled="scope.row.documentStatus == 'draft' ? false : true"
                      @click="updateHandle(scope.row.ordersId, 'edit')">编辑</el-button>
                    <el-button size="mini" type="text" class="JNPF-table-delBtn"
                      :disabled="scope.row.documentStatus == 'draft' ? false : true"
    ...
    <el-dialog :title="'更多查询'" :close-on-click-modal="false" :close-on-press-escape="false" :visible.sync="visible"
      ...
      <el-row :gutter="20">
        ...
        <el-form ref="diaForm" :model="linesQuery" label-width="120px" label-position="top">
          <el-col :span="12">
            <el-form-item label="停止单号">
              <el-input v-model="linesQuery.orderNo" placeholder="请输入停止单号" clearable />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="订单号">
              <el-input v-model="linesQuery.saleOrderNo" placeholder="请输入订单号" clearable />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="客户产品图号">
              <el-input v-model="linesQuery.customerProductDrawingNo" placeholder="请输入来源单号" clearable />
            </el-form-item>
          </el-col>
          ...
          <el-col :span="12">
            <el-form-item label="停止日期">
              <el-date-picker v-model="linesQuery.stopDateArr" type="daterange" value-format="yyyy-MM-dd"
                style="width: 100%;" clearable start-placeholder="请选择开始日期" end-placeholder="请选择结束日期"
...
<script>
import { getQuotationList, deleteApplyStopOrders, ordersStopLineList } from '@/api/salesManagement/stopTicket'
...
export default {
  ...
  methods: {
    ...
    sortChange({ prop, order }) {
      let newProp;
      if (prop === 'saleOrderNo' || prop === "productCode" || prop === "productName" || prop === "productDrawingNo" || prop === "productDrawingNo" ) {
        newProp = prop
      } else {
      ...
      if (this.activeName == "dataTable") {
        ...
        if (prop === "orderNo"|| prop === "createTime") {
          newProp = prop
    ...
    initData() {
      ...
      // getQuotationList(this.dataForm).then(res => {
      //   console.log("res++", res);
      ...
      if (this.activeName === 'dataTable') {
        ...
        if (this.linesQuery.stopDateArr && this.linesQuery.stopDateArr.length > 0) {
          this.linesQuery.stopStartDate = this.linesQuery.stopDateArr[0]
          this.linesQuery.stopEndDate = this.linesQuery.stopDateArr[1]
        } else {
          this.linesQuery.stopStartDate = ''
          this.linesQuery.stopEndDate = ''
        }
        ...
        ordersStopLineList(this.linesQuery).then(res => {
          console.log("res++", res);
    ...
    handleDel(id) {
      this.$confirm(this.$t('common.delTip'), this.$t('common.tipTitle'), {
        ...
        deleteApplyStopOrders(id).then(res => {
          this.initData()
          this.$message({
            type: 'success',
            message: "删除成功",
