      ...
      <div class="JNPF-preview-main org-form">
        <div :class="['JNPF-common-page-header', type === 'look' ? 'noButtons' : '']">
          <el-page-header @back="goBack"
            :content="type == 'add' ? '新建工序外协退货通知单' : type == 'edit' ? '编辑工序外协退货通知单' : type == 'look' ? '查看工序外协退货通知单' : '新建工序外协退货通知单'" />
            ...
            <el-tab-pane label="基础信息" name="jcInfo">
              ...
              <el-row :gutter="15" class="">
                <el-form ref="elForm" :model="dataForm" :rules="rules" size="small" label-width="100px"
                  ...
                  <el-col :sm="8" :xs="24" v-if="type === 'look'">
                    <el-form-item label="退货完成时间" prop="deliveryCompletionDate" ref="deliveryCompletionDate">
                      <el-time-select style="width: 100%;" :disabled="type == 'look' ? true : false"
                        v-model="dataForm.deliveryCompletionDate" placeholder="退货完成时间">
                      </el-time-select>
                    </el-form-item>
                  </el-col>
                  <el-col :span="8" v-if="type === 'look'">
                    <el-form-item label="退货状态" prop="receivingStatus" ref="receivingStatus">
                      <el-input type="text"
                        v-model="dataForm.receivingStatus === 'returning' ? '未完成' : dataForm.receivingStatus === 'returned' ? '已完成' : '已停止'"
                        placeholder="退货状态" :disabled="type != 'add' ? true : false">
                      </el-input>
                    </el-form-item>
                  </el-col>
                  <!-- <el-col :span="8" v-if="type === 'look'">
              ...
              <el-form :model="dataFormTwo" v-bind="dataFormTwo" ref="productForm">
                <el-table style="border: 1px solid #e3e7ee;" hasNO fixedNO v-bind="dataFormTwo.data"
                  ...
                  <el-table-column prop="routingIdName" label="工艺路线名称" min-width="200">
                    <template slot-scope="scope">
                  ...
                  <el-table-column prop="processIdName" label="工序名称" min-width="200">
                    <template slot-scope="scope">
                  ...
                  <el-table-column prop="receiptQuantity" :label="type === 'look' ? '已入库数量' : '已出库数量'" min-width="180">
                    <template slot-scope="scope">
                      <el-form-item prop="receiptQuantity">
                        <!-- <el-input v-model="scope.row.receiptQuantity" readonly :disabled="type === 'look'" maxlength="20"
                          placeholder="入库数量">{{
                  ...
                  <!-- <el-table-column prop="receivingStatus" label="退货状态" width="130" v-if="type === 'look'" key="receivingStatus">
                    <template slot-scope="scope">
  ...
  data() {
    return {
      ...
      dataForm: {
        ...
        receivingStatus: "returning",              //收货状态
        remark: "",
        salesman: "",
        deliverDate: '',
        createTime: '',
        ...
        if (!this.dataForm.id) {
          ...
          getpurPurchaseReceiptReturnGoodsdetail(this.dataForm.id).then(res => {
            ...
            if (this.type === 'copy') {
              ...
              this.dataForm.receivingStatus = "returning"
                  ...
                  linesData.forEach(item => {
                    _data.forEach(item2 => {
                      if (item.id === item2.ordersLineId) {
                        ...
                        item2.returnQuantity = item.returnQuantity ? item.returnQuantity : 0
                        item2.receivedQuantity = ''
                        item2.id = ''
                        item2.receivingStatus = 'returning'
      ...
      if (this.type === 'add' || this.type === 'copy' || this.type === 'edit') {
        _dataFormTwo = this.dataFormTwo.data.map(item => {
          return {
            ...
            qualifiedQuantity: item.receivedQuantity,
          }
        })
      }
      if (this.type == 'add' || this.type === 'copy' || this.type === 'edit') {
