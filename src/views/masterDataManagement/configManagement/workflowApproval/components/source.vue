<template>
  <div>
    <el-drawer :title="types === 'look' ? '查看发料清单' : '配置发料清单'" :visible.sync="drawer" :direction="direction" :wrapperClosable="false"
      ...
      <div class="JNPF-flex-main">
        ...
        <el-scrollbar class="column-list">
          <div v-if="types !== 'look'">
            <el-button type="text" class="topButton" icon="el-icon-plus" @click="openProductDialog('product')">选择产品</el-button>|
            <el-button type="text" class="topButton" icon="el-icon-plus" @click="openProductDialog('process')">选择工序产品</el-button>|
            <el-button type="text" class="topButton" icon="el-icon-delete" @click="batchDelete">批量删除</el-button>|
          </div>

          <el-form :model="dataFormTwo" v-bind="dataFormTwo" ref="productForm">
            <el-table hasNO fixedNO v-bind="dataFormTwo.data" @selection-change="handeleProductInfoData" hasC :data="dataFormTwo.data" id="table"
              style="width: 100%">
              ...
              <el-table-column label="操作" width="90" fixed="right" v-if="types !== 'look'">
                <template slot-scope="scope">
                  <el-button type="text" class="JNPF-table-delBtn" :disabled="types === 'look'" @click="deleteth(scope.$index)">
                    {{ $t(`common.delBtn`) }}
                  </el-button>
                </template>
              </el-table-column>
    ...
    <ComSelect-page ref="ComSelect-page" @change="addth" :tableItems="type==='product' ? ProductTableItems : ProductTableItems2" title="选择产品" treeTitle="产品分类"
  ...
  methods: {
    ...
    openProductDialog(type) {
      ...
      if (type === 'product') {
        ...
        if (this.routingId) {
          this.ProductListRequestObj.queryType = 3
          this.ProductListRequestObj.routingId = this.routingId
          this.$refs['ComSelect-page'].openDialog()
        }
      }
