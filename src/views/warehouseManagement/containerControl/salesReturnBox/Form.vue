      ...
      <div class="JNPF-preview-main org-form">
        <div :class="['JNPF-common-page-header', type === 'look' ? 'noButtons' : '']">
          <el-page-header @back="goBack"
            :content="type == 'add' ? '新建销售退货装箱单' : type == 'edit' ? '编辑销售退货装箱单' : '查看销售退货装箱单'" />
          ...
          <el-tabs v-model="activeName" @tab-click="handleClick">
            <el-tab-pane label="基础信息" name="jcInfo">
              ...
              <el-row :gutter="30" class="">
                <el-form ref="elForm" :model="dataForm" :rules="rules" size="small" label-width="100px"
                  ...
                  <el-col :sm="6" :xs="24">
                    <el-form-item label="销售退货单号" prop="sourceOrderNo" ref="sourceOrderNo">
                      <ComSelect-page :beforeSubmit="beforeSubmit" :renderTree="false" :isdisabled="type === 'look'"
                        ...
                        :tableItems="purchaseTableItems" :placeholder="'请选择销售退货单号'" title="选择销售退货单号"
                  ...
                  <el-col :sm="6" :xs="24" v-if="dataForm.packingType === 'mixed'">
                    <el-form-item label="标准箱" prop="standardBoxName">
                      <ComSelect-page clearable :beforeSubmit="beforeSubmit1" :value="dataForm.standardBoxName"
                  ...
                  <el-col :sm="6" :xs="24" v-if="dataForm.packingType === 'mixed'">
                    <el-form-item label="零头箱" prop="oddBoxName">
                      <ComSelect-page clearable :beforeSubmit="beforeSubmit2" :value="dataForm.oddBoxName"
                        :renderTree="false" :isdisabled="type == 'look'" placeholder="请选择产品名称" ref="ComSelect-page"
                        @change="changeStandard" :tableItems="ProductTableItems" title="选择产品" treeTitle="产品分类"
                        :methodArr="ProductMethodArr" :listMethod="getProductList" :listRequestObj="ProductListRequestObj"
                        :searchList="ProductTableSearchList" :elementShow="true"
            ...
            <el-tab-pane label="装箱条码" name="boxCode">
              <CodeTable @lookCode="lookCode" @delth="delequipment_process_relList" :tableData="tableData"
                ...
                }" :btnType="type" :show="activeName === 'boxCode'" />
            </el-tab-pane>
          </el-tabs>
        </div>
        <codeForm :standardBoxName="dataForm.standardBoxName" :oddBoxName="dataForm.oddBoxName"
          ...
          @codeData="mixCodeData" />
        <sourceForm v-if="sourceFormVisible" ref="sourceForm" />
      </div>
    </transition>
  </div>
</template>
  ...
  data() {
    return {
      ...
      purPurchaseTableSearchList: [
        { prop: "orderNo", label: "销售退货单号", type: 'input' },
        { prop: "deliverDate", label: "退货日期", type: 'input' },
      ],
      activeName: 'jcInfo',
      btnLoading: false,
      index: null,
  ...
  methods: {
    ...
    async beforeSubmit(data, paramsObj) {
      ...
      if (paramsObj.oldData.length) {
        flag = await this.$confirm('已选择过销售退货单号，是否切换，切换后会清空供应商信息和产品信息是否继续？', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
        ...
        getQuotationsendlist(this.dataForm.documentId).then(res => {
          ...
          res.data.noticeLineList.forEach(item => {
            _data.push({
              ...
              packedQuantity: '',         // 待装箱
              waitPackingQuantity: this.jnpf.math('subtract',[item.deliveryQuantity,item.scrapQuantity,item.packingQuantity]),
              totalNum: this.jnpf.math('subtract',[item.deliveryQuantity,item.scrapQuantity,item.packingQuantity]),
