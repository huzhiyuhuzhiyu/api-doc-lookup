        ...
        <el-table-column label="操作" width="140" :fixed="hasFixed ? 'right' : false">
          <template slot-scope="scope">
            ...
            <el-button size="mini" type="text" @click="handlerOpenSource(scope)"
              :style="{ color: scope.row.allocated ? '#67c23a' : '#1890ff' }">货位设置</el-button>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    UploadProduct(data) {
      ...
      // 调用上传文件接口
      ioBoundUpload(formData, 'outbound').then(res => {
    ...
    downLoadTemplate() {
      ...
      a.setAttribute('href', location.origin + '/static/直接出库导入模板.xlsx')
      a.click()
    },
  }
}
