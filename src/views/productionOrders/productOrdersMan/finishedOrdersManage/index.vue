...
<template>
    ...
    <div class="JNPF-common-layout-center JNPF-flex-main">
      <el-row class="JNPF-common-search-box" :gutter="16">
        <el-form @submit.native.prevent>
          ...
          <el-col :span="4">
            <el-form-item>
              <el-input v-model="orderForm.blankProductsDrawingNo" @keyup.enter.native="dataFormSubmit()"
                placeholder="请输入下料图号" clearable />
            </el-form-item>
          </el-col>
          <el-col :span="4">
            <el-form-item>
      ...
      <div class="JNPF-common-layout-main JNPF-flex-main ">
        <div class="JNPF-common-head">
          <div>
            <el-button size="mini" @click="batchBlanking" type="primary">批量下料</el-button>
        ...
        <JNPF-table ref="dataTable"  :select-on-click-row="true" v-loading="listLoading" :data="tableData" @sort-change="sortChange" custom-column
  ...
  @select="handleSelection" @row-click="clickRow">
        ...
        scope.row.orderNo
          ...
          <el-table-column prop="blankStatus" label="下料状态" width="110" fixed="right">

            <template slot-scope="scope">
              <el-tag type="warning" disable-transitions
                v-if="scope.row.blankStatus === 'not_finished'">未完成</el-tag>

              <el-tag type="success" disable-transitions v-if="scope.row.blankStatus === 'finished'">已完成</el-tag>
            </template>
          </el-table-column>
          ...
          <!-- <el-table-column prop="receiveStatus" label="接收状态" width="110" sortable="custom" fixed="right">
              <template slot-scope="scope">
                ...
                <el-tag type="success" disable-transitions v-else-if="scope.row.receiveStatus === 'received'">已接收</el-tag>
                <el-tag type="danger" disable-transitions v-else-if="scope.row.receiveStatus === 'refused'">已拒绝</el-tag>
              </template>
            </el-table-column>
          ...
          <el-table-column prop="blankProductsName" label="下料产品名称" min-width="160"
            show-overflow-tooltip></el-table-column>
          <el-table-column prop="blankProductsDrawingNo" sortable="custom" label="下料图号" width="120"></el-table-column>
          <el-table-column prop="blankingQuantity" label="已派下料数量" min-width="140"
            show-overflow-tooltip></el-table-column>
          ...
          <el-table-column prop="insertOrderFlag" label="是否插单" width="120" sortable="custom">
            <template slot-scope="scope">
    ...
    <batchDispatchForm v-if="batchDispatchFormVisibles" ref="batchDispatchForms" @refreshDataList="initData"
      @close="closeForm" />
    <blankingForm v-if="blankingFormVisibles" ref="blankingForms" @refreshDataList="initData" @close="closeForm" />
    <!-- 更多查询弹框 -->
    <el-dialog :title="title" :close-on-click-modal="false" :close-on-press-escape="false" :visible.sync="visible"
      ...
      <el-row :gutter="20">
        ...
        <el-form ref="diaForm" :model="orderForm" label-width="120px" label-position="top">
          ...
          <el-col :span="12">
            <el-form-item label="下料图号">
              <el-input v-model="orderForm.blankProductsDrawingNo" placeholder="请输入下料图号" clearable />
            </el-form-item>
          </el-col>
          <el-col :span="12">
          ...
          <el-col :span="12">
            <el-form-item label="下料状态">
              <el-select v-model="orderForm.blankStatus" placeholder="请选择下料状态" clearable>
                <el-option v-for="(item, index) in blankStatusList" :key="index" :label="item.label"
                  :value="item.value"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="派工状态">
              <el-select v-model="orderForm.dispatchStatus" placeholder="请选择下料派工状态" clearable>
...
import blankingForm from './blankingForm'
...
export default {
  name: 'carrierProfile',
  components: { Form, TableFormProduct, Forms, batchDispatchForm, blankingForm },
  data() {
    return {
      hasfixed:true,
      sourceList: [],
  ...
  methods: {
    ...
    batchBlanking() {
      if (this.selectArr.length) {
        let flag = this.checkProductIds(this.selectArr)
        console.log("flag", flag);
        if (!flag) return this.$message.error("批量下料必须是下料产品都一样")
        let arr = this.selectArr.filter(item => item.blankingProductsId && item.orderStatus == 'normal')
        for (let index = 0; index < arr.length; index++) {
          const item = arr[index];
          if (Number(item.blankingQuantity) >= Number(item.productionQuantity)) {
            return this.$message({
              message: "订单" + item.orderNo + "已派下料数量等于生产数量，无法进行下料",
              type: "error"
            })
        ...
        this.blankingFormVisibles = true
        this.$nextTick(() => {
          this.$refs.blankingForms.init(arr)
        })
      } else {
        this.$message({
          message: "请先选择数据",
        ...
        if (b.length) {
          ...
          for (let index = 0; index < b.length; index++) {
            ...
            if (Number(item.dispatchedQuantity) >= Number(item.blankingQuantity)) {
              this.$message({
                message: "订单" + item.orderNo + "派工数量等于下料数量，无法进行派工",
                type: "error"
              })
              submitFlag = false
              flag = true
