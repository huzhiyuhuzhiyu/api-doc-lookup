<template>
  <div class="JNPF-common-layout">
                ...
                <el-table-column prop="orderNo" label="入库单号" min-width="200" sortable="custom">
                  <template slot-scope="scope">
                    <el-link type="primary"
                      @click.native="addOrUpdateHandle(scope.row.id, true, scope.row.sourceType, scope.row.sourceNo)">
                      {{ scope.row.orderNo }}
                    </el-link>
                  </template>
                </el-table-column>
            ...
            <div class="JNPF-common-layout-main JNPF-flex-main">
              ...
              <JNPF-table v-loading="listLoading" :data="linesTableData" fixedNO @sort-change="sortChange" custom-column
                ...
                <el-table-column prop="orderNo" label="入库单号" min-width="200" sortable="custom">
                  <template slot-scope="scope">
                    <el-link type="primary"
                      @click.native="addOrUpdateHandle(scope.row.moveId, true, scope.row.sourceType, scope.row.sourceNo)">
                      {{ scope.row.orderNo }}
                    </el-link>
                  </template>
                </el-table-column>
    ...
    <InboundOrdersForm v-if="inboundOrdersFormVisible" ref="InboundOrdersForm" @close="closeForm"
      :originTypeList="originTypeList" />
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    addOrUpdateHandle(id, btnType, sourceType, orderNo) {
      if (['inbound_order_production', 'inbound_production'].includes(sourceType) && btnType && !orderNo) { // 批量入库的数据
        this.inboundOrdersFormVisible = true
        this.$nextTick(() => { this.$refs.InboundOrdersForm.init(id, btnType, sourceType + '_batch', id) })
      } else if (['inbound_order_production', 'inbound_production'].includes(sourceType)) {
        this.inboundOrdersFormVisible = true
