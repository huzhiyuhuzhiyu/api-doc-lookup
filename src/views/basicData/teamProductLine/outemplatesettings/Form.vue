      ...
      <div class="contain">
        <div class="JNPF-common-layout-center JNPF-flex-main" v-loading="formLoading">
          <div class="JNPF-common-layout-main JNPF-flex-main">
            ...
            <div v-if="dataForm.objectType === 'custom'">
              <div class="subtitle">
                <h5>客户信息</h5>
              </div>
              <TableForm :value="linesList" @input="contentChanges" ref="linesForm" :tableItems="linesListItems" :btnType="btnType" @addth="addOrDelLinesItem" @deleteth="addOrDelLinesItem" />
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  beforeCreate() {
    this.templateList = [
      { label: '默认模板', value: '001' },
      { label: '卡尔玛模板', value: '002' }
    ]
  },
  data() {
    return {
      src: '',
      optionsobjectType: [{ label: '客户', value: 'custom' }, { label: '默认', value: 'default_template' }],
      title: "",
      btnType: "",
      btnLoading: false,
      formLoading: true,
  ...
  methods: {
    ...
    init(id, btnType) {
      ...
      this.dataForm.templateName = '默认模板'
      this.changeclick('001')
      this.title = '新建标签打印模板'
      this.refeshLinesListItems()
    },
    async handleConfirm(submitModel) {
      ...
      if (submitFlag) {
        let dataObj = []
        if (this.dataForm.objectType === 'custom') {
          dataObj = this.linesList.map(line => {
            return {
