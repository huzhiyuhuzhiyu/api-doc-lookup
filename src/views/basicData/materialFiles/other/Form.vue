  ...
  <transition name="el-zoom-in-center">
    <div class="JNPF-preview-main org-form">
      <div :class="['JNPF-common-page-header', btnType ? 'noButtons' : '']">
        <el-page-header @back="goBack" :content="btnType ? '查看其他物品档案' : title" />
...
export default {
      ...
      tab.tabContent.forEach(tc => {
        ...
        if (tc.type == "custom") {
          ...
          if (tc.prop === 'productCategoryIdText') {
            ...
            tc.requestObj = { classAttribute: "other" }
            tc.dialogTitle = '选择其他档案分类'
          }
          // 材质
          ...
          tc.itemRules.push({
            validator: (rule, value, callback) => {
              if (!value) { callback() }
                ...
                checkCodeExist({ classAttribute: "other", code: this.dataForm.code }).then((res) => {
  ...
  computed: {
    openMode() {
      return this.title === '新建其他物品档案' ? '新建' : this.title === '编辑其他物品档案' ? '编辑' : '只读'
    }
  },
  methods: {
    // 渲染自定义属性表单
    ...
    init(id, btnType = false) {
      ...
      if (!!id) {
        this.dataForm.id = id
        this.title = btnType ? '查看其他物品档案' : '编辑其他物品档案'
        // 获取详情
