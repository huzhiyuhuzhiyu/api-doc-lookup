    ...
    <div class="JNPF-common-layout-center JNPF-flex-main">
      <el-row class="JNPF-common-search-box" :gutter="16">
        <el-form @submit.native.prevent>
          <el-col :span="4">
            <el-form-item>
              <el-input @keyup.native.enter="search()"  v-model="listQuery.code" placeholder="请输入模具编码" clearable />
            </el-form-item>
          </el-col>
          <el-col :span="4">
            <el-form-item>
              <el-input @keyup.native.enter="search()"  v-model="listQuery.name" placeholder="请输入模具名称" clearable />
            </el-form-item>
          </el-col>
          <el-col :span="4">
            <el-form-item>
              <el-select v-model="listQuery.bindProdFlag" placeholder="请选择是否有绑定产品" align="center">
                <el-option label="已有绑定产品" :value="true"></el-option>
                <el-option label="未绑定产品" :value="false"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
          <!-- <el-col :span="4">
            <el-form-item>
              <el-select v-model="listQuery.state" placeholder="请选择模具状态" clearable>
      ...
      <div class="JNPF-common-layout-main JNPF-flex-main" v-loading="listLoading">
          ...
          <div>
            <!-- <topOpts @add="addOrUpdateHandle('', false, 'add')">
              <el-button size="mini" type="primary" icon="el-icon-printer"
                @click="printTool('p039')">打印模具二维码</el-button>
        ...
        <JNPF-table v-if="istable" :data="tableData" :fixedNO="true" :customColumn="true" ref="dataTable"
          @sort-change="sortChange" :setColumnDisplayList="setColumnDisplayList" custom-column hasC @selection-change="handleSelectionChange" customKey="JNPFTableKey_513051">
          <el-table-column prop="code" label="模具编码" min-width="200" />
          <el-table-column prop="name" label="模具名称" min-width="200" sortable="custom"></el-table-column>
          <el-table-column prop="bindProdFlag" label="是否有绑定产品" width="200" sortable="custom" align="center">
            <template slot-scope="scope">
              <el-tag v-if="scope.row.bindProdFlag" type="success">是</el-tag>
              <el-tag v-else type="danger">否</el-tag>
            </template>
          </el-table-column>
          <el-table-column prop="bindingUserName" label="绑定人" min-width="120" />
          <el-table-column prop="bindingTime" label="绑定时间" min-width="160" />
          <el-table-column label="操作" width="180" fixed="right">
            <template slot-scope="scope">
              <tableOpts @edit="addOrUpdateHandle(scope.row, 'edit')" editText="绑定产品" :hasDel="false"
...
export default {
  ...
  name: 'taskConfig',
  components: { Form, Diagram, PrintBrowse, SuperQuery, ExportForm },
  data() {
    return {
      setColumnDisplayList: ['bindingUserName', 'bindingTime'],
      fileList: [],
      loadingText: '',
      file: {},
      uploadVisib: false,
  ...
  methods: {
    ...
    downLoadTemplate() {
      ...
      a.setAttribute('href', location.origin + '/static/模具导入模板.xlsx')
      a.click()
    },
    UploadProduct(data) {
      ...
      //调用上传文件接口
      toolupload(formData, 'mold').then(res => {
        if (!res.data) {
    ...
    addOrUpdateHandle(row, btnType) {
      ...
      this.$nextTick(() => {
        this.$refs.Form.init(row, btnType)
      })
