    ...
    <div class="JNPF-preview-main org-form">
      <div :class="['JNPF-common-page-header', btnType === 'look' ? 'noButtons' : '']">
        ...
        <el-page-header @back="goBack"
          :content="btnType == 'add' ? '新建生产退料通知单' : btnType == 'edit' ? '编辑生产退料通知单' : btnType == 'copy' ? '复制生产退料通知单' : '查看生产退料通知单'" />
          ...
          <el-tab-pane label="通知单信息" name="orderInfo">
              ...
              <el-row :gutter="30" class="custom-row">
                <el-col :sm="8" :xs="24">
                  <el-form-item label="退料单号" prop="orderNo">
                    <el-input readonly placeholder="请选择退料单号" :disabled="btnType == 'look' ? true : false"
                      v-model="dataForm.orderNo" @focus="openSeleceproduction"></el-input>
                  </el-form-item>
                </el-col>
                <el-col :sm="8" :xs="24">
                ...
                <el-col :sm="8" :xs="24">
                  <el-form-item label="退料日期" prop="operationDate">
                    <el-date-picker v-model="dataForm.operationDate" type="date" value-format="yyyy-MM-dd"
                      style="width: 100%;" placeholder="请选择退料日期" :disabled="btnType == 'look' ? true : false">
                    </el-date-picker>
                  </el-form-item>
                </el-col>
                <el-col :sm="24" :xs="24">
            ...
            <el-form :model="dataFormTwo" v-bind="dataFormTwo" ref="productForm" class="data-form">
              <el-table ref="product" :data="dataFormTwo.productData" v-bind="dataFormTwo.data" hasC hasNO fixedNO
                ...
                <el-table-column prop="receivedQuantity" label="已领数量" width="auto" show-overflow-tooltip>
                </el-table-column>
                <el-table-column prop="returnedQuantity" label="已退数量" width="auto" show-overflow-tooltip>
                </el-table-column>
                <el-table-column prop="num" label="退料数量" width="170">
                  <template slot="header">
                    <span class="required">*</span>退料数量
                  </template>
                  <template slot-scope="scope">
                    <el-form-item :prop="'productData.' + scope.$index + '.' + 'num'" :rules='productRules.num'>
                      <el-input v-model="scope.row.num" placeholder="请输入退料数量" :disabled="btnType == 'look'" maxlength="11"
  ...
  methods: {
    ...
    submitCustomerProduct(selectedIds, selectedList) {
      ...
      detailordershengchan(selectedIds).then(res => {
        ...
        if (this.dataForm.orderNo && res.msg == 'Success') {
          this.$confirm('此操作将切换产品数据,是否继续？', '提示', {
            ...
            this.dataForm.orderNo = selectedList[0].all.orderNo
            ...
            res.data.materialList.forEach(item => {
              this.dataFormTwo.productData.push({
                ...
                productionOrderId: selectedIds,
                materialListId: item.id,
          ...
          this.$nextTick(() => {
            ...
            if (this.btnType == 'copy') {
              ...
              detailordershengchan(res.data.collect.productionOrderId).then(row => {
                this.dataFormTwo.productData = row.data.materialList
              })
            } else {
        ...
        this.$refs['productForm'].validate((valid) => {
          if (valid) {
            ...
            let obj = {
              ...
              collectLineList: this.dataFormTwo.productData
            }
