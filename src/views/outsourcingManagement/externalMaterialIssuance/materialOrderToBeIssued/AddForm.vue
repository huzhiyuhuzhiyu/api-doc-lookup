                      ...
                      <el-col :sm="8" :xs="24" v-if="btnType == 'look'">
                        <el-form-item label="发货完成时间" prop="deliveryCompletionDate">
                          <el-date-picker v-model="dataForm.deliveryCompletionDate" type="datetime"
                            placeholder="请选择发货完成时间" :disabled="btnType == 'look'" style="width: 100%;"
    ...
    return {
      ...
      productRules: {
        deliveryQuantity: [
          ...
          {
            validator: this.formValidate('positiveNumber', '发货数量必须大于0', (errMsg, index) => {
  ...
  methods: {
    ...
    getAddressInfoFun(id) {
      getAddressInfo(id).then((row) => {
        let dfaddress = row.data.filter((item) => {
        ...
        if (dfaddress.length) {
          ...
          if (this.dataForm.country === 'CN') {
            this.defaultAddress =
              dfaddress[0].countryText +
              dfaddress[0].provinceText +
              dfaddress[0].cityText +
              dfaddress[0].areaText +
              dfaddress[0].address
          } else {
    ...
    init(id, btnType, data) {
      ...
      if (data) {
        ...
        this.ProductListRequestObj.cooperativePartnerId = data[0].cooperativePartnerId
          ...
          if (!this.dataForm.exchangeGoodsFlag && isQuantity) {
            this.$confirm(`总发货数量大于订单数量,是否继续？`, '提示', {
              ...
              .then(() => {
                ...
                let formMethod = null
                if (this.btnType == 'edit') {
                  formMethod = updateoutReceiptGoods
                } else if (this.btnType == 'add' || this.btnType == 'copy') {
