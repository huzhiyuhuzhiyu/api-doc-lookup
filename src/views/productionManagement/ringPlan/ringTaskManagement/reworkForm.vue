                    ...
                    <el-tabs v-model="activeName" @tab-click="handleClick" class=".el-table">
                            ...
                            <el-collapse v-model="activeNames">
                                        ...
                                        <el-row :gutter="20" class="custom-row">
                                            ...
                                            <el-col :sm="6" :xs="24">
                                                <el-form-item label="工艺路线名称" prop="routingName">
                                                    <el-input v-model="dataForm.routingName" placeholder="工艺路线名称"
                                                              @focus="openRoutingFun" @clear="clear" :disabled="preData" clearable></el-input>
                                                </el-form-item>
                                            </el-col>
                                            <el-col :sm="6" :xs="24">
                                            ...
                                            <el-col :sm="6" :xs="24">
                                                <el-form-item label="返工生产数量" prop="productionQuantity">
                                                    <el-input v-model="dataForm.productionQuantity" placeholder="返工生产数量" :disabled="preData">
                                                    </el-input>
                                                </el-form-item>
                                            </el-col>
                                            <el-col :sm="6" :xs="24">
                                ...
                                <el-collapse-item title="工序信息" name="productInfo" style="border-bottom: 1px solid #dcdfe6 !important;">
                                    <div  style="height:34px">
                                        <div style="float: left"  v-if="btnType !== 'look'">
                                    ...
                                    <el-form :model="dataFormTwo" v-bind="dataFormTwo" ref="processRef" class="data-form">
                                        <JNPF-table ref="products" :data="dataFormTwo.data" v-loading="tableloading" hasC :height="customStyleData"
                                                    @selection-change="delProcessFun" :partentOrChild="'childrenLineTwo'" custom-column style="width: 100%;" fixedNO fixedSelect>
                        ...
                        <el-tab-pane label="领料信息" name="annex">
                            <el-collapse v-model="activeNames2">
                                <el-collapse-item title="基本信息" name="pickbasicInfo" class="orderInfo" v-if="$store.getters.configData.produce.enable_rework_arrange_auto_picking">
...
export default {
    ...
    data() {
        return {
            ...
            activeNames2: ['pickInfo', 'pickbasicInfo'],
            collect: {
                orderNo: "",
                operationDate: "",
                personId: "",
    ...
    methods: {
        ...
        init(id, btnType) {
            if (id) {
                this.preData = id
                const repairQuantity = this.preData.reduce((result, item) => this.jnpf.math('+', [result, item.repairQuantity]), 0)
                const reworkRepairQuantity = this.preData.reduce((result, item) => this.jnpf.math('+', [result, item.reworkRepairQuantity]), 0)

                this.dataForm = {
                    ...this.preData[0],
                    id: this.preData[0].productsId,
                    drawingNo: this.preData[0].productDrawingNo,
                    taskMethod: 'not_appoint',
                    productionQuantity: this.jnpf.math('-', [repairQuantity, reworkRepairQuantity]),
                    remark: '',
                }
                detailProcess(this.preData[0].routingId).then(res => {
                    const selectArr = res.data.routingLineList.filter(item => item.processId === this.preData[0].processId)
                    ...
                    selectArr.forEach((item, index) => {
                        ...
                        if (index == 0) {
                            item.firstFlag = true;
                            item.lastFlag = false;
                        } else if (index == selectArr.length - 1) {
                            item.firstFlag = false;
                            item.lastFlag = true;
                            item.stockFlag = true;
                            if (selectArr[index].processingType !== "self_produced") {
                        ...
                        if (item.processingType !== "self_produced" && selectArr.length > 1) {
                            ...
                            selectArr[index - 1].stockFlag = true;
                            selectArr[index - 1].reportFlag = true;
                        }
                    });
                    if (selectArr.length == 1) {
                        selectArr[0].firstFlag = true;
                        selectArr[0].lastFlag = true;
                        selectArr[0].stockFlag = true;
                        if (selectArr[0].processingType !== "self_produced") {
                            selectArr[0].reportFlag = false;
                        } else {
                            selectArr[0].reportFlag = true;

                        }
                    }
                    this.dataFormTwo.data = selectArr
                })
                id = void (0)
            }
        ...
        handleConfirm(value) {
            ...
            this.$refs['dataForm'].validate((valid) => {
                ...
                if (valid) {
                    ...
                    if (this.preData) {
                        obj.unqualifiedList = this.preData.map(item => ({
                            ...item,
                            reworkRepairQuantity: item.repairQuantity
                        }))
                    }
