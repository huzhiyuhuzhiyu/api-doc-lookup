<template>
  <el-dialog :title="!dataForm.id ? '新建数据表分类' : '编辑数据表分类'" :close-on-click-modal="false" :close-on-press-escape="false"
    ...
    <el-form ref="dataForm" v-loading="formLoading" :model="dataForm" :type="dataForm.type" :rules="dataRule"
      ...
      <el-form-item label="上级分类" prop="parentName">
        ...
        <ComSelect-list  v-model="dataForm.parentName" placeholder="请选择上级分类" auth
          @change="onOrganizeChange" :title="'选择上级分类'" :method="getDataCatogryList" :requestObj="requestObjTwo"
          :paramsObj="{}" />
      </el-form-item>
...
export default {
  ...
  methods: {
    init(id, parentId,btntype) {
      ...
      this.$nextTick(() => {
        if (this.dataForm.id) {
          detailDataCatogryList(this.dataForm.id).then(res => {
    ...
    async dataFormSubmit() {
      ...
      if (valid) {
        ...
        let formMethod = this.dataForm.id ? editDataCatogryList : addDataCatogryList
