    ...
    <transition name="el-zoom-in-center">
        <div class="JNPF-preview-main org-form">
            <div class="JNPF-common-page-header">
                ...
                <el-page-header @back="goBack" content="批量派工" />
          ...
          ">
                    ...
                    <el-row :gutter="30" class="custom-row">
                        <el-col :sm="12" :xs="24">
                            <el-form-item label="工艺路线编码" prop="routingCode">
                                <el-input readonly placeholder="请输入工艺路线编码" disabled
                                    v-model="reworkForm.routingCode"></el-input>
                            </el-form-item>
                        </el-col>
                        ...
                        <el-col :sm="6" :xs="24">
                            <el-form-item label="计划结束日期" prop="planEndDate">
                                <el-date-picker v-model="reworkForm.planEndDate" type="date" value-format="yyyy-MM-dd"
                                    style="width: 100%" placeholder="计划结束日期">
                                </el-date-picker>
                ...
                <el-form :model="dataFormTwo" v-bind="dataFormTwo" ref="productForm">
                    <JNPF-table ref="product" class="tableContainer" v-bind="dataFormTwo.prodOrderList"
                        :data="dataFormTwo.prodOrderList" :fixedNO="false" style="width: 100%">
                        <el-table-column prop="orderNo" label="生产订单号" min-width="200"></el-table-column>
                        <el-table-column prop="productCode" label="产品编码" min-width="130" />
                        <el-table-column prop="productName" label="产品名称" min-width="130" />
                        <el-table-column prop="productDrawingNo" label="产品图号" min-width="130" />
                        <el-table-column prop="deliveryDate" label="交货日期" min-width="130" />
                        <el-table-column prop="mainUnit" label="单位" min-width="130" />
                        <el-table-column prop="productionQuantity" label="需生产数量" min-width="130" />
                        <el-table-column prop="completedQuantity" label="已完成数量" min-width="130" />
                        <el-table-column prop="blankingQuantity" label="已派下料数量" min-width="130"></el-table-column>
                        <el-table-column prop="dispatchedQuantity" label="已派工数量" min-width="130" />
                        <el-table-column prop="dispatchQuantity" label="派工数量" min-width="130">
                            <template slot="header"> <span class="required">*</span>派工数量 </template>

                            <template slot-scope="scope">
                                <el-form-item :prop="'prodOrderList.' + scope.$index + '.' + 'dispatchQuantity'"
                                    :rules="productRules.dispatchQuantity">
                                    <el-input v-model="scope.row.dispatchQuantity" maxlength="20"
                                        placeholder="请输入派工数量">{{ scope.row.dispatchQuantity }}
                                    </el-input>
                                </el-form-item>
                            </template>
                        </el-table-column>
                        <el-table-column label="操作" min-width="120" fixed="right"
                            v-if="dataFormTwo.prodOrderList.length > 1">
                ...
                <el-table ref="product" :data="workerList" :fixedNO="false" style="width: 100%" border class="objName">
                    ...
                    <el-table-column prop="equipmentId" label="设备" min-width="150">
                        <template slot-scope="scope">
                            ...
                            <el-button @click="selectDeviceFun(scope)" type="text" class="underline-button" :disabled="scope.row.processingType != 'self_produced' ||(scope.row.firstFlag && dataFormTwo.prodOrderList[0].blankingProductsId? true: false)">
                                {{scope.row.equipmentId ? scope.row.equipmentName+"("+scope.row.equipmentCode+')' : "请选择设备"}}
                             </el-button>
                        </template>
                    </el-table-column>
...
export default {
                    ...
                    { required: true, trigger: ["blur"], message: "请输入派工数量" },
                    {
                        validator: this.formValidate({
                            ...
                            params: [
                                ...
                                "",
                                (errMsg, index) => {
                                    this.$message.error("第" + (index + 1) + "行：派工数量" + errMsg);
                                },
                            ],
                        }),
                        trigger: "blur",
    ...
    methods: {
        // 开始日期前一天
        previousDayS(){
            if(!this.reworkForm.planStartDate) return this.$message.error("请先选择日期")
            let date = new Date( this.reworkForm.planStartDate);
                    date.setDate(date.getDate() - 1);
                     this.reworkForm.planStartDate = date.toISOString().split('T')[0];
                     console.log(this.reworkForm.planStartDate);
        ...
        nextDayS(){
         ...
         // 结束日期前一天
         previousDayE(){
            if(!this.reworkForm.planEndDate) return this.$message.error("请先选择日期")
            let date = new Date( this.reworkForm.planEndDate);
                    date.setDate(date.getDate() - 1);
                     this.reworkForm.planEndDate = date.toISOString().split('T')[0];
                     console.log(this.reworkForm.planEndDate);
        ...
        handleDel(row) {
            ...
            // }
            this.dataFormTwo.prodOrderList.splice(row.$index,1);
        ...
        selectResouce(item, type) {
            ...
            if (type == "设备") {
                ...
                this.workerList[this.index].equipmentName = item.split("_")[1] ;
                this.workerList[this.index].equipmentCode = item.split("_")[2];
        ...
        init(data) {
            console.log("批量1派工", data);
            this.reworkForm.routingCode = data[0].routingCode;
            this.reworkForm.routingName = data[0].routingName;
            this.reworkForm.routingId = data[0].routingId;
            data.forEach((item) => {
                if (item.blankingProductsId) {
                    if (item.blankingQuantity - item.dispatchedQuantity > 0) {
                        item.dispatchQuantity = item.blankingQuantity - item.dispatchedQuantity;
                    }
                } else {
                    item.dispatchQuantity = item.productionQuantity - item.dispatchedQuantity;
                }
            });
            this.dataFormTwo.prodOrderList = JSON.parse(JSON.stringify(data));
            let obj = {
                id: data[0].id,
                dispatch: "1",
            };
        ...
        async handleConfirm() {
            ...
            this.$refs["dataForms"].validate((valid) => {
                if (valid) {
                    console.log("this.reworkForm",this.reworkForm);
                    ...
                    if (!valid_2) {
                        ...
                        for (let index = 0; index < this.dataFormTwo.prodOrderList.length; index++) {
                            const item = this.dataFormTwo.prodOrderList[index];
                            console.log();
                            ...
                            if (!item.blankingProductsId) {
                                ...
                                if ( Number(item.dispatchQuantity) + Number(item.dispatchedQuantity) > Number(item.productionQuantity) ) {
                                    this.$message({
                                        message:
                                            "第" + (index + 1) + "行派工数量+已派工数量不能超过需生产数量",
                                        type: "error",
                                        duration: 1500,
                                    });
                                    break;
                                ...
                                if ( Number(item.dispatchQuantity) + Number(item.dispatchedQuantity) > Number(item.blankingQuantity) ) {
                                    this.$message({
                                        message:
                                            "第" + (index + 1) + "行派工数量+已派工数量不能超过已派下料数量",
                                        type: "error",
                                        duration: 1500,
                                    });
                                    break;
                        ...
                        for (let index = 0; index < this.dataFormTwo.prodOrderList.length; index++) {
                            ...
                            if (!item.blankingProductsId) {
                                ...
                                if (  Number(item.dispatchQuantity) + Number(item.dispatchedQuantity) > Number(item.productionQuantity) ) {
                                    this.$message({
                                        message: "第" + (index + 1) + "行派工数量+已派工数量不能超过需生产数量",
                                        type: "error",
                                        duration: 1500,
                                    });
                                    submitFlag = false;
                                ...
                                console.log(item.dispatchQuantity,item.dispatchedQuantity,item.blankingQuantity);
                                console.log(6);
                                if ( Number(item.dispatchQuantity) + Number(item.dispatchedQuantity) > Number(item.blankingQuantity) ) {
                                    this.$message({
                                        message: "第" + (index + 1) + "行派工数量+已派工数量不能超过已派下料数量",
                                        type: "error",
                                        duration: 1500,
                                    });
                                    submitFlag = false;
                        ...
                        this.workerList.forEach((item) => {
                            ...
                            item.planStartDate = this.fmatDateFun(this.reworkForm.planStartDate);
                            item.planEndDate = this.fmatDateFun(this.reworkForm.planEndDate);
                        });
                        let obj = {
                            prodOrderList: this.dataFormTwo.prodOrderList,
                            dispatchList: this.workerList,
                        };
                        if (submitFlag === false) return;
                        this.btnLoading = true;
                       
                        this.reworkForm.planEndDate=this.fmatDateFun(this.reworkForm.planEndDate)
                        this.reworkForm.planStartDate=this.fmatDateFun(this.reworkForm.planStartDate)
