<template>
  <div class="app-container JNPF-common-layout">
        ...
        <el-form @submit.native.prevent>
          ...
          <el-col :span="6">
            <el-form-item label="车间">
              <JNPFTreeSelect v-model="query.mrpWorkshopId" placeholder="请选择" :options="mrpWorkshopIdOptions"
    ...
    <div v-loading="loading"  style="background-color: #FFFFFF; padding-left: 10px;">
      <div style="display: flex; padding: 1vh 0;">
       <!-- <div style="width: 33%; display: flex; align-items: center;">
          <el-tag closable :hit="true" style="margin-left: 20px; transform: scale(1.2);">车间</el-tag>
        </div> -->
        <!-- 五个小方块 -->
        <div style="display: flex; align-items: center; flex: 1; ">
          ...
          <div style="min-width: 118px; overflow: hidden; display: flex; align-items: center;"
            ...
            <div class="tags" :style="{ backgroundColor: stateList[0].bgc }">
              <strong style="transform: scale(1.2); display: inline-block;">{{ online }}</strong>
            </div>

            <div style="text-align: center;" :style="{ width: settings.rightWidth }">
              <p style="line-height: 22px; ">{{ stateList[0].name }}</p>
              <p style="line-height: 22px; ">{{ online_ratio }}</p>
            </div>
          </div>
          ...
          <div style="min-width: 118px; overflow: hidden; display: flex; align-items: center;"
            ...
            <div class="tags" :style="{ backgroundColor: stateList[1].bgc }">
              <strong style="transform: scale(1.2); display: inline-block;">{{ boot }}</strong>
            </div>

            <div style="text-align: center;" :style="{ width: settings.rightWidth }">
              <p style="line-height: 22px; ">{{ stateList[1].name }}</p>
              <p style="line-height: 22px; ">{{ boot_ratio }}</p>
            </div>
          </div>
          ...
          <div style="min-width: 118px; overflow: hidden; display: flex; align-items: center;"
            ...
            <div class="tags" :style="{ backgroundColor: stateList[2].bgc }">
              <strong style="transform: scale(1.2); display: inline-block;">{{ warn }}</strong>
            </div>

            <div style="text-align: center;" :style="{ width: settings.rightWidth }">
              <p style="line-height: 22px; ">{{ stateList[2].name }}</p>
              <p style="line-height: 22px; ">{{ warn_ratio }}</p>
            </div>
          </div>
          ...
          <div style="min-width: 118px; overflow: hidden; display: flex; align-items: center;"
            ...
            <div class="tags" :style="{ backgroundColor: stateList[3].bgc }">
              <strong style="transform: scale(1.2); display: inline-block;">{{ error }}</strong>
            </div>

            <div style="text-align: center;" :style="{ width: settings.rightWidth }">
              <p style="line-height: 22px; ">{{ stateList[3].name }}</p>
              <p style="line-height: 22px; ">{{ error_ratio }}</p>
            </div>
          </div>
          ...
          <div style="min-width: 118px; overflow: hidden; display: flex; align-items: center;"
            ...
            <div class="tags" :style="{ backgroundColor: stateList[4].bgc }">
              <strong style="transform: scale(1.2); display: inline-block;">{{ offline }}</strong>
            </div>

            <div style="text-align: center;" :style="{ width: settings.rightWidth }">
              <p style="line-height: 22px; ">{{ stateList[4].name }}</p>
              <p style="line-height: 22px; ">{{ offline_ratio }}</p>
            </div>
          </div>
          ...
          <div style="min-width: 118px; overflow: hidden; display: flex; align-items: center;"
            ...
            <div class="tags" :style="{ backgroundColor: stateList[5].bgc }">
              <strong style="transform: scale(1.2); display: inline-block;">{{ totalNum }}</strong>
            </div>

            <div style="text-align: center;" :style="{ width: settings.rightWidth }">
              <p style="line-height: 22px; ">{{ stateList[5].name }}</p>
              <p style="line-height: 22px; ">{{ stateList[5].ratio }}</p>
            </div>
          </div>
      ...
      <div style="height: 100%; background-color: #fff;">
        <div style="height: calc(100vh - 244px); overflow: auto; " :class="{quanping:quanping}" @scroll="">
          <div v-for="(item, index) in list" id="chartdata" :key="index"
            ...
            <div
              ...
              <div style="flex: 1;">
                <div style="color: #949499; font-size: 12px;overflow: hidden;text-overflow:ellipsis; white-space: nowrap;">{{ item.name }}</div>
              ...
              <div style="width: 40px; display: flex; justify-content: space-between; align-items: center;">
                ...
                <span style="font-size: 25px;font-weight: bold; color: gray; padding: 0 5px ; " class="em-span"
                  title="详情" @click="handles(item)">···</span>
              </div>
            </div>
            ...
            <div style="display: flex; align-items: center;">
              <div style="width: 50%;">
                <div style="color: #949499; font-size: 12px;">小时最高：<b style="color: #000;">{{ item.output.max }}</b>
                </div>
                <div style="color: #949499; font-size: 12px;">本日产量：<b style="color: #2BA245;">{{ item.output.sum }}</b>
                </div>
              </div>
              <div style="width: 50%;">
                <div style="color: #949499; font-size: 12px;">本周产量：<b style="color: #000;">{{ item.thisWeekOutput.max }}</b></div>
                <div style="color: #949499; font-size: 12px;">本月产量：<b style="color: #2BA245;">{{ item.thisMonthOutput.max}}</b></div>
              </div>
            </div>

            <!-- 折线图 -->
            <div style="width: 100%; min-height: 12.5vh; padding-left: 5px;" class="equipment-operations"
              id="equipment-operation"></div>
          </div>
        ...
        <div style="width: 100%; max-height: 87vh; overflow-y: auto;">
              ...
              <el-row :gutter="10">
                ...
                <el-col :span="12">
                    <div class="box" style="height: 200px;background-color: #6B3BB7;color: white;">
                        <div class="title"><h1 style="font-size: 80px;line-height: 0px;margin-right:20px;">OEE</h1><span class="text" style="font-size:27px !important;">设备综合效率</span></div>
                        <h2 class="value" style="font-size: 80px;">{{(equipmentWorkpieceRatio.oee*100).toFixed(0)}}%</h2>
                    </div>
                </el-col>
          ...
          <div class="mson-move">
            <div style="display: flex;">
              ...
              <div style="color: #000000; margin-left: 15px;display: flex;align-items: center;justify-content: center;">
                <div>
                  {{ today.sum }}
                </div>
              </div>
          ...
          <div style="width: 100%; padding: 20px; margin-top: 20px; background-color: #fff;">
            ...
            <div style="width: 100%;display: flex;justify-content: space-around;text-align: center; margin-top: 40px;">
              <div style="width: 33%;">
                <div>
                  最近一小时平均循环时间
                </div>
...
<script>
import { getListOutPut, getColorListApi } from "@/api/equipment";
...
import { getDateJSON } from '@/utils/jx/jx-date';
...
export default {
  ...
  methods: {
            ...
            this.stateList.forEach(el => {
              if (item.result.online) { //在线
                if (item.result.boot && el.code == "boot") {
                  ...
                  this.$set(item, "state_color", el.bgc);
    ...
    getTodayOutput() {
      ...
      //moment(this.time.toLocaleString()).format('YYYY-MM-DD 00:00:00')
      //moment(this.time.toLocaleString()).format('YYYY-MM-DD 23:59:59')
    ...
    getThisWeekOutput() {
      ...
      // let startTime = moment(start.toLocaleString()).format('YYYY-MM-DD 00:00:00')
      // let stopTime = moment(this.time.toLocaleString()).format('YYYY-MM-DD 23:59:59')
    ...
    customchart() {
        ...
        var option = {
            ...
            xAxis: {
                ...
                min: moment(Number(new Date().getTime())).format('YYYY-MM-DD 06:00:00'), //将data里最小时间的整点时间设为min,否则min会以data里面的min为开始进行整点递增
        ...
        let _iotData =   {
            ...
            data: [ // 维度0 维度1 维度2
                ...
                //     name: '深睡',
                //     value: [0, '2022-10-26 00:00:00', '2022-10-26 23:59:59']//0,1,2代表y轴的索引，后两位代表x轴数据开始和结束
                // },
                // {
                //     itemStyle: { normal: { color: colors[1] } },
                //     name: '深睡',
                //     value: [0, '2022-10-26 06:13:00', '2022-10-26 8:22:00']
                // }
            ]
        }

        for (var i = 0; i < this.iotTimeData.length; i++) {
            // console.log(this.iotTimeData[i]);
            let item = {
    ...
    getEquipmentWorkpieceRatio(uid){
        ...
        getEquipmentWorkpieceRatio(query).then(res=>{
            ...
            // console.log(this.equipmentWorkpieceRatio);
        })
    },
    ...
    disposeIotTimeData(arr){
        ...
        let _stateRatioData = this.stateRatioData;
