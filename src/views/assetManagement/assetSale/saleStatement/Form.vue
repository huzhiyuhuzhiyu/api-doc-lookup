    ...
    <transition name="el-zoom-in-center">
      <div class="JNPF-preview-main org-form">
        <div class="JNPF-common-page-header">
          <el-page-header @back="goBack" content="生成出售对账单" />
          <div class="options">
              ...
              @click="batchDelete">批量删除</el-button>| -->
                  ...
                  <el-form :model="dataFormTwo" v-bind="dataFormTwo" ref="productForm">
                    <el-table style="border: 1px solid #e3e7ee;" hasNO fixedNO v-bind="dataFormTwo.data"
                      ...
                      <el-table-column prop="orderNo" label="出售单号" width="190" show-overflow-tooltip>
                      ...
                      <el-table-column prop="propertyName" label="资产名称" min-width="200" show-overflow-tooltip>
                        <template slot-scope="scope">
                          <el-form-item :prop="'data.' + scope.$index + '.' + 'propertyName'">
                            <div class="viewData">
                      ...
                      <el-table-column prop="propertyCode" label="资产编码" width="140" show-overflow-tooltip>
                        <template slot-scope="scope">
                          <el-form-item :prop="'data.' + scope.$index + '.' + 'propertyCode'">
                            <div class="viewData">
                              <span>{{ scope.row.propertyCode ? scope.row.propertyCode : '调价' }}</span>
                            </div>
                          </el-form-item>
                        </template>
                      </el-table-column>
                      ...
                      <el-table-column prop="excludingTaxAmount" label="总金额(不含税)" min-width="140">
                        <template slot-scope="scope">
                          <el-form-item :prop="'data.' + scope.$index + '.' + 'excludingTaxAmount'"
                            ...
                            <div class="viewData green">
                              <span v-if="scope.row.propertyCode!='调价'">
                                +{{ scope.row.excludingTaxAmount }}
                              </span>
                              <el-input v-if="scope.row.propertyCode=='调价'" v-model="scope.row.excludingTaxAmount" disabled>
                                {{ scope.row.excludingTaxAmount }}
                              </el-input>
                            </div>
                          </el-form-item>
                      ...
                      <el-table-column prop="taxRate" label="税率" min-width="140">
                        <template slot-scope="scope">
                          <el-form-item :prop="'data.' + scope.$index + '.' + 'taxRate'"
                            :rules="productFormRules.taxRate">
                            <div class="viewData ">
                              <span >
                                {{ scope.row.taxRate }}
                              </span>
                        ...
                        <template slot-scope="scope">
                          <el-form-item :prop="'data.' + scope.$index + '.' + 'taxAmount'"
                            ...
                            <div class="viewData green">
                              ...
                              <el-input v-if="scope.row.propertyCode=='调价'" v-model="scope.row.taxAmount" disabled>
                                {{ scope.row.taxAmount }}
                              </el-input>
                            </div>
                          </el-form-item>
                        ...
                        <template slot-scope="scope">
                          <el-form-item :prop="'data.' + scope.$index + '.' + 'includingTaxAmount'"
                            ...
                            <div  class="viewData green">
                              ...
                              <el-input v-if="scope.row.propertyCode=='调价'" v-model="scope.row.includingTaxAmount" >
                                {{ scope.row.includingTaxAmount }}
                              </el-input>
                            </div>
                          </el-form-item>
...
<script>
import {addPropertyAccountReconciliation} from '@/api/bimPropertyCategory/index'
...
export default {
  ...
  methods: {
    ...
    init(data) {
      ...
      _data.forEach((item) => {
        let excludingTaxAmount =this.jnpf.numberFormat(item.totalAmount / (1 + (item.tax * 1) / 100), 6)
        this.dataFormTwo.data.push({
          accountsReceivableId: '',
          ...
          id: null,
          includingTaxAmount: item.totalAmount,
          ...
          remark: item.remark,
          excludingTaxPrice: this.jnpf.numberFormat(item.costPrice / (1 + (item.tax * 1) / 100), 2),
          excludingTaxAmount: excludingTaxAmount,
          taxAmount:this.jnpf.numberFormat(item.totalAmount- excludingTaxAmount),
          taxRate: item.tax,
      ...
      this.dataFormTwo.data.forEach((item, index) => {
        this.includingTaxAmount += this.jnpf.numberFormat(item.includingTaxAmount)
      ...
      this.$refs['elForm'].validate((valid) => {
        if (valid) {
          if (!valid_2) {
            ...
            if (submitFlag) {
              setTimeout(() => {
                addPropertyAccountReconciliation(_data)
                  .then((res) => {
